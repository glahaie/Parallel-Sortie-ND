...
clTreePopulation::~clTreePopulation() {
  ...
  if (mp_cIntLabels) { //int labels
    for (i = 0; i < m_iNumSpecies; i++) {
      for (j = 0; j < m_iNumTypes; j++) {
        for (k = 0; k < mp_iNumTreeIntVals[i] [j]; k++) {
          delete[] mp_cIntLabels[i] [j] [k];
          mp_cIntLabels[i] [j] [k] = NULL;
        }
        delete[] mp_cIntLabels[i] [j];
        mp_cIntLabels[i] [j] = NULL;
      }
      delete[] mp_cIntLabels[i];
      mp_cIntLabels[i] = NULL;
    }
    delete[] mp_cIntLabels;
    mp_cIntLabels = NULL;
  }
  if (mp_cFloatLabels) { //float labels
    for (i = 0; i < m_iNumSpecies; i++) {
      for (j = 0; j < m_iNumTypes; j++) {
        for (k = 0; k < mp_iNumTreeFloatVals[i] [j]; k++) {
          delete[] mp_cFloatLabels[i] [j] [k];
          mp_cFloatLabels[i] [j] [k] = NULL;
        }
        delete[] mp_cFloatLabels[i] [j];
        mp_cFloatLabels[i] [j] = NULL;
      }
      delete[] mp_cFloatLabels[i];
      mp_cFloatLabels[i] = NULL;
    }
    delete[] mp_cFloatLabels;
    mp_cFloatLabels = NULL;
  }
  if (mp_cCharLabels) { //char labels
    for (i = 0; i < m_iNumSpecies; i++) {
      for (j = 0; j < m_iNumTypes; j++) {
        for (k = 0; k < mp_iNumTreeCharVals[i] [j]; k++) {
          delete[] mp_cCharLabels[i] [j] [k];
          mp_cCharLabels[i] [j] [k] = NULL;
        }
        delete[] mp_cCharLabels[i] [j];
        mp_cCharLabels[i] [j] = NULL;
      }
      delete[] mp_cCharLabels[i];
      mp_cCharLabels[i] = NULL;
    }
    delete[] mp_cCharLabels;
    mp_cCharLabels = NULL;
  }
  if (mp_cBoolLabels) { //bool labels
    for (i = 0; i < m_iNumSpecies; i++) {
      for (j = 0; j < m_iNumTypes; j++) {
        for (k = 0; k < mp_iNumTreeBoolVals[i] [j]; k++) {
          delete[] mp_cBoolLabels[i] [j] [k];
          mp_cBoolLabels[i] [j] [k] = NULL;
        }
        delete[] mp_cBoolLabels[i] [j];
        mp_cBoolLabels[i] [j] = NULL;
      }
      delete[] mp_cBoolLabels[i];
      mp_cBoolLabels[i] = NULL;
    }
    delete[] mp_cBoolLabels;
    mp_cBoolLabels = NULL;
  }
  ...
  for (i = 0; i < m_iNumSpecies; i++) {
    ...
    if (mp_iNumTreeCharVals) {
      delete[] mp_iNumTreeCharVals[i];
      mp_iNumTreeCharVals[i] = NULL;
    }
  ...
  if (mp_iNumTreeCharVals) {
    delete[] mp_iNumTreeCharVals;
    mp_iNumTreeCharVals = NULL;
  }
  if (mp_iNumTreeBoolVals) {
...
clTreePopulation::clTreePopulation(clSimManager * p_oSimManager) :
  ...
  try
  {
    ...
    strcpy( m_cNameString, "treepopulation" );
    //         m_iNumHeightDivs = 10;
    m_iSizeHeightDivs = 4;
    m_iLengthGrids = 8;
    m_iNumTypes = 7;
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clTreePopulation::clTreePopulation" );
    throw( stcErr );
  }
}
...
clTree * clTreePopulation::GetShortestTreeInCell(int iX, int iY) {
  ...
  if ( iX < 0 || iX >= m_iNumXCells || iY < 0 || iY >= m_iNumYCells )
  {
    ...
    strcpy( stcErr.cFunction, "clTreePopulation::GetShortestTreeInCell" );
    sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Invalid grid numbers.  X = ", iX, ", Y = ", iY );
    throw( stcErr );
  }
...
clTree * clTreePopulation::GetTallestTreeInCell(int iX, int iY) {
  ...
  if ( iX < 0 || iX >= m_iNumXCells || iY < 0 || iY >= m_iNumYCells )
  {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::GetTallestTreeInCell" );
    sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Invalid grid numbers.  X = ", iX, ", Y = ", iY );
    throw( stcErr );
  }
...
void clTreePopulation::BarebonesDataSetup(DOMDocument * p_oDoc) {
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clTreePopulation::BarebonesDataSetup" );
      stcErr.iErrorCode = DATA_MISSING;
      strcpy( stcErr.cMoreInfo, "tr_speciesList" );
      throw( stcErr );
    }
    ...
    if ( 0 == iNumNodes )
    {
      ...
      strcpy( stcErr.cFunction, "clTreePopulation::BarebonesDataSetup" );
      stcErr.iErrorCode = DATA_MISSING;
      strcpy( stcErr.cMoreInfo, "tr_species" );
      throw( stcErr );
    }
    ...
    for ( i = 0; i < iNumNodes; i++ )
    {
      ...
      XMLString::release(&sTag);
      strncpy( mp_speciesCodes[i].cName, cData, MAX_SPECIES_NAME_SIZE - 1 );
      //This doesn't always happen with strncpy - make it explicit
      mp_speciesCodes[i].cName[MAX_SPECIES_NAME_SIZE - 1] = '\0';
      delete[] cData;
    }
...
void clTreePopulation::GetPlotDimensions() {
  ...
  if ( !p_oPlot) {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clTreePopulation::GetData");
    stcErr.iErrorCode = CANT_FIND_OBJECT;
    strcpy(stcErr.cMoreInfo, "clPlot");
    throw( stcErr );
  }
...
void clTreePopulation::ReadParameters(DOMDocument * p_oDoc) {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreePopulation::ReadParameters" );
    throw( stcErr );
  }
}
...
void clTreePopulation::DoTreeDataStructureSetup() {
  try
  {
    ...
    mp_iNumTreeCharVals = new short int * [m_iNumSpecies];
    ...
    for ( i = 0; i < m_iNumSpecies; i++ )
    {
      ...
      mp_iNumTreeCharVals[i] = new short int[m_iNumTypes];
    ...
    for ( i = 0; i < iNumBehaviors; i++ )
    {
      ...
      for ( j = 0; j < iNumCombos; j++ )
      {
        ...
        mp_iNumTreeCharVals[combo.iSpecies] [combo.iType] += iNewChars;
    ...
    mp_cIntLabels = new char * * * [m_iNumSpecies];
    mp_cFloatLabels = new char * * * [m_iNumSpecies];
    mp_cCharLabels = new char * * * [m_iNumSpecies];
    mp_cBoolLabels = new char * * * [m_iNumSpecies];
    for ( i = 0; i < m_iNumSpecies; i++ )
    {
      mp_cIntLabels[i] = new char * * [m_iNumTypes];
      mp_cFloatLabels[i] = new char * * [m_iNumTypes];
      mp_cCharLabels[i] = new char * * [m_iNumTypes];
      mp_cBoolLabels[i] = new char * * [m_iNumTypes];
      for ( j = 0; j < m_iNumTypes; j++ )
      {
        mp_cIntLabels[i] [j] = new char * [mp_iNumTreeIntVals[i] [j]];
        for ( k = 0; k < mp_iNumTreeIntVals[i] [j]; k++ )
        {
          mp_cIntLabels[i] [j] [k] = new char[MAX_TREE_LABEL_SIZE];
          strcpy( mp_cIntLabels[i] [j] [k], "\0" );
        }
        mp_cFloatLabels[i] [j] = new char * [mp_iNumTreeFloatVals[i] [j]];
        for ( k = 0; k < mp_iNumTreeFloatVals[i] [j]; k++ )
        {
          mp_cFloatLabels[i] [j] [k] = new char[MAX_TREE_LABEL_SIZE];
          strcpy( mp_cFloatLabels[i] [j] [k], "\0" );
        }
        mp_cCharLabels[i] [j] = new char * [mp_iNumTreeCharVals[i] [j]];
        for ( k = 0; k < mp_iNumTreeCharVals[i] [j]; k++ )
        {
          mp_cCharLabels[i] [j] [k] = new char[MAX_TREE_LABEL_SIZE];
          strcpy( mp_cCharLabels[i] [j] [k], "\0" );
        }
        mp_cBoolLabels[i] [j] = new char * [mp_iNumTreeBoolVals[i] [j]];
        for ( k = 0; k < mp_iNumTreeBoolVals[i] [j]; k++ )
        {
          mp_cBoolLabels[i] [j] [k] = new char[MAX_TREE_LABEL_SIZE];
          strcpy( mp_cBoolLabels[i] [j] [k], "\0" );
        }
      } //end of for (j = 0; j < m_iNumTypes; j++)
    } //end of for (i = 0; i < m_iNumSpecies; i++)
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreePopulation::DoTreeDataStructureSetup" );
    throw( stcErr );
  }
}
...
void clTreePopulation::CreateTreesFromInitialDensities(DOMDocument * p_oDoc) {
  try
  {
    ...
    if ( NULL == mp_fSizeClasses )
    {
      delete[] p_fDensities;
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromInitialDensities" );
      stcErr.iErrorCode = DATA_MISSING;
      strcpy( stcErr.cMoreInfo, "Size classes must be defined to use initial densities." );
      throw( stcErr );
    }
    ...
    for ( i = 0; i < iNumSpecies; i++ )
    {
      ...
      { //if it's not a species we recognize error out
        ...
        strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromInitialDensities" );
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cMoreInfo, "Unrecognized species in initial densities:  " );
        strcat( stcErr.cMoreInfo, cData );
      ...
      for ( j = 0; j < iNumClasses; j++ )
      {
        ...
        if ( strcmp("Seedling", cData) == 0 )
        {
          ...
          if (fNumTrees < 0) {
            ...
            stcErr.iErrorCode = BAD_DATA;
            strcpy(stcErr.cFunction, "clTreePopulation::CreateTreesFromInitialDensities");
            strcpy(stcErr.cMoreInfo, "Found negative tree density.");
            delete[] cData; cData = NULL;
            delete[] p_fDensities;
            throw(stcErr);
          }
          if (fNumTrees == 0) {
            ...
            if (cData[0] != '0') {
              ...
              stcErr.iErrorCode = BAD_DATA;
              strcpy(stcErr.cFunction, "clTreePopulation::CreateTreesFromInitialDensities");
              sprintf(stcErr.cMoreInfo, "%s%s%s", "Tree density is not a number: \"", cData, "\".");
              delete[] cData; cData = NULL;
              delete[] p_fDensities;
              throw(stcErr);
            }
        ...
        { //not the "seedling" class - creating saplings or adults
          ...
          if ( fUpperLimit != 0 )
          {
            ...
            if (false == bFound) {
              ...
              stcErr.iErrorCode = BAD_DATA;
              strcpy(stcErr.cFunction, "clTreePopulation::CreateTreesFromInitialDensities");
              sprintf(stcErr.cMoreInfo, "%s%f%s", "Found unrecognized tree size class: \"", fUpperLimit, "\".");
              delete[] p_fDensities;
              throw(stcErr);
            }
            ...
            if (fNumTrees < 0) {
              ...
              stcErr.iErrorCode = BAD_DATA;
              strcpy(stcErr.cFunction, "clTreePopulation::CreateTreesFromInitialDensities");
              sprintf(stcErr.cMoreInfo, "%s%f%s", "Found negative tree density: \"", fNumTrees, "\".");
              delete[] cData; cData = NULL;
              delete[] p_fDensities;
              throw(stcErr);
            }
            ...
            if (strData[0] != '0') {
              ...
              stcErr.iErrorCode = BAD_DATA;
              strcpy(stcErr.cFunction, "clTreePopulation::CreateTreesFromInitialDensities");
              sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized tree size class \"", cData, "\".");
              delete[] cData; cData = NULL;
              delete[] p_fDensities;
              throw(stcErr);
            }
    ...
    if ( fSeedlingHeight1Upper < 0.1 || fSeedlingHeight2Upper < 0.1 || fSeedlingHeight2Upper < fSeedlingHeight1Upper )
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromInitialDensities" );
      strcpy( stcErr.cMoreInfo, "Seedling height classes must be greater than 10 cm and cannot overlap." );
      delete[] p_fDensities;
      throw( stcErr );
    }
    ...
    * * * p_ints = NULL, * * * p_floats = NULL, //array for each data member type
    ...
    if ( 0 != p_oNodeList->getLength() )
    {
      ...
      if ( fMapPlotLenX > m_fPlotLengthX )
      {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        sprintf( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
        sprintf( "%s%s%s", stcErr.cMoreInfo, "Tree map X length (", cData, ") larger than plot" );
        throw( stcErr );
      }
    }
    ...
    if ( 0 != p_oNodeList->getLength() )
    {
      ...
      if ( fMapPlotLenY > m_fPlotLengthY )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        sprintf( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
        sprintf( "%s%s%s", stcErr.cMoreInfo, "Tree map Y length (", cData, ") larger than plot" );
        throw( stcErr );
        delete[] cData; cData = NULL;
      }
      delete[] cData; cData = NULL;
    ...
    if ( 0 != p_oNodeList->getLength() )
    {
      ...
      if ( iTemp > m_iNumSpecies )
      {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
        strcpy( stcErr.cMoreInfo, "Tree map has more species than the par file" );
        throw( stcErr );
      }
      else if ( 0 != iTemp )
      {
        for ( i = 0; i < iTemp; i++ )
        {
          ...
          if ( -1 == iSpecies )
          {
            ...
            stcErr.iErrorCode = BAD_DATA;
            strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
            strcpy( stcErr.cMoreInfo, "Tree map has species not in par file - " );
            strcat( stcErr.cMoreInfo, cData );
            delete[] cData; cData = NULL;
            throw(stcErr);
    ...
    for ( i = 0; i < p_oSettingsList->getLength(); i++ )
    {
      ...
      if ( -1 == iSpecies || iType < 0 || iType > woody_debris || ( 0 == iType && strcmp( cData, "0" ) != 0 ) )
      {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clPopulation::CreateTreesFromTreeMap" );
        sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Invalid species or type.  Species: ", iSpecies, " Type: ", iType );
        throw( stcErr );
      }
      ...
      if ( 0 != p_oNodeList->getLength() )
      {
        ...
        if ( p_iNumInts[iSpecies] [iType] > 0 )
        {
          ...
          if ( p_ints[iSpecies] [iType] )
          {
            ...
            stcErr.iErrorCode = BAD_DATA;
            strcpy( stcErr.cFunction, "clPopulation::CreateTreesFromTreeMap" );
            sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Duplicate settings for this species and type.  Species: ",
                iSpecies, " Type: ", iType );
            throw( stcErr );
          }
          ...
          for ( j = 0; j < p_iNumInts[iSpecies] [iType]; j++ )
          {
            ...
            if ( 0 == p_ints[iSpecies] [iType] [j].iCodeInMap && strcmp( cData, "0" ) != 0 )
            {
              ...
              stcErr.iErrorCode = BAD_DATA;
              strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
              sprintf( stcErr.cMoreInfo, "%s%s", "Can't convert code to number: ", cData );
              delete[] cData; cData = NULL;
              throw( stcErr );
            }
            delete[] cData; cData = NULL;
      ...
      if ( 0 != p_oNodeList->getLength() )
      {
        ...
        if ( p_iNumChars[iSpecies] [iType] > 0 )
        {
          ...
          for ( j = 0; j < p_iNumChars[iSpecies] [iType]; j++ )
          {
            ...
            cData = XMLString::transcode( p_oElement->getAttributeNode( sVal )->getNodeValue() );
            XMLString::release(&sVal);
            p_chars[iSpecies] [iType] [j].iCodeForTree = GetCharDataCode( cData, iSpecies, iType );
      ...
      for ( j = 0; j < m_iNumTypes; j++ )
      {
        ...
        if ( p_ints[i] [j] || p_floats[i] [j] || p_chars[i] [j] || p_bools[i] [j] )
        {
          ...
          { //required values absent - error
            ...
            stcErr.iErrorCode = DATA_MISSING;
            strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
            strcpy( stcErr.cMoreInfo, "Cannot load map without X, Y, and diameter values." );
            throw( stcErr );
          }
        }
      }
    ...
    for ( i = 0; i < iNumTrees; i++ )
    {
      ...
      if ( ( 0 == iSpecies && strcmp( cData, "0" ) != 0 ) || iSpecies < 0 || iSpecies >= m_iNumSpecies )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
        sprintf( stcErr.cMoreInfo, "%s%s", "Unrecognized species in tree map:  ", cData );
        delete[] cData; cData = NULL;
        throw( stcErr );
      }
      delete[] cData; cData = NULL;
      ...
      if ( ( 0 == iType && strcmp( cData, "0" ) != 0 ) || iType < 0 || iType >= m_iNumTypes )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
        sprintf( stcErr.cMoreInfo, "%s%s", "Unrecognized type in tree map: ", cData );
        delete[] cData; cData = NULL;
        throw( stcErr );
      }
      delete[] cData; cData = NULL;
      ...
      for ( j = 0; j < p_oChildren->getLength(); j++ )
      {
        ...
        { //unrecognized code - throw error
          ...
          strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
          stcErr.iErrorCode = BAD_DATA;
          sprintf( stcErr.cMoreInfo, "%s%d", "Unrecognized float code in map: ", iMapCode );
          throw( stcErr );
        }
        ...
        if ( 0 == fTemp && cData[0] != '0' )
        {
          ...
          strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
          stcErr.iErrorCode = BAD_DATA;
          sprintf( stcErr.cMoreInfo, "%s%s", "Invalid float value in map: ", cData );
          delete[] cData; cData = NULL;
          throw( stcErr );
        }
        delete[] cData; cData = NULL;
      ...
      if ( fDiam < 0 && fHeight > 0 )
      {
        if ( seedling == iType )
        {
          ...
          if ( fHeight < 0.10 )
          {
            ...
            stcErr.iErrorCode = BAD_DATA;
            strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
            strcpy( stcErr.cMoreInfo, "Seedlings must be at least 10 cm tall." );
            throw( stcErr );
          }
      ...
      if ( fX < 0 || fY < 0 || fDiam < 0 )
      {
        ...
        stcErr.iErrorCode = DATA_MISSING;
        strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
        strcpy( stcErr.cMoreInfo, "X, Y, or diameter for a tree" );
        throw( stcErr );
      }
      ...
      for ( j = 0; j < p_oChildren->getLength(); j++ )
      {
        ...
        { //unrecognized code - throw error
          ...
          strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
          stcErr.iErrorCode = BAD_DATA;
          sprintf( stcErr.cMoreInfo, "%s%d", "Unrecognized int code in map: ", iMapCode );
          throw( stcErr );
        }
        cData = XMLString::transcode( p_oElement->getFirstChild()->getNodeValue() );
        iTemp = atoi( cData );
        ...
        if ( 0 == iTemp && strcmp( cData, "0" ) != 0 )
        {
          ...
          strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
          stcErr.iErrorCode = BAD_DATA;
          sprintf( stcErr.cMoreInfo, "%s%s", "Invalid int value in map: ", cData );
          delete[] cData; cData = NULL;
          throw( stcErr );
        }
        delete[] cData; cData = NULL;
      ...
      for ( j = 0; j < p_oChildren->getLength(); j++ )
      {
        ...
        { //unrecognized code - throw error
          ...
          strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
          stcErr.iErrorCode = BAD_DATA;
          sprintf( stcErr.cMoreInfo, "%s%d", "Unrecognized char code in map: ", iMapCode );
          throw( stcErr );
        }
        cData = XMLString::transcode( p_oElement->getFirstChild()->getNodeValue() );
      ...
      for ( j = 0; j < p_oChildren->getLength(); j++ )
      {
        ...
        { //unrecognized code - throw error
          ...
          strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTreeMap" );
          stcErr.iErrorCode = BAD_DATA;
          sprintf( stcErr.cMoreInfo, "%s%d", "Unrecognized bool code in map: ", iMapCode );
          throw( stcErr );
        }
        cData = XMLString::transcode( p_oElement->getFirstChild()->getNodeValue() );
        ...
        { //invalid bool - throw error
          modelErr stcErr;
          strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromMapFile" );
          stcErr.iErrorCode = BAD_DATA;
          sprintf( stcErr.cMoreInfo, "%s%s", "Invalid bool value in map: ", cData );
          delete[] cData; cData = NULL;
          throw( stcErr );
        }
        delete[] cData; cData = NULL;
      ...
      for ( j = 0; j < p_oNodeList->getLength(); j++ )
      {
                        ...
                        && iTreeCode != mp_iDbhCode[iSpecies] [iType] ) )
        {
          ...
          {
            modelErr stcErr;
            strcpy( stcErr.cFunction, "clTreePopulation::CreateTreeFromTreeMap" );
            stcErr.iErrorCode = BAD_DATA;
            sprintf( stcErr.cMoreInfo, "%s%s", "Invalid float value in map: ", cData );
            delete[] cData; cData = NULL;
            throw( stcErr );
...
/////////////////////////////////////////////////////////////////////////////*/
/* */
clTreeSearch * clTreePopulation::Find(const char * cArgs) {
  using namespace std;
  ...
  string strTempArg, strArgType, strArgVal, strArgs, strTemp; //for use by anybody - but don't plan on needing it later
  ...
  if ( cArgs == NULL ) return p_results;
  strArgs = cArgs;
  if ( strArgs.length() == 0 ) return p_results;
  ...
  while ( strArgs.length() > 0 )
  {
    ...
    //from strArgs
    strTempArg = strArgs.substr( 0, strArgs.find_first_of( "::" ) );
    pos = strArgs.find_first_of( "::" ) != string::npos ? strArgs.find_first_of( "::" ) + 2 : strArgs.length();
    strArgs.erase( 0, pos );
    ...
    if ( "type" == strArgType )
    {
      if ( "" != strArgVal )
      {
        ...
        while ( strArgVal.length() > 0 )
        {
          ...
          {
            modelErr stcErr;
            strcpy( stcErr.cFunction, "clTreePopulation::Find" );
            stcErr.iErrorCode = BAD_DATA;
            sprintf( stcErr.cMoreInfo, "%s%d", "Unrecognized type:  ", iType );
            throw( stcErr );
          }
      ...
      if ( "" != strArgVal )
      {
        ...
        while ( strArgVal.length() > 0 )
        {
          ...
          if ( iSpecies < 0 || iSpecies >= m_iNumSpecies )
          {
            ...
            strcpy( stcErr.cFunction, "clTreePopulation::Find" );
            stcErr.iErrorCode = BAD_DATA;
            strcpy( stcErr.cMoreInfo, "Unrecognized species number:  " );
            strcat( stcErr.cMoreInfo, strTemp.c_str() );
            throw( stcErr );
          }
    ...
    else if ( "height" == strArgType )
    {
      if ( "" != strArgVal )
      {
        ...
        if ( 0 == fTemp && "0" != strArgVal.substr( 0, 1 ) )
        {
          ...
          strcpy( stcErr.cFunction, "clTreePopulation::Find" );
          stcErr.iErrorCode = BAD_DATA;
          strcpy( stcErr.cMoreInfo, "Unrecognized float value:  " );
          strcat( stcErr.cMoreInfo, strArgVal.c_str() );
          throw( stcErr );
        }
    ...
    else
    {
      ...
      strcpy( stcErr.cFunction, "clTreePopulation::Find" );
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cMoreInfo, "Unknown search argument:  " );
      strcat( stcErr.cMoreInfo, strArgType.c_str() );
      throw( stcErr );
    }
  } //end of while (strArgs.length() > 0)
...
void clTreePopulation::UpdateTree(clTree * p_oTree, short int iCode, int iValue) {
  ...
  try
  {
    ...
    if ( iCode < 0 || iCode >= mp_iNumTreeIntVals[p_oTree->m_iSpecies] [p_oTree->m_iType] )
    {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree(int)" );
      sprintf( stcErr.cMoreInfo, "%s%d", "Invalid data code:  ", iCode );
      throw( stcErr );
    }
...
void clTreePopulation::UpdateTree(clTree * p_oTree, short int iCode,
    const char * cValue) {
  //For chars - no allometry - so just update the value
  try
  {
    //Make sure the code is a good one - if not, throw error
    if ( iCode < 0 || iCode >= mp_iNumTreeCharVals[p_oTree->m_iSpecies] [p_oTree->m_iType] )
    {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree(char)" );
      sprintf( stcErr.cMoreInfo, "%s%d", "Invalid data code:  ", iCode );
      throw( stcErr );
    }
    strncpy( p_oTree->mp_cCharValues[iCode], cValue, MAX_TREE_CHAR_VALUE_SIZE - 1 );
    //This doesn't always happen with strncpy - make it explicit
    p_oTree->mp_cCharValues[iCode] [MAX_TREE_CHAR_VALUE_SIZE - 1] = '\0';
  }
  catch ( modelErr & err )
  {
    throw( err );
...
void clTreePopulation::UpdateTree(clTree * p_oTree, short int iCode, bool bValue) {
  ...
  try
  {
    ...
    if ( iCode < 0 || iCode >= mp_iNumTreeBoolVals[p_oTree->m_iSpecies] [p_oTree->m_iType] )
    {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree(bool)" );
      sprintf( stcErr.cMoreInfo, "%s%d", "Invalid data code:  ", iCode );
      throw( stcErr );
    }
...
void clTreePopulation::UpdateTree(clTree * p_oTree, short int iCode,
  ...
  try
  {
    ...
    if ( iCode < 0 || iCode >= mp_iNumTreeFloatVals[iSp] [iType] )
    {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree(float)" );
      sprintf( stcErr.cMoreInfo, "%s%d", "Invalid data code:  ", iCode );
      throw( stcErr );
    }
    ...
    if ( iCode == mp_iXCode[iSp] [iType] || iCode == mp_iYCode[iSp] [iType] )
    {
      ...
      strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree" );
      strcpy( stcErr.cMoreInfo, "X and Y cannot be changed." );
      throw( stcErr );
    }
      ...
      if ( seedling == p_oTree->m_iType )
      {
        ...
        { //update diam10
          ...
          if (fValue < MINDIAM) {
            ...
            stcErr.iErrorCode = ILLEGAL_OP;
            strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree" );
            strcpy( stcErr.cMoreInfo, "Attempt to set seedling diameter below minimum." );
            throw( stcErr );
          }
        ...
        else if ( mp_iHeightCode[iSp] [iType] == iCode )
        {
          ...
          if (fValue <= 0) {
            ...
            stcErr.iErrorCode = ILLEGAL_OP;
            strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree" );
            strcpy( stcErr.cMoreInfo, "Attempt to set seedling height to negative value." );
            throw( stcErr );
          }
      ...
      else if ( sapling == p_oTree->m_iType )
      {
        ...
        { //update diam10
          ...
          if (fValue < m_fNewSeedlingDiam10) {
            ...
            stcErr.iErrorCode = ILLEGAL_OP;
            strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree" );
            strcpy( stcErr.cMoreInfo, "Attempt to set sapling diameter below minimum." );
            throw( stcErr );
          }
          p_oTree->mp_fFloatValues[iCode] = fValue;
        ...
        { //update height
          ...
          if (fValue <= 0) {
            ...
            stcErr.iErrorCode = ILLEGAL_OP;
            strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree" );
            strcpy( stcErr.cMoreInfo, "Attempt to set sapling height to negative value." );
            throw( stcErr );
          }
        ...
        { //update dbh
          if (fValue <= 0) {
            ...
            stcErr.iErrorCode = ILLEGAL_OP;
            strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree" );
            strcpy( stcErr.cMoreInfo, "Attempt to set sapling DBH to negative value." );
            throw( stcErr );
          }
      ...
      else if ( adult == p_oTree->m_iType || snag == p_oTree->m_iType )
      {
        ...
        { //update height
          if (fValue <= 0) {
            ...
            stcErr.iErrorCode = ILLEGAL_OP;
            strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree" );
            strcpy( stcErr.cMoreInfo, "Attempt to set adult height to negative value." );
            throw( stcErr );
          }
          p_oTree->mp_fFloatValues[iCode] = fValue;
          //calculate dbh based on height
        ...
        { //update dbh
          if (fValue <= 0) {
            ...
            stcErr.iErrorCode = ILLEGAL_OP;
            strcpy( stcErr.cFunction, "clTreePopulation::UpdateTree" );
            strcpy( stcErr.cMoreInfo, "Attempt to set adult DBH to negative value." );
            throw( stcErr );
          }
          p_oTree->mp_fFloatValues[mp_iDbhCode[iSp] [iType]] = fValue;
          //calculate height based on dbh
...
void clTreePopulation::ChangeTreeType(clTree * p_oTree, enum iTreeType iNewType) {
  try
  {
    ...
    p_cCharCopy = new char * [mp_iNumTreeCharVals[iSp] [iOldType]];
    for ( i = 0; i < mp_iNumTreeCharVals[iSp] [iOldType]; i++ )
    {
      p_cCharCopy[i] = new char[MAX_TREE_CHAR_VALUE_SIZE];
      strcpy( p_cCharCopy[i], p_oTree->mp_cCharValues[i] );
    }
    ...
    delete[] p_oTree->mp_cCharValues; p_oTree->mp_cCharValues = NULL;
    ...
    p_oTree->mp_cCharValues = new char * [mp_iNumTreeCharVals[iSp] [iNewType]];
    for ( i = 0; i < mp_iNumTreeCharVals[iSp] [iNewType]; i++ )
    {
      p_oTree->mp_cCharValues[i] = new char[MAX_TREE_CHAR_VALUE_SIZE];
      strcpy( p_oTree->mp_cCharValues[i], "" );
    }
    ...
    for ( i = 0; i < mp_iNumTreeIntVals[iSp] [iOldType]; i++ )
      for ( j = 0; j < mp_iNumTreeIntVals[iSp] [iNewType]; j++ )
        if ( strcmp( mp_cIntLabels[iSp] [iNewType] [j], mp_cIntLabels[iSp] [iOldType] [i] ) == 0 )
          p_oTree->mp_iIntValues[j] = p_iIntCopy[i];
    for ( i = 0; i < mp_iNumTreeFloatVals[iSp] [iOldType]; i++ )
      for ( j = 0; j < mp_iNumTreeFloatVals[iSp] [iNewType]; j++ )
        if ( strcmp( mp_cFloatLabels[iSp] [iNewType] [j], mp_cFloatLabels[iSp] [iOldType] [i] ) == 0 )
          p_oTree->mp_fFloatValues[j] = p_fFloatCopy[i];
    for ( i = 0; i < mp_iNumTreeCharVals[iSp] [iOldType]; i++ )
      for ( j = 0; j < mp_iNumTreeCharVals[iSp] [iNewType]; j++ )
        if ( strcmp( mp_cCharLabels[iSp] [iNewType] [j], mp_cCharLabels[iSp] [iOldType] [i] ) == 0 )
          strcpy( p_oTree->mp_cCharValues[j], p_cCharCopy[i] );
    for ( i = 0; i < mp_iNumTreeBoolVals[iSp] [iOldType]; i++ )
      for ( j = 0; j < mp_iNumTreeBoolVals[iSp] [iNewType]; j++ )
        if ( strcmp( mp_cBoolLabels[iSp] [iNewType] [j], mp_cBoolLabels[iSp] [iOldType] [i] ) == 0 )
...
float clTreePopulation::GetMinAdultDBH(int iSpecies) {
  if ( iSpecies < 0 || iSpecies >= m_iNumSpecies )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::GetMinAdultDBH" );
    sprintf( stcErr.cMoreInfo, "%s%d", "Unrecognized species ", iSpecies );
    throw( stcErr );
  }
...
void clTreePopulation::AddTreeToHashTable(clTree * p_oNewTree) {
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clTreePopulation::AddTreeToHashTable" );
      stcErr.iErrorCode = BAD_DATA;
      delete p_oNewTree;
      sprintf( stcErr.cMoreInfo, "%s%f%s%f", "Tree outside plot: X = ", fX, " Y = ", fY );
      throw( stcErr );
    }
...
void clTreePopulation::RemoveTreeFromHashTable(clTree * p_oByeTree) {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreePopulation::RemoveTreeFromHashTable" );
    throw( stcErr );
  }
}
...
void clTreePopulation::UpdateTreeInHashTable(clTree * p_oChangedTree) {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreePopulation::UpdateTreeInHashTable" );
    throw( stcErr );
  }
}
...
void clTreePopulation::EmptyHashTable() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreePopulation::EmptyHashTable" );
    throw( stcErr );
  }
}
...
void clTreePopulation::SortHashTable() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreePopulation::SortHashTable" );
    throw( stcErr );
  }
}
...
void clTreePopulation::TimestepCleanup() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreePopulation::TimestepCleanup" );
    throw( stcErr );
  }
}
...
/////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::TranslateSpeciesNameToCode(const char * cSpeciesName) {
  ...
  for ( i = 0; i < m_iNumSpecies; i++ )
  {
    if ( strcmp( mp_speciesCodes[i].cName, cSpeciesName ) == 0 )
...
/////////////////////////////////////////////////////////////////////////////
const char* clTreePopulation::TranslateSpeciesCodeToName(int iSpecies) {
  ...
  for ( i = 0; i < m_iNumSpecies; i++ )
  {
    if ( mp_speciesCodes[i].iCode == iSpecies )
      return mp_speciesCodes[i].cName;
  } //end of for (i = 0; i < m_iNumSpecies; i++)
...
clTree * clTreePopulation::CreateTree(float fX, float fY, int iSp, int iType,
  ...
  if ( fX < 0 || fY < 0 || fX >= m_fPlotLengthX || fY >= m_fPlotLengthY )
  {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::CreateTree" );
    sprintf( stcErr.cMoreInfo, "%s%f%s%f", "Either X or Y is outside plot.  X = ", fX, " Y = ", fY );
    throw( stcErr );
  }
  ...
  if ( iSp < 0 || iSp >= m_iNumSpecies )
  {
    ...
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::CreateTree" );
    sprintf( stcErr.cMoreInfo, "%s%d", "Unrecognized species ", iSp );
    throw( stcErr );
  }
  ...
  if ( ( fDiam <= 0 && seedling != iType ) || ( fDiam < 0 && seedling == iType ) )
  {
    ...
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::CreateTree" );
    strcpy( stcErr.cMoreInfo, "Diameter cannot be less than zero." );
    throw( stcErr );
  ...
  if ( seedling == iType )
  {
    ...
    p_oTree = new clTree( iType, iSp, mp_iNumTreeFloatVals[iSp] [iType], mp_iNumTreeIntVals[iSp] [iType],
        mp_iNumTreeCharVals[iSp] [iType], mp_iNumTreeBoolVals[iSp] [iType], this );
  ...
  else
  {
    ...
    strcpy( stcErr.cFunction, "clTreePopulation::CreateTree" );
    stcErr.iErrorCode = BAD_DATA;
    sprintf( stcErr.cMoreInfo, "%s%d%s", "The tree type \"", iType, "\" is not supported." );
    throw( stcErr );
  }
...
// RegisterDataMember()
//////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::RegisterDataMember(const char * cLabel, int iSpecies,
    int iType, short int * * p_iNumTreeVals, char * * * * p_cLabels) {
  ...
  {
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clTreePopulation::RegisterDataMember" );
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cMoreInfo, "Data label is required." );
    throw( stcErr );
  }
  ...
  for ( i = 0; i < iNumVals; i++ )
    if ( 0 == strlen( p_cLabels[iSpecies] [iType] [i] ) )
    {
      iReturnCode = i;
      break;
    }
  ...
  if ( -1 == iReturnCode )
  {
    ...
    strcpy( stcErr.cFunction, "clTreePopulation::RegisterDataMember" );
    stcErr.iErrorCode = ILLEGAL_OP;
    strcpy( stcErr.cMoreInfo, "Too many tree data member registrations" );
    throw( stcErr );
  }

  strncpy( p_cLabels[iSpecies] [iType] [iReturnCode], cLabel, MAX_TREE_LABEL_SIZE - 1 );
  //This doesn't always happen with strncpy - make it explicit
  p_cLabels[iSpecies] [iType] [iReturnCode] [MAX_TREE_LABEL_SIZE - 1] = '\0';
  return iReturnCode;
...
// RegisterInt()
//////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::RegisterInt(const char * cLabel, int iSpecies, int iType) {
  return RegisterDataMember(cLabel, iSpecies, iType, mp_iNumTreeIntVals,
      mp_cIntLabels);
}
...
// RegisterFloat()
//////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::RegisterFloat(const char * cLabel, int iSpecies, int iType) {
  return RegisterDataMember(cLabel, iSpecies, iType, mp_iNumTreeFloatVals,
      mp_cFloatLabels);
}
...
// RegisterChar()
//////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::RegisterChar(const char * cLabel, int iSpecies, int iType) {
  return RegisterDataMember(cLabel, iSpecies, iType, mp_iNumTreeCharVals,
      mp_cCharLabels);
}
...
// RegisterBool()
//////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::RegisterBool(const char * cLabel, int iSpecies, int iType) {
  return RegisterDataMember(cLabel, iSpecies, iType, mp_iNumTreeBoolVals,
      mp_cBoolLabels);
}
...
// GetDataCode()
//////////////////////////////////////////////////////////////////////////////
short int clTreePopulation::GetIntDataCode(const char * cLabel, int iSpecies,
    int iType) {
  char cSearcher[MAX_TREE_LABEL_SIZE];
  ...
  { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::GetDataIntCode" );
    sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Unrecognized species or type.  Species = ", iSpecies, " Type = ", iType );
    throw stcErr;
  }
  ...
  if ( 0 == strlen( cLabel ) )
  {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::GetDataCode" );
    strcpy( stcErr.cMoreInfo, "Data label is required." );
    throw stcErr;
  }

  //If the label is longer than the max size, truncate it like the original
  //would have been
  strncpy( cSearcher, cLabel, MAX_TREE_LABEL_SIZE - 1 );
  //This doesn't always happen with strncpy - make it explicit
  cSearcher[MAX_TREE_LABEL_SIZE - 1] = '\0';

  //Search for the label to return
  for ( i = 0; i < mp_iNumTreeIntVals[iSpecies] [iType]; i++ )
    if ( strcmp( mp_cIntLabels[iSpecies] [iType] [i], cSearcher ) == 0 )
    {
      iReturnCode = i;
      break;
    }
...
//----------------------------------------------------------------------------
short int clTreePopulation::GetFloatDataCode(const char * cLabel, int iSpecies,
    int iType) {
  char cSearcher[MAX_TREE_LABEL_SIZE];
  short int iReturnCode = -1, i;
  ...
  { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::GetFloatDataCode" );
    sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Unrecognized species or type.  Species = ", iSpecies, " Type = ", iType );
    throw stcErr;
  }
  ...
  for ( i = 0; i < mp_iNumTreeFloatVals[iSpecies] [iType]; i++ )
    if ( strcmp( mp_cFloatLabels[iSpecies] [iType] [i], cSearcher ) == 0 )
    {
      iReturnCode = i;
      break;
    }
...
//----------------------------------------------------------------------------
short int clTreePopulation::GetCharDataCode(const char * cLabel, int iSpecies,
    int iType) {
  char cSearcher[MAX_TREE_LABEL_SIZE];
  short int iReturnCode = -1, i;
  ...
  { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::GetCharDataCode" );
    sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Unrecognized species or type.  Species = ", iSpecies, " Type = ", iType );
    throw stcErr;
  }
  ...
  //Search for the label to return
  for ( i = 0; i < mp_iNumTreeCharVals[iSpecies] [iType]; i++ )
    if ( strcmp( mp_cCharLabels[iSpecies] [iType] [i], cSearcher ) == 0 )
    {
      iReturnCode = i;
      break;
    }
...
//----------------------------------------------------------------------------
short int clTreePopulation::GetBoolDataCode(const char * cLabel, int iSpecies,
    int iType) {
  char cSearcher[MAX_TREE_LABEL_SIZE];
  short int iReturnCode = -1, i;
  ...
  { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::GetBoolDataCode" );
    sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Unrecognized species or type.  Species = ", iSpecies, " Type = ", iType );
    throw stcErr;
  }
  ...
  for ( i = 0; i < mp_iNumTreeBoolVals[iSpecies] [iType]; i++ )
    if ( strcmp( mp_cBoolLabels[iSpecies] [iType] [i], cSearcher ) == 0 )
    {
      iReturnCode = i;
      break;
    }
...
// GetDataLabel
//////////////////////////////////////////////////////////////////////////////
const char* clTreePopulation::GetIntDataLabel(short int iCode, int iSpecies,
    int iType) {
  //validate the species and type codes
  if ( iSpecies < 0 || iSpecies >= m_iNumSpecies || iType < 0 || iType >= m_iNumTypes )
  { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::GetIntDataLabel" );
    sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Unrecognized species or type.  Species = ", iSpecies, " Type = ", iType );
    throw stcErr;
  }
  ...
  if ( iCode < 0 || iCode >= mp_iNumTreeIntVals[iSpecies] [iType] )
    ...
    return mp_cIntLabels[iSpecies] [iType] [iCode];
}

//----------------------------------------------------------------------------
const char* clTreePopulation::GetFloatDataLabel(short int iCode, int iSpecies,
    int iType) {
  //validate the species and type codes
  if ( iSpecies < 0 || iSpecies >= m_iNumSpecies || iType < 0 || iType >= m_iNumTypes )
  { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::GetFloatDataLabel" );
    sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Unrecognized species or type.  Species = ", iSpecies, " Type = ", iType );
    throw stcErr;
  }
  ...
  if ( iCode < 0 || iCode >= mp_iNumTreeFloatVals[iSpecies] [iType] )
    ...
    return mp_cFloatLabels[iSpecies] [iType] [iCode];
}

//----------------------------------------------------------------------------
const char* clTreePopulation::GetCharDataLabel(short int iCode, int iSpecies,
    int iType) {
  //validate the species and type codes
  if ( iSpecies < 0 || iSpecies >= m_iNumSpecies || iType < 0 || iType >= m_iNumTypes )
  { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::GetCharDataLabel" );
    sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Unrecognized species or type.  Species = ", iSpecies, " Type = ", iType );
    throw stcErr;
  }

  //If the code's invalid, return an empty string
  if ( iCode < 0 || iCode >= mp_iNumTreeCharVals[iSpecies] [iType] )
    return "";
  else
    return mp_cCharLabels[iSpecies] [iType] [iCode];
}

//----------------------------------------------------------------------------
const char* clTreePopulation::GetBoolDataLabel(short int iCode, int iSpecies,
    int iType) {
  //validate the species and type codes
  if ( iSpecies < 0 || iSpecies >= m_iNumSpecies || iType < 0 || iType >= m_iNumTypes )
  { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clTreePopulation::GetBoolDataLabel" );
    sprintf( stcErr.cMoreInfo, "%s%d%s%d", "Unrecognized species or type.  Species = ", iSpecies, " Type = ", iType );
    throw stcErr;
  }
  ...
  if ( iCode < 0 || iCode >= mp_iNumTreeBoolVals[iSpecies] [iType] )
    ...
    return mp_cBoolLabels[iSpecies] [iType] [iCode];
}
...
short int clTreePopulation::GetXCode(int iSpecies, int iType) {
  ...
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || iType < 0 || iType
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clTreePopulation::GetXCode");
    sprintf(stcErr.cMoreInfo, "%s%d%s%d",
        "Unrecognized species or type.  Species = ", iSpecies, " Type = ",
        iType);
    throw stcErr;
  }
...
short int clTreePopulation::GetYCode(int iSpecies, int iType) {
  ...
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || iType < 0 || iType
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clTreePopulation::GetYCode");
    sprintf(stcErr.cMoreInfo, "%s%d%s%d",
        "Unrecognized species or type.  Species = ", iSpecies, " Type = ",
        iType);
    throw stcErr;
...
short int clTreePopulation::GetHeightCode(int iSpecies, int iType) {
  ...
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || iType < 0 || iType
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clTreePopulation::GetHeightCode");
    sprintf(stcErr.cMoreInfo, "%s%d%s%d",
        "Unrecognized species or type.  Species = ", iSpecies, " Type = ",
        iType);
    throw stcErr;
...
short int clTreePopulation::GetDbhCode(int iSpecies, int iType) {
  ...
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || (iType != sapling && iType
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clTreePopulation::GetDbhCode");
    sprintf(stcErr.cMoreInfo, "%s%d%s%d",
        "Unrecognized species or type.  Species = ", iSpecies, " Type = ",
        iType);
    throw stcErr;
...
short int clTreePopulation::GetCrownRadiusCode(int iSpecies, int iType) {
  ...
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || (iType != sapling && iType
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clTreePopulation::GetCrownRadiusCode");
    sprintf(stcErr.cMoreInfo, "%s%d%s%d",
        "Unrecognized species or type.  Species = ", iSpecies, " Type = ",
        iType);
    throw stcErr;
...
short int clTreePopulation::GetCrownDepthCode(int iSpecies, int iType) {
  ...
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || (iType != sapling && iType
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clTreePopulation::GetCrownDepthCode");
    sprintf(stcErr.cMoreInfo, "%s%d%s%d",
        "Unrecognized species or type.  Species = ", iSpecies, " Type = ",
        iType);
    throw stcErr;
...
short int clTreePopulation::GetAgeCode(int iSpecies) {
  ...
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies) { //throw an error
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clTreePopulation::GetAgeCode");
    sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized species.  Species = ",
        iSpecies);
    throw stcErr;
  }
...
short int clTreePopulation::GetDiam10Code(int iSpecies, int iType) {
  ...
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies || (iType != sapling && iType
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clTreePopulation::GetDiam10Code");
    sprintf(stcErr.cMoreInfo, "%s%d%s%d",
        "Unrecognized species or type.  Species = ", iSpecies, " Type = ",
        iType);
    throw stcErr;
...
void clTreePopulation::DoDataUpdates() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreePopulation::DoDataUpdates" );
    throw( stcErr );
  }
}
...
void clTreePopulation::DeleteStumps() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreePopulation::DeleteStumps" );
    throw( stcErr );
  }
}
...
void clTreePopulation::DataMemberRegistrations() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreePopulation::DataMemberRegistrations" );
    throw( stcErr );
  }
}
...
void clTreePopulation::CreateTreesFromTextTreeMap(xercesc::DOMDocument *p_oDoc) {
  ...
  char* cTemp = new char[MAX_TREE_CHAR_VALUE_SIZE];
  char** p_sColHeader = NULL;
  ...
  try
  {
    ...
    sFileName[0] = '\0';
    FillSingleValue(p_oElement, "tr_treemapFile", sFileName, MAX_FILENAME_SIZE, false);
    ...
    if (!DoesFileExist(sFileName)) {
      ...
      strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTextTreeMap");
      sprintf(stcErr.cMoreInfo, "%s%s%s", "Can't find file \"", sFileName, "\".");
      throw( stcErr );
    }
    ...
    if (iNumColumns > 0) {
      p_sColHeader = new char*[iNumColumns];
      for (i = 0; i < iNumColumns; i++) p_sColHeader[i] = new char[MAX_TREE_LABEL_SIZE+1];
      ...
      for (i = 0; i < iNumColumns; i++) {
        pos2 = sTemp.find("\t", pos);
        strcpy(p_sColHeader[i], sTemp.substr(pos, pos2-pos).c_str());
        pos = pos2 + 1;
      ...
      for (i = 0; i < iNumColumns; i++) {
        for (iSpecies = 0; iSpecies < m_iNumSpecies; iSpecies++) {
          for (iType = 0; iType < m_iNumTypes; iType++) {
            ...
            if (GetFloatDataCode(p_sColHeader[i], iSpecies, iType) > -1) {
              if (notassigned == p_iMemberType[i]) {
                ...
                sprintf( stcErr.cMoreInfo, "%s%s%s", "Tree data member \"",
                    p_sColHeader[i], "\" of ambiguous type." );
                strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTextTreeMap" );
                throw( stcErr );
              }
            }
            ...
            //Is it a char?
            if (GetCharDataCode(p_sColHeader[i], iSpecies, iType) > -1) {
              if (notassigned == p_iMemberType[i]) {
        ...
        if (notassigned == p_iMemberType[i]) {
          ...
          modelErr stcErr;
          stcErr.iErrorCode = BAD_DATA;
          sprintf( stcErr.cMoreInfo, "Tree data member \"%s\" missing.",
              p_sColHeader[i]);
          strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTextTreeMap" );
          throw( stcErr );
        }
    ...
    while (!in.eof()) {
      ...
      //Get species
      strcpy(cTemp, sSp.c_str());
      iSpecies = TranslateSpeciesNameToCode(cTemp);
      if (iSpecies == -1) {
        delete[] sFileName; delete[] cTemp;
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTextTreeMap");
        sprintf(stcErr.cMoreInfo, "%s%s%s%s%s", "The file \"", sFileName,
            "\" has an invalid species: \"",
            sSp.c_str(), "\".");
        throw(stcErr);
      }
      ...
      if (fDiam <= 0) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clTreePopulation::CreateTreesFromTextTreeMap");
        sprintf(stcErr.cMoreInfo, "%s%s%s%f%s", "The file \"", sFileName,
            "\" has an invalid tree diameter: \"",
            fDiam, "\".");
        throw(stcErr);
      }
      ...
      while (i < iNumColumns) {
        if (isfloat == p_iMemberType[i]) {
          ...
          iCode = GetCharDataCode(p_sColHeader[i], p_oTree->GetSpecies(), p_oTree->GetType());
          if (iCode > -1) {
            p_oTree->SetValue(iCode, cTemp);
          }
        }
        i++;
      }
    ...
    if (iNumColumns > 0) {
      ...
      for (i = 0; i < iNumColumns; i++) delete[] p_sColHeader[i];
      delete[] p_sColHeader;
    }
