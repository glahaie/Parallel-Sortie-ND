...
clBehaviorBase::clBehaviorBase(clSimManager *p_oSimManager)
  ...
  try {
   ...
   strcpy(stcErr.cFunction, "clBehaviorBase::clBehaviorBase");
   throw(stcErr);
 }
}
...
void clBehaviorBase::SetSpeciesTypeCombos(
  ...
  try {
    ...
    if (mp_whatSpeciesTypeCombos) {
      ...
      modelErr stcErr;
      strcpy(stcErr.cFunction, "clBehaviorBase::SetSpeciesTypeCombos");
      stcErr.iErrorCode = DATA_READ_ONLY;
      strcpy(stcErr.cMoreInfo, "behavior species/type data");
      throw(stcErr);
    }
    ...
    if (0 >= m_iNumSpeciesTypeCombos) {
      ...
      //Otherwise - problem - throw error
        ...
        modelErr stcErr;
        strcpy(stcErr.cFunction, "clBehaviorBase::SetSpecies");
    ...
    for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
      if (mp_whatSpeciesTypeCombos[i].iSpecies < 0 ||
        ...
        strcpy(stcErr.cFunction, "clBehaviorBase::SetSpecies");
        stcErr.iErrorCode = BAD_DATA;
        sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized species number:  ",
      ...
      if (mp_whatSpeciesTypeCombos[i].iType < 0 ||
        ...
        modelErr stcErr;
        strcpy(stcErr.cFunction, "clBehaviorBase::SetTypes");
        stcErr.iErrorCode = BAD_DATA;
        sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized type:  ",
...
struct stcSpeciesTypeCombo clBehaviorBase::GetSpeciesTypeCombo(short int iIndex) {
  ...
  if (iIndex < 0 || iIndex >= m_iNumSpeciesTypeCombos) {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clBehaviorBase::GetSpeciesTypeCombo");
    stcErr.iErrorCode = BAD_DATA;
    sprintf(stcErr.cMoreInfo, "%s%d%s", "Index \"", iIndex,
...
short int clBehaviorBase::GetBehaviorSpecies(short int iIndex) {
  ...
  if (iIndex < 0 || iIndex >= m_iNumBehaviorSpecies) {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clBehaviorBase::GetBehaviorSpecies");
    stcErr.iErrorCode = BAD_DATA;
    sprintf(stcErr.cMoreInfo, "%s%d%s", "Index \"", iIndex,
        "\" is not a valid species index.");
    throw(stcErr);
  }
...
DOMElement* clBehaviorBase::GetParentParametersElement(xercesc::DOMDocument *p_oDoc) {
  ...
  if (0 == p_oNodeList->getLength()) {
    ...
    strcpy(stcErr.cFunction, "clBehaviorBase::GetParentParametersElement");
    sprintf(stcErr.cMoreInfo, "%s%d", m_sXMLRoot.c_str(), m_iBehaviorListNumber);
    throw (&stcErr);
  }
