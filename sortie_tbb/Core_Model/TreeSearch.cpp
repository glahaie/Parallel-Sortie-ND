...
clTreeSearch::clTreeSearch(clTreePopulation *p_oTreePop, clPlot *p_oPlot) {
  try {
    ...
    strcpy(stcErr.cFunction, "clTreeSearch::clTreeSearch");
    throw(stcErr);
  }
}
...
void clTreeSearch::Setup() {
  try {
    ...
    || (!m_bAllUsed && !m_bDistanceHeightUsed && !m_bSpeciesUsed
      ...
      strcpy(stcErr.cFunction, "clTreeSearch::Setup");
      strcpy(stcErr.cMoreInfo, "Invalid search request.");
      throw(stcErr);
    }
    ...
    if (m_bDistanceHeightUsed) {
      ...
      if (m_fFromX < 0 || m_fFromX >= mp_oTreePop->m_fPlotLengthX ||
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clTreeSearch::Setup");
        strcpy(stcErr.cMoreInfo, "Either X or Y search point is outside plot.");
        throw(stcErr);
      }
    ...
    if (m_bTypeUsed) {
      ...
      if (m_iWhatTypes >= pow(2, mp_oTreePop->m_iNumTypes)) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clTreeSearch::Setup");
        strcpy(stcErr.cMoreInfo, "Invalid types in tree search.");
        throw(stcErr);
      }
...
void clTreeSearch::StartOver() {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clTreeSearch::StartOver");
    throw(stcErr);
  }
}
