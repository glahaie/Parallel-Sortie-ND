...
int main( int argc, char * argv[] )
{
  ...
  int iNumTimesteps, //for figuring out the number of timesteps
       iLastSeparator, iStrLen, i;

  //Capture the application path
  char cPath[MAX_FILENAME_SIZE];
  //Because strncpy doesn't always place on the terminating \0, make sure it
  //happens
  for (i = 0; i < MAX_FILENAME_SIZE; i++) {
    cPath[i] = '\0';
  }
  iLastSeparator = 0;
  iStrLen = strlen( argv[0] );
  if ( iStrLen > MAX_FILENAME_SIZE )
  {
  ...
  //Only copy the file path if we actually have a path
  if (iLastSeparator != 0)
    strncpy( cPath, argv[0], iLastSeparator + 1 );

  //***************************************
  //  Version numbers
  //***************************************
  int iMajorVersion = 7, iMinorVersion = 1;
  clSimManager p_oSimManager( iMajorVersion, iMinorVersion, cPath );
  ...
  //***************************************
  //-----------------------------------------------------------------
  //For testing purposes - initialize a number of threads
  char* processor = getenv("PROCESSOR");
  int n;
  if (processor != NULL) {
    n = std::stoi(processor);
  } else {
      n = tbb::task_scheduler_init::deferred;
  }
  if (n == 0) n = tbb::task_scheduler_init::deferred;
  if (n > 4) n = tbb::task_scheduler_init::automatic;
    ...
    try
    {
      ...
      if ( INPUT_FILE == stcMsg.iMessageCode )
      {
        ...
        if ( "" == strArg1 )
        {
          ...
          strcpy( stcError.cFunction, "main" );
        ...
        else
        {
          ...
          else
          {
            ...
            if ( 0 == iNumTimesteps && strArg1.length() > 1 )
            {
              ...
              strcpy( stcMsg.cMoreInfo, "Couldn't read number of timesteps" );
              SendMessage( stcMsg );
            }
            else
            {
