 ...
 * for ints, floats, char strings, and bools.  All stcRecords for a grid have
 ...
 * arrays (int, float, char string, bool). All package objects for a grid are
...
class clGrid : public clWorkerBase{
  ...
  friend class clGridTest;   /**<For testing purposes*/
...
protected:
   ...
   * @param cGridName An identifying name string for the grid object.
   ...
   */
  clGrid(clSimManager *p_oSimManager, const char *cGridName,
...
/**
   ...
   * @param cLabel A string with the name of this variable - keep it short.
   * @param iNumVals Number total allowed variables of this type
   * @param p_cLabelList Pointer to the correct labels array
   ...
   * will be thrown.
   */
  inline short int RegisterDataMember(const char*cLabel, short int iNumVals,
    char **p_cLabelList);
...
public:
   ...
   * @param iNumCharVals Number of package char data members.
  ...
  void ChangePackageDataStructure(short int iNumIntVals, short int iNumFloatVals,
      short int iNumCharVals, short int iNumBoolVals);
  ...
  /**
   * Retrieves the value of a char grid data member at an (X, Y) location. This
   * function determines which internal grid cell this location is and places
   * the grid value into the variable passed.
   * @param fX X coordinate of point for which the value is desired.
   * @param fY Y coordinate of point for which the value is desired.
   * @param iCode Code for the value desired. This is what is returned from
   * registering a variable or from using GetDataCode.
   * @param p_cValHolder Pointer to where the return value should be placed.
   * @throw BAD_DATA error if the point is not within the grid or the code is
   * not valid.
   */
  ...
  /**
   * Retrieves the value of a char data member for a particular grid cell. The
   * grid cell numbering in each direction starts at 0 and goes to number of
   * grid cells minus one.
   * @param iX Grid number in the X direction for cell for which the value is
   * desired.
  ...
  * @return Number of char data members.
  */
  int GetNumberCharDataMembers() {return m_iNumCharVals;};
  ...
  * @return Number of package char data members.
  */
  int GetNumberCharPackageDataMembers() {return m_iNumPackageCharVals;};
  ...
  */
  ...
  /**
   * Sets the value of a char data member for a particular grid cell. The grid
   * cell numbering in each direction starts at 0 and goes to number of grid
   * cells minus one.
   * @param iX Grid number in the X direction for cell for which the value is
   * desired.
   ...
   * @param cLabel String with the name of this variable.  Keep it short.
   * @return A code that will be used to access values of that type in the
   * future. This is faster than trying to do lookups by character string.
   * @throw ILLEGAL_OP error if there is no more space for new data members.
   */
  short int RegisterInt(const char*cLabel);
   ...
   * @throw ILLEGAL_OP error if there is no more space for new data members.
   */
  short int RegisterFloat(const char*cLabel);

  /**
   * Registers a char data member. This will not check to make sure
   * that there is not already a variable registered with that name.
   * @param cLabel String with the name of this variable.  Keep it short.
   * @return A code that will be used to access values of that type in the
   * future.  This is faster than trying to do lookups by character string.
   * @throw ILLEGAL_OP error if there is no more space for new data members.
   */
  short int RegisterChar(const char*cLabel);
   ...
   * @throw ILLEGAL_OP error if there is no more space for new data members.
   */
  short int RegisterBool(const char*cLabel);
   ...
   * @throw ILLEGAL_OP error if there is no more space for new data members.
   */
  short int RegisterPackageInt(const char*cLabel);
   ...
   * @throw ILLEGAL_OP error if there is no more space for new data members.
   */
  short int RegisterPackageFloat(const char*cLabel);

  /**
   * Registers a package char data member.  This function must only be used if
   * you need to set up packages with a different data structure than the main
   * grid.  If the data structures are the same, then packages automatically get
   * the same data member registrations as the main grid.
   * @param cLabel String with the name of this variable.  Keep it short.
   ...
   * @throw ILLEGAL_OP error if there is no more space for new data members.
   */
  short int RegisterPackageChar(const char*cLabel);
   ...
   * @throw ILLEGAL_OP error if there is no more space for new data members.
   */
  short int RegisterPackageBool(const char*cLabel);
...
//wanted the choice to be explicit.
   ...
   * @param cLabel The data member's label.
   * @return The code for the value, -1 if the label is unrecognized.
   */
  short int GetIntDataCode(const char *cLabel);
   ...
   * @return The code for the value, -1 if the label is unrecognized.
   */
  short int GetFloatDataCode(const char *cLabel);

  /**
   * Gets the code for a char data member.  This function will not be
   * responsible for duplicate labels.
   * @param cLabel The data member's label.
   * @return The code for the value, -1 if the label is unrecognized.
   */
  short int GetCharDataCode(const char *cLabel);
   ...
   * @return The code for the value, -1 if the label is unrecognized.
   */
  short int GetBoolDataCode(const char *cLabel);
   ...
   * @return The code for the value, -1 if the label is unrecognized.
   */
  short int GetPackageIntDataCode(const char *cLabel);
   ...
   * @return The code for the value, -1 if the label is unrecognized.
   */
  short int GetPackageFloatDataCode(const char *cLabel);

  /**
   * Gets the code for a package char data member.  This function will not be
   * responsible for duplicate labels.
   * @param cLabel The data member's label.
   * @return The code for the value, -1 if the label is unrecognized.
   */
  short int GetPackageCharDataCode(const char *cLabel);
   ...
   * @return The code for the value, -1 if the label is unrecognized.
   */
  short int GetPackageBoolDataCode(const char *cLabel);
   ...
   * unrecognized.
   */
  const char* GetIntDataLabel(short int iCode);
   ...
   * unrecognized.
   */
  const char* GetFloatDataLabel(short int iCode);

  /**
   * Gets the label for a char data member.  This function will not be
   * responsible for duplicate labels.
   * @param iCode The data member's code.
   * @return The code's data label, or an empty string if the code is
   * unrecognized.
   */
  const char* GetCharDataLabel(short int iCode);
   ...
   * unrecognized.
   */
  const char* GetBoolDataLabel(short int iCode);
   ...
   * unrecognized.
   */
  const char* GetPackageIntDataLabel(short int iCode);
   ...
   * unrecognized.
   */
  const char* GetPackageFloatDataLabel(short int iCode);

  /**
   * Gets the label for a package char data member.  This function will not be
   * responsible for duplicate labels.
   * @param iCode The data member's code.
   * @return The code's data label, or an empty string if the code is
   * unrecognized.
   */
  const char* GetPackageCharDataLabel(short int iCode);
   ...
   * unrecognized.
   */
  const char* GetPackageBoolDataLabel(short int iCode);
...
//These next are structured the way they are so they can be overloaded.
  ...
  */
   ...
   * @param iCode Data member code.
   * @param p_cValHolder Address of variable into which to place the requested
   * data member value.
   */
