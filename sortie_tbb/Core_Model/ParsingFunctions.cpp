...
void FillSpeciesSpecificValue( xercesc::DOMElement *p_oParent, const char *cTagName,
  ...
  try {
    ...
    sTag = XMLString::transcode(cTagName);
    ...
    if (0 == iNumNodes) {
      if (!bRequired) return; //if it wasn't required exit without error
        ...
        modelErr stcErr;
        strcpy(stcErr.cFunction, "FillSpeciesSpecificValue");
        stcErr.iErrorCode = DATA_MISSING;
        strcpy(stcErr.cMoreInfo, cTagName);
        throw(stcErr);
      }
    }
    ...
    sTag = XMLString::transcode(cSubTagName);
    ...
    if (0 == iNumNodes) {
      if (!bRequired) return; //if it wasn't required exit without error
        ...
        strcpy(stcErr.cFunction, "FillSpeciesSpecificValue");
        stcErr.iErrorCode = DATA_MISSING;
        strcpy(stcErr.cMoreInfo, cSubTagName);
        throw(stcErr);
      }
    }
    ...
    for (i = 0; i < iNumSpecies; i++)
      if (-1 == p_iSpeciesRead[i]) {
        ...
        strcpy(stcErr.cFunction, "FillSpeciesSpecificValue");
        stcErr.iErrorCode = DATA_MISSING;
        sprintf(stcErr.cMoreInfo,"%s%s",cTagName, " for all species indicated");
        throw(stcErr);
      }
    delete[] p_iSpeciesRead;
  } //end of try block
          ...
          for (j = 0; j < iNumSpecies; j++) {
            if (iCode == p_array[j].code) {
              if (strcmp(cData, "1") == 0)
                ...
                delete[] p_iSpeciesRead;
                modelErr stcErr;
                strcpy(stcErr.cFunction, "FillSpeciesSpecificValue(bool)");
                stcErr.iErrorCode = BAD_DATA;
                sprintf(stcErr.cMoreInfo, "%s%s", "Unexpected value in ", cTagName);
                throw(stcErr);
              }
              p_iSpeciesRead[k] = iCode;
              k++;
...
/////////////////////////////////////////////////////////////////////////////*/
void FillSpeciesSpecificValue(xercesc::DOMElement *p_oParent, const char *cTagName,
       const char *cSubTagName, float *p_array, clTreePopulation *p_oPop,
       bool bRequired) {
  try {
    DOMNodeList *p_oNodeList;  //node list object - for catching all nodes with
                               //a particular tag name
...
/////////////////////////////////////////////////////////////////////////////*/
void FillSpeciesSpecificValue(xercesc::DOMElement *p_oParent, const char *cTagName,
       const char *cSubTagName, int *p_array, clTreePopulation *p_oPop,
       bool bRequired) {
  try {
    DOMNodeList *p_oNodeList;  //node list object - for catching all nodes with
                               //a particular tag name
...
short int GetNodeSpeciesCode(xercesc::DOMNode *p_oDocNode, clTreePopulation *p_oPop) {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "GetNodeSpeciesCode");
    throw (stcErr);
  }
  return -1;
}
...
void FillSingleValue(xercesc::DOMElement *p_oParent,
                   const char *cTagName, int *p_iValToFill, bool bRequired) {
  try {
    ...
    if (NULL == p_oParent || NULL == p_iValToFill) {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "FillSingleValue(int)");
      strcpy(stcErr.cMoreInfo, "Null pointer was passed to the function.");
      stcErr.iErrorCode = CANT_FIND_OBJECT;
      throw(stcErr);
    } else if (strlen(cTagName) == 0) {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "FillSingleValue(int)");
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cMoreInfo, "Tag name required.");
      throw(stcErr);
    }
    ...
    //string - if it's longer than 1, the text != 0 and it's not a valid value
      ...
      if (strlen(cData) > 1) {
        ...
        strcpy(stcErr.cFunction, "FillSingleValue(int)");
        stcErr.iErrorCode = BAD_DATA;
        sprintf(stcErr.cMoreInfo, "%s%s", cTagName, " is not an integer");
        throw(stcErr);
      }
      else {
        *p_iValToFill = iVal;
...
//////////////////////////////////////////////////////////////////////////////
void FillSingleValue(xercesc::DOMElement *p_oParent,
                const char *cTagName, float *p_fValToFill, bool bRequired) {
  try {
    ...
    if (NULL == p_oParent || NULL == p_fValToFill) {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "FillSingleValue(float)");
      strcpy(stcErr.cMoreInfo, "Null pointer was passed to the function.");
      stcErr.iErrorCode = CANT_FIND_OBJECT;
      throw(stcErr);
    } else if (strlen(cTagName) == 0) {
    ...
    //string - if it's longer than 1, the text != 0 and it's not a valid value
      ...
      if (cData[0] != '0') {
        ...
        strcpy(stcErr.cFunction, "FillSingleValue(float)");
        stcErr.iErrorCode = BAD_DATA;
        sprintf(stcErr.cMoreInfo, "%s%s", cTagName, " is not a float");
        throw(stcErr);
      }
      else {
        *p_fValToFill = fVal;
...
//////////////////////////////////////////////////////////////////////////////
// FillSingleValue() - char version
/////////////////////////////////////////////////////////////////////////////*/
void FillSingleValue(xercesc::DOMElement *p_oParent,
          const char *cTagName, char *p_cValToFill, int iMaxSize, bool bRequired) {
  try {
    xercesc::DOMNodeList *p_oNodeList; //for searching the document
    XMLCh *sVal;
    char *cData;  //for returning the text data
    ...
    //Error trap - make sure the pointers aren't NULL and that the tag name
    //isn't empty
    if (NULL == p_oParent || NULL == p_cValToFill) {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "FillSingleValue(string)");
      strcpy(stcErr.cMoreInfo, "Null pointer was passed to the function.");
      stcErr.iErrorCode = CANT_FIND_OBJECT;
      throw(stcErr);
    } else if (strlen(cTagName) == 0) {
    ...
    //Query the document for elements with our given tag name
    sVal = XMLString::transcode(cTagName);
    ...
    else strncpy(p_cValToFill, cData, iMaxSize);
    p_cValToFill[iMaxSize - 1] = '\0'; //sometimes this isn't done w/ strncpy
    delete[] cData;
    return;
  } //end of try block
  catch (modelErr&err) {throw(err);}
...
//////////////////////////////////////////////////////////////////////////////
void FillSingleValue(xercesc::DOMElement *p_oParent,
          const char *cTagName, bool *p_bValToFill, bool bRequired) {
  try {
    DOMNodeList *p_oNodeList; //for searching the document
    XMLCh *sVal;
    char *cData;  //for returning the text data
    ...
    if (NULL == p_oParent || NULL == p_bValToFill) {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "FillSingleValue(bool)");
      strcpy(stcErr.cMoreInfo, "Null pointer was passed to the function.");
      stcErr.iErrorCode = CANT_FIND_OBJECT;
      throw(stcErr);
    } else if (strlen(cTagName) == 0) {
...
/*void WriteXMLNode(DOMNode *p_oNode, char *cFile) {
  ...
  //Create a hook to the file for the DOMWriter to write to
  LocalFileFormatTarget *p_oOut = new LocalFileFormatTarget(cFile);

  //Get a handle to the DOMImplementation so we can create the DOMWriter
  DOMImplementation *p_oImpl=DOMImplementationRegistry::getDOMImplementation(
                   XMLString::transcode("LS"));

  //Get a handle to a new DOMWriter object
  DOMWriter* p_oXMLWriter = ((DOMImplementationLS*)p_oImpl)->createDOMWriter();

  //Write the file
  p_oXMLWriter->writeNode(p_oOut, *p_oNode);

  //Clean up
  p_oXMLWriter->release();
  delete p_oOut;
} */
//---------------------------------------------------------------------------
