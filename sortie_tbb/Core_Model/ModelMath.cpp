...
float clModelMath::CalculateBasalArea(float fDbh) {
  ...
  { //throw error
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clModelMath::CalculateBasalArea" );
    strcpy( stcErr.cMoreInfo, "DBH is not a positive number." );
...
float clModelMath::CalculateLognormalFunction(float fX0, float fXb,
  ...
  if ( 0 == fXb )
  {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clModelMath::CalculateLognormalFunction" );
    strcpy( stcErr.cMoreInfo, "Xb cannot be zero." );
    throw( stcErr );
  }
  if (0 >= fX0 || 0 > fDistance) {
    ...
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clModelMath::CalculateLognormalFunction" );
    strcpy( stcErr.cMoreInfo, "X0 and distance must be greater than zero." );
    throw( stcErr );
  }
...
float clModelMath::AddBarkToDBH(float fDIB, float fA, float fB, float fC) {
  if (fDIB <= 0) {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clModelMath::AddBarkToDBH");
    strcpy(stcErr.cMoreInfo, "DBH is not a positive number.");
    stcErr.iErrorCode = BAD_DATA;
    throw( stcErr );
  }
  if (fDIB >= 1E18) {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clModelMath::AddBarkToDBH");
    strcpy(stcErr.cMoreInfo, "DBH is too large.");
    stcErr.iErrorCode = BAD_DATA;
    throw( stcErr );
  }
