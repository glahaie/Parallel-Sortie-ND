...
* @param cTagName Tag name of the parent node (parent_tag).
* @param cSubTagName Tag name of the child node (child_tag).
...
*/
 void FillSpeciesSpecificValue(xercesc::DOMElement *p_oParent, const char *cTagName,
       const char *cSubTagName, floatVal *p_array, int iNumSpecies,
...
*/
 void FillSpeciesSpecificValue(xercesc::DOMElement *p_oParent, const char *cTagName,
       const char *cSubTagName, doubleVal *p_array, int iNumSpecies,
       clTreePopulation *p_oPop, bool bRequired);
...
@endhtmlonly
 <p>
 This function will extract the values and place them in a given array, matching
 species codes.  If the data isn't present in the document, the action taken
 depends on the value of the flag bRequired.  If bRequired is false, and the
 data isn't there, the function simply exits.  If bRequired is true, the
 ...
 @param p_oParent Pointer to the parent element of the element to extract.  If this
        is a top-level element, the parent will be the XML document root element
        (at the time of this writing, named paramFile).
 @param cTagName Tag name of the parent node (parent_tag).
 @param cSubTagName Tag name of the child node (child_tag).
 @param p_array Pointer to the array to put the extracted values in.
 @param iNumSpecies Number of species expected (should equal the size of the
 intVal array)
 @param p_oPop Pointer to the tree population.  This is necessary for translating
        species names into codes
 @param bRequired Whether or not this value is required.
 @throw Error if the value is required and it is not found, or if it is found
 and not all species are present (whether or not it is required).
*/
 void FillSpeciesSpecificValue(xercesc::DOMElement *p_oParent, const char *cTagName,
       const char *cSubTagName, intVal *p_array, int iNumSpecies,
       clTreePopulation *p_oPop, bool bRequired);
...
*/
 void FillSpeciesSpecificValue(xercesc::DOMElement *p_oParent, const char *cTagName,
       const char *cSubTagName, boolVal *p_array, int iNumSpecies,
       clTreePopulation *p_oPop, bool bRequired);
...
*/
 void FillSpeciesSpecificValue(xercesc::DOMElement *p_oParent, const char *cTagName,
       const char *cSubTagName, float *p_fArray, clTreePopulation *p_oPop,
       bool bRequired);
...
*/
void FillSpeciesSpecificValue(xercesc::DOMElement *p_oParent, const char *cTagName,
     const char *cSubTagName, int *p_iArray, clTreePopulation *p_oPop,
     bool bRequired);

/**
 GetNodeSpeciesCode
 Translates a species string into a species number.  For those nodes that have
 an attribute named "species", this will extract the name of the species in that
 attribute and turn it into a code.
...
/**
  ...
  @param cTagName The tag name of the node.
...
*/
 void FillSingleValue(xercesc::DOMElement *p_oParent, const char *cTagName,
...
/**
  ...
  This is currently not protected against overflow.  I could not
  successfully trap for the error codes the documentation says are supposed to
  be set.
...
*/
  ...
  * element (at the time of this writing, named paramFile).
  * @param cTagName The tag name of the node.
  * @param p_cValToFill Pointer to the variable to put the extracted value in.
  * @param iMaxSize (char only) Max number of chars to be copied over into char
  * variable
  * @param bRequired Whether or not this value is required.
  * @throw Error if the value is required and it is not found.
  */
 void FillSingleValue(xercesc::DOMElement *p_oParent, const char *cTagName,
                            char *p_cValToFill, int iMaxSize, bool bRequired);
...
/**
 Writes the contents of a node to file.

 DON'T USE THIS FUNCTION!  I have been experiencing strange memory problems.
*/
// void WriteXMLNode(DOMNode *p_oNode, char* cFile);

/**
  Smart class that will swap streambufs and replace them when object goes out
 of scope. I stole this from somewhere but I don't remember where.
*/
/* class clStreamBufSwapper
 {
   public:


     //  Constructor.  Swaps two stream buffers so that the streams write to each other's
     //  buffers.
     //  @param orig Stream one.
     //  @param replacement Stream two.
     ...
     ~clStreamBufSwapper() { str_.rdbuf(buf_); }
   private:
     std::streambuf * buf_; //<One buffer.
     std::ostream & str_; //<The other buffer.
 } ; */
