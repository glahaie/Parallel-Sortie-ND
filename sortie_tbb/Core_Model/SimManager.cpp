...
// Constructor()
//////////////////////////////////////////////////////////////////////////////
clSimManager::clSimManager( int iMajorVersion, int iMinorVersion, const char *cAppPath)
{
  try
  {
    m_iMajorVersion = iMajorVersion;
    m_iMinorVersion = iMinorVersion;
    strcpy(m_cAppPath, cAppPath);
    ...
    mp_oXMLParser = ( ( DOMImplementationLS * ) p_oImpl )->createDOMBuilder( DOMImplementationLS::MODE_SYNCHRONOUS, 0 );
    mp_oXMLErrorHandler = new clXercesErrorHandler;
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clSimManager::clSimManager" );
    GoToNoDataState();
    throw( stcErr );
  }
}
...
clSimManager::~clSimManager()
{
  ...
  mp_oXMLParser->release();
  delete mp_oXMLErrorHandler; mp_oXMLErrorHandler = NULL;
...
void clSimManager::GoToNoDataState()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSimManager::GoToNoDataState" );
    GoToNoDataState();
    throw( stcErr );
  }
}
...
// ReadFile()
/////////////////////////////////////////////////////////////////////////////
void clSimManager::ReadFile(const char * cFileName )
{
  try
  {
    DOMDocument * p_oDoc;
    ...
    //Check the file type
    iFileType = GetFileType( cFileName );

    //Parameter file - load it
    if ( parfile == iFileType )
    {
      p_oDoc = mp_oXMLParser->parseURI( cFileName );
      strncpy( m_cParFilename, cFileName, MAX_FILENAME_SIZE - 1 );
      //This doesn't always happen with strncpy - make it explicit
      m_cParFilename[MAX_FILENAME_SIZE - 1] = '\0';
    ...
    else if ( tree == iFileType )
    {
      ...
      if ( Initialized != m_eSimState )
      {
        ...
        strcpy( stcErr.cFunction, "clSimManager::ReadFile" );
        strcpy( stcErr.cMoreInfo, "Parameter file must be loaded" );
        throw( stcErr );
      }
      else
      {
    ...
    else
    {
      ...
      strcpy( stcErr.cFunction, "clSimManager::ReadFile" );
      strcpy( stcErr.cMoreInfo, cFileName );
      throw( stcErr );
    }
...
// GetFileType()
/////////////////////////////////////////////////////////////////////////////
fileType clSimManager::GetFileType( const char * cFileName )
{
  try
  {
    using namespace std;
    ...
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clSimManager::GetFileType" );

    //Does this file exist and can it be opened?
    filetest = fopen( cFileName, "r" );
    if ( filetest == NULL )
    { //file does not exist
...
unsigned long clSimManager::RunSim( int iNumStepsToRun )
{
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clSimManager::RunSim" );
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cMoreInfo, "Number of timesteps to run must be greater than zero." );
      throw( stcErr );
    }
    ...
    {
      p_oDoc = mp_oXMLParser->parseURI( m_cParFilename );
    ...
    if ( m_eSimState != Initialized && m_eSimState != Paused )
    {
      ...
      strcpy( stcErr.cFunction, "clSimManager::RunSim" );
      stcErr.iErrorCode = MODEL_NOT_READY;
      strcpy( stcErr.cMoreInfo, "Valid parameter file not loaded" );
      throw( stcErr );
    }
    ...
    for ( iTimestep = iStartTimestep; iTimestep < iEndTimestep; iTimestep++ )
    {
      ...
      stcMsg = CheckForMessage(m_cAppPath);
      if ( NO_MESSAGE != stcMsg.iMessageCode )
      {
        if ( MODEL_PAUSED == stcMsg.iMessageCode )
        {
          ...
          sprintf(stcMsg.cMoreInfo, "%s%d", "Finished timestep ", m_iCurrentTimestep);
          SendMessage( stcMsg );
          endRun = clock();
...
void clSimManager::TimestepCleanup()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSimManager::TimestepCleanup" );
    GoToNoDataState();
    throw( stcErr );
  }
}
...
void clSimManager::EndOfRunCleanup()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSimManager::EndOfRunCleanup" );
    GoToNoDataState();
    throw( stcErr );
  }
}
...
// GetPopulationObject()
/////////////////////////////////////////////////////////////////////////////
clPopulationBase * clSimManager::GetPopulationObject( const char * cPopName )
{
  try
  {
    clWorkerBase * p_oTempObject = NULL;
    ...
    p_oTempObject = mp_oPopulationManager->PassObjectPointer( cPopName );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSimManager::GetPopulationObject" );
    GoToNoDataState();
    throw( stcErr );
  }
  return NULL;
...
clGrid * clSimManager::GetGridObject( const char * cGridName )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSimManager::GetGridObject" );
    GoToNoDataState();
    throw( stcErr );
  }
  return NULL;
...
// GetBehaviorObject()
/////////////////////////////////////////////////////////////////////////////
clBehaviorBase * clSimManager::GetBehaviorObject( const char * cBehaviorName )
{
  try
  {
    clWorkerBase * p_oTempObject = NULL;
    ...
    p_oTempObject = mp_oBehaviorManager->PassObjectPointer( cBehaviorName );
    //Recast the pointer as a population pointer
    if ( p_oTempObject )
      p_oBehavior = dynamic_cast < clBehaviorBase * > ( p_oTempObject );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSimManager::GetBehaviorObject" );
    GoToNoDataState();
    throw( stcErr );
  }
  return NULL;
...
void clSimManager::ReadParameterFile( DOMDocument * p_oDoc, fileType iFileType )
{
  try
  {
    ...
    { //nope - throw error
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clSimManager::ReadParameterFile" );
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cMoreInfo, "Document pointer is NULL." );
      throw( stcErr );
    }
...
void clSimManager::ReadTreeFile( DOMDocument * p_oDoc, fileType iFileType )
{
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clSimManager::ReadTreeFile" );
      stcErr.iErrorCode = MODEL_NOT_READY;
      throw( stcErr );
    }
...
void clSimManager::ReadMapFile( DOMDocument * p_oDoc, fileType iFileType )
{
  try
  {
    ...
    if ( No_Data == m_eSimState )
    {
      modelErr stcErr;
      stcErr.iErrorCode = MODEL_NOT_READY;
      strcpy( stcErr.cFunction, "clSimManager::ReadMapFile" );
      throw( stcErr );
    }
...
void clSimManager::DoSetup( DOMDocument * p_oDoc )
{
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clSimManager::DoSetup" );
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cMoreInfo, "Timesteps must be a positive number." );
      throw( stcErr );
    }
    ...
    if ( 0 >= m_fNumYearsPerTimestep )
    {
      ...
      strcpy( stcErr.cFunction, "clSimManager::DoSetup" );
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cMoreInfo, "Number of years per timestep must be a positive number." );
      throw( stcErr );
    }
    ...
    if ( iTemp < 0 || iTemp >= m_iNumTimesteps )
    {
      ...
      strcpy( stcErr.cFunction, "clSimManager::DoSetup" );
      stcErr.iErrorCode = BAD_DATA;
      sprintf( stcErr.cMoreInfo, "%s%d", "Current timestep is not within run - value is ", iTemp );
      throw( stcErr );
    }
...
// RunBatch()
/////////////////////////////////////////////////////////////////////////////
void clSimManager::RunBatch( const char * cBatchFile )
{
  using namespace std;
  ...
  * p_oNodeList;
  ...
  struct stcParFiles
  {
    char cParFileName[MAX_FILENAME_SIZE];
    short int iNumRuns;
  }
  * p_parFileList;
  ...
  if (NULL == cBatchFile && 0 == strlen(m_cBatchFilename))
    return;
  if (NULL != cBatchFile) {
    strncpy( m_cBatchFilename, cBatchFile, MAX_FILENAME_SIZE - 1 );
  }
  ...
  if ( strlen( m_cBatchFilename ) == 0 )
  {
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clSimManager::RunBatch" );
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cMoreInfo, "Need batch file path." );
    throw( stcErr );
  }

  //Make sure the file is a batch file - if not throw error
  if ( batchfile != GetFileType( m_cBatchFilename ) )
  {
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clSimManager::RunBatch" );
    stcErr.iErrorCode = BAD_FILE_TYPE;
  ...
  //***********************************
  //Parse the document
  p_oBatchFileDoc = mp_oXMLParser->parseURI( m_cBatchFilename );
  ...
  for ( i = 0; i < iNumBatchGroups; i++ )
  {
    ...
    { //no par filename - throw error
      ...
      sprintf( stcErr.cMoreInfo, "%s%d", "ba_parFile for batch group ", i );
      throw( stcErr );
    } //end of if (0 == iNumNodes)


    //Assign the par filename into its bucket
    strncpy( p_parFileList[i].cParFileName, XMLString::transcode( p_oNodeList->item( 0 )->getFirstChild()->getNodeValue() ), MAX_FILENAME_SIZE );


    //Check to see if we can open the par file
    file.open( p_parFileList[i].cParFileName );
    if ( file.bad() )
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clSimManager::RunBatch" );
      stcErr.iErrorCode = BAD_FILE;
      sprintf( stcErr.cMoreInfo, "%s%s", "Couldn't open parameter file ", p_parFileList[i].cParFileName );
      throw( stcErr );
    } //end of if (file.bad())
    file.close();
    ...
    if ( 0 == p_oNodeList->getLength() )
    {
      ...
      strcpy( stcErr.cFunction, "clSimManager::RunBatch" );
      stcErr.iErrorCode = DATA_MISSING;
      sprintf( stcErr.cMoreInfo, "%s%s", "ba_numTimesToRun for parameter file ", p_parFileList[i].cParFileName );
      throw( stcErr );
    ...
    if ( 0 == p_parFileList[i].iNumRuns && strcmp( cNumRunsText, "0" ) != 0 )
    {
      ...
      strcpy( stcErr.cFunction, "clSimManager::RunBatch" );
      stcErr.iErrorCode = BAD_DATA;
      sprintf( stcErr.cMoreInfo, "%s%s", "Number of runs is not a number:  ", cNumRunsText );
      throw( stcErr );
    }
    else if ( 0 > p_parFileList[i].iNumRuns )
    {
      ...
      strcpy( stcErr.cFunction, "clSimManager::RunBatch" );
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cMoreInfo, "Number of runs cannot be less than zero." );
      throw( stcErr );
    }
  ...
  for ( i = iStart; i < iNumBatchGroups; i++ )
  {
    try
    {
      ...
      strncpy( m_cParFilename, p_parFileList[i].cParFileName, MAX_FILENAME_SIZE - 1 );
      //This doesn't always happen with strncpy - make it explicit
      m_cParFilename[MAX_FILENAME_SIZE - 1] = '\0';

      iFileType = GetFileType( m_cParFilename );

      p_oParFileDoc = mp_oXMLParser->parseURI( p_parFileList[i].cParFileName );
      ...
      cout << "\nBeginning runs for file " << p_parFileList[i].cParFileName << ".\n" << flush;
    ...
    {
      if (strstr(err.cMoreInfo, "Random seed") == NULL) {
        sprintf(err.cMoreInfo, "%s%s%ld", err.cMoreInfo,
            "Random seed: ", m_iActualSeed);
      }
      GoToNoDataState();
...
// CreateGrid()
/////////////////////////////////////////////////////////////////////////////
clGrid * clSimManager::CreateGrid( const char * cGridName, short int iNumIntVals, short int iNumFloatVals,
    short int iNumCharVals, short int iNumBoolVals, float fXCellLength, float fYCellLength )
{
  try
  {
    ...
    return mp_oGridManager->CreateGrid( cGridName, iNumIntVals, iNumFloatVals, iNumCharVals, iNumBoolVals,
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSimManager::CreateGrid" );
    GoToNoDataState();
    throw( stcErr );
  }
  return NULL;
