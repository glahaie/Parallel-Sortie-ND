...
void clAllometry::GetData(DOMDocument * p_oDoc, clTreePopulation * p_oPop) {
  ...
  for (int i = 0; i < m_iNumSpecies; i++) {
    if (mp_fDbhToDiam10Slope[i] == 0) {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "clAllometry::GetData");
...
float clAllometry::CalcAdultHeight(const float & fDbh, const int & iSpecies) {
  if (iSpecies >= m_iNumSpecies || iSpecies < 0) {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clAllometry::CalcAdultHeight");
...
float clAllometry::CalcAdultDbh(float fHeight, const int & iSpecies) {
  if (iSpecies >= m_iNumSpecies || iSpecies < 0) {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clAllometry::CalcAdultDbh");
    sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized species ", iSpecies);
    throw(stcErr);
  }
  ...
  if (fHeight >= mp_fMaxTreeHeight[iSpecies]) {
    ...
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clAllometry::CalcAdultDbh");
    sprintf(stcErr.cMoreInfo, "%s%s", "Cannot set tree height greater than ",
        "maximum canopy height for this species.");
    throw(stcErr);
  }
...
float clAllometry::CalcSaplingHeight(const float & fDbh, const int & iSpecies) {
  if (iSpecies >= m_iNumSpecies || iSpecies < 0) {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clAllometry::CalcSaplingHeight");
    sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized species ", iSpecies);
    throw(stcErr);
  }
...
float clAllometry::CalcSaplingDbh(float fHeight, const int & iSpecies) {
  if (iSpecies >= m_iNumSpecies || iSpecies < 0) {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clAllometry::CalcSaplingDbh");
    sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized species ", iSpecies);
    throw(stcErr);
  }
  //If the height is greater than or equal to canopy height, throw bad data error
...
float clAllometry::CalcSeedlingHeight(float fDiam10, const int & iSpecies) {
  if (iSpecies >= m_iNumSpecies || iSpecies < 0) {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clAllometry::CalcSeedlingHeight");
    sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized species ", iSpecies);
    throw(stcErr);
  }
...
float clAllometry::CalcSeedlingDiam10(float fHeight, const int & iSpecies) {
  if (iSpecies >= m_iNumSpecies || iSpecies < 0) {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clAllometry::CalcSeedlingDiam10");
    sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized species ", iSpecies);
    throw(stcErr);
  }
  ...
  if (fHeight >= 30.1) {
    ...
    strcpy(stcErr.cFunction, "clAllometry::CalcSeedlingDiam10");
    sprintf(stcErr.cMoreInfo, "%s%s", "Cannot set tree height greater than ",
        "maximum tree height for this species.");
    throw(stcErr);
  }
...
float clAllometry::ConvertDiam10ToDbh(float fDiam10, const int & iSpecies) {
  if (iSpecies >= m_iNumSpecies || iSpecies < 0) {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clAllometry::ConvertDiam10ToDbh");
    sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized species ", iSpecies);
    throw(stcErr);
  }
...
float clAllometry::ConvertDbhToDiam10(const float & fDbh, const int & iSpecies) {
  if (iSpecies >= m_iNumSpecies || iSpecies < 0) {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clAllometry::ConvertDbhToDiam10");
    sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized species ", iSpecies);
    throw(stcErr);
  }
...
float clAllometry::GetMaxTreeHeight(int iSpecies) {
  if (iSpecies < 0 || iSpecies >= m_iNumSpecies) {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clTreePopulation::GetMaxTreeHeight");
    sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized species ", iSpecies);
    throw(stcErr);
  }
...
float clAllometry::CalcNonSpatDensDepExpAdultCrownRad(clTree *p_oTree) {
  ...
  char cSearcher[15];
  sprintf(cSearcher, "%s%d", "type=", clTreePopulation::adult);
  clPlot *p_oPlot = m_oPop->mp_oSimManager->GetPlotObject();
  clTreeSearch *p_oAdults = m_oPop->Find(cSearcher);
  ...
  for (i = 0; i < m_iNumSpecies; i++) {
    if (standard == p_iWhatFunction[i]) {
      ...
      sprintf(stcErr.cMoreInfo, "%s%d",
          "Invalid adult height-diam function code: ", p_iWhatFunction[i]);
      throw(stcErr);
    }
  }
  ...
  if (iHowMany > 0) {
    ...
    for (i = 0; i < iHowMany; i++) {
      if (0 == p_fTemp[i].val) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clAllometry::GetData");
        strcpy(stcErr.cMoreInfo, "No values in adult linear slope may be 0.");
        throw(stcErr);
...
try {
  ...
  for (i = 0; i < m_iNumSpecies; i++) {
    if (standard_cr == p_iWhatFunction[i]) {
      ...
      strcpy(stcErr.cFunction, "clAllometry::GetData");
      sprintf(stcErr.cMoreInfo, "%s%d",
          "Invalid adult crown radius-diam function code: ",
          p_iWhatFunction[i]);
      throw(stcErr);
    }
  }
  ...
  if (iHowMany > 0) {
    ...
    for (i = 0; i < m_iNumSpecies; i++) {
      sprintf(cName, "%s%s%s", "tr_nciCR",
          m_oPop->TranslateSpeciesCodeToName(i), "NeighborLambda");
      FillSpeciesSpecificValue(p_oElement, cName, "tr_ncrlVal", p_fTemp,
...
void clAllometry::SetupSaplingHeightDiam(xercesc::DOMElement * p_oElement) {
  ...
  for (i = 0; i < m_iNumSpecies; i++) {
    if (standard == p_iWhatFunction[i]) {
      ...
      strcpy(stcErr.cFunction, "clAllometry::GetData");
      sprintf(stcErr.cMoreInfo, "%s%d",
          "Invalid sapling height-diam function code: ", p_iWhatFunction[i]);
      throw(stcErr);
    }
  }
...
try {
  ...
  for (i = 0; i < m_iNumSpecies; i++) {
    if (standard_cr == p_iWhatFunction[i]) {
      ...
      strcpy(stcErr.cFunction, "clAllometry::GetData");
      sprintf(stcErr.cMoreInfo, "%s%d",
          "Invalid sapling crown radius-diam function code: ",
          p_iWhatFunction[i]);
      throw(stcErr);
    }
  }
...
try {
  ...
  for (i = 0; i < m_iNumSpecies; i++) {
    if (standard_cd == p_iWhatFunction[i]) {
      ...
      strcpy(stcErr.cFunction, "clAllometry::GetData");
      sprintf(stcErr.cMoreInfo, "%s%d",
          "Invalid sapling crown height-diam function code: ",
          p_iWhatFunction[i]);
      throw(stcErr);
    }
  }
  ...
  if (iHowMany > 0) {
    ...
    for (i = 0; i < m_iNumSpecies; i++) {
      sprintf(cName, "%s%s%s", "tr_nciCD",
          m_oPop->TranslateSpeciesCodeToName(i), "NeighborLambda");
      FillSpeciesSpecificValue(p_oElement, cName, "tr_ncdlVal", p_fTemp,
          iHowMany, m_oPop, true);
      for (j = 0; j < iHowMany; j++) {
...
try {
  ...
  for (i = 0; i < m_iNumSpecies; i++) {
    if (standard_cd == p_iWhatFunction[i]) {
      ...
      strcpy(stcErr.cFunction, "clAllometry::GetData");
      sprintf(stcErr.cMoreInfo, "%s%d",
          "Invalid adult crown height-diam function code: ",
          p_iWhatFunction[i]);
      throw(stcErr);
    }
  }
...
try {
  ...
  for (i = 0; i < m_iNumSpecies; i++) {
    if (standard == p_iWhatFunction[i]) {
      ...
      strcpy(stcErr.cFunction, "clAllometry::GetData");
      sprintf(stcErr.cMoreInfo, "%s%d",
          "Invalid seedling height-diam function code: ", p_iWhatFunction[i]);
      throw(stcErr);
    }
  }
