...
clDimensionAnalysis::clDimensionAnalysis( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "DimensionAnalysis" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clDimensionAnalysis::clDimensionAnalysis" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (mp_iEquationID[i] < 1 || mp_iEquationID[i] > 9 ) {
        ...
        strcpy(stcErr.cFunction, "clDimensionAnalysis::GetData");
        sprintf(stcErr.cMoreInfo, "%s%d%s%d%s", "Unidentified equation ID \"",
          mp_iEquationID[i], "\" for species \"", mp_iWhatSpecies[i], "\".");
        throw(stcErr);
      }
    }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (mp_iWhatDia[i] < DBH || mp_iWhatDia[i] > DBH2 ) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clDimensionAnalysis::GetData");
        sprintf(stcErr.cMoreInfo, "%s%d%s%d%s", "Unidentified dia meaning \"",
          mp_iWhatDia[i], "\" for species \"", mp_iWhatSpecies[i], "\".");
        throw(stcErr);
      }
    }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (mm == p_iTempValues[i].val) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clDimensionAnalysis::GetData");
        sprintf(stcErr.cMoreInfo, "%s%d%s%d%s", "Unidentified DBH units code \"",
          p_iTempValues[i].val, "\" for species \"", mp_iWhatSpecies[i], "\".");
        throw(stcErr);
      }
    }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (g == p_iTempValues[i].val) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clDimensionAnalysis::GetData");
        sprintf(stcErr.cMoreInfo, "%s%d%s%d%s", "Unidentified biomass units code \"",
          p_iTempValues[i].val, "\" for species \"", mp_iWhatSpecies[i], "\".");
        throw(stcErr);
      }
    }
...
void clDimensionAnalysis::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDimensionAnalysis::Action" );
    throw( stcErr );
  }
}
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    ...
    if ( clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType
         ...
         && clTreePopulation::snag != mp_whatSpeciesTypeCombos[i].iType )
         {
           ...
           modelErr stcErr;
           strcpy( stcErr.cFunction, "clDimensionAnalysis::RegisterTreeDataMembers" );
