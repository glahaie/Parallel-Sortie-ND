...
clStormKilledPartitionedBiomass::clStormKilledPartitionedBiomass( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
	...
	{
		//Set the namestring
		strcpy( m_cNameString, "Storm Killed Paritioned Biomass" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clStormKilledPartitionedBiomass::clStormKilledPartitionedBiomass" );
    throw( stcErr );
  }
}
...
// SetNameData()
/////////////////////////////////////////////////////////////////////////////*/
void clStormKilledPartitionedBiomass::SetNameData(char *cNameString) {
 try {

   //Check the string passed and set the flags accordingly
   if (strcmp("StormKilledPartitionedDBHBiomass", cNameString) == 0) {
     m_bIsDbh = true;
   } else if (strcmp("StormKilledPartitionedHeightBiomass", cNameString) == 0) {
     m_bIsDbh = false;
   }
   else {
     modelErr stcErr;
     ...
     sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized behavior name \"",
        cNameString, "\".");
     strcpy(stcErr.cFunction, "clStormKilledPartitionedBiomass::SetNameData");
     throw(stcErr);
   }
 }
  ...
  try
  {
    ...
    for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
      if (mp_whatSpeciesTypeCombos[i].iType != clTreePopulation::snag) {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cMoreInfo, "Storm Killed Partitioned Biomass can only be applied to snags.");
        strcpy(stcErr.cFunction, "clStormKilledPartitionedBiomass::GetData");
        throw(stcErr);
      }
    }
...
void clStormKilledPartitionedBiomass::GetTreeCodes(clTreePopulation *p_oPop)
{
  ...
  for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
    ...
    if (-1 == mp_iStmDmgCodes[mp_iWhatSpecies[i]]) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cMoreInfo, "All snags to which the Storm Killed Partitioned Biomass behavior are applied must also have the Storm Damage Applier applied.");
      strcpy(stcErr.cFunction, "clStormKilledPartitionedBiomass::GetTreeCodes");
      throw(stcErr);
    }
  }
...
void clStormKilledPartitionedBiomass::SetupGrid(clTreePopulation *p_oPop)
{
  ...
  if (NULL == mp_oGrid) {
    ...
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "leaf_", i);
      mp_iLeafCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    //Register the data member called "branch_x"
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "branch_", i);
      mp_iBranchCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    //Register the data member called "bole_x"
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "bole_", i);
      mp_iBoleCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    //Register the data member called "hleaf_x"
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "hleaf_", i);
      mp_iHLeafCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    //Register the data member called "hbole_x"
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "hbole_", i);
      mp_iHBoleCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
  }
  else {
    ...
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "leaf_", i);
      mp_iLeafCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
      if (-1 == mp_iLeafCodes[i]) {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clStormKilledPartitionedBiomass::SetupGrid" );
        sprintf( stcErr.cMoreInfo, "%s%s%s",
          "Couldn't find the \"", cLabel,
    ...
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "branch_", i);
      mp_iBranchCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
    ...
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "bole_", i);
      mp_iBoleCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
    ...
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "hleaf_", i);
      mp_iHLeafCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
    ...
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "hbole_", i);
      mp_iHBoleCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
...
void clStormKilledPartitionedBiomass::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clStormKilledPartitionedBiomass::Action" );
    throw( stcErr );
  }
}
