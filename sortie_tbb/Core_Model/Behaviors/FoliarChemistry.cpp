...
clFoliarChemistry::clFoliarChemistry( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "FoliarChemistry" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clFoliarChemistry::clFoliarChemistry" );
    throw( stcErr );
  }
}
...
void clFoliarChemistry::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clFoliarChemistry::GetData" );
    throw( stcErr );
  }
}
...
void clFoliarChemistry::GetAppliesTo()
{
  ...
  for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
    if (clTreePopulation::seedling == mp_whatSpeciesTypeCombos[i].iType) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clFoliarChemistry::GetAppliesTo" );
      strcpy( stcErr.cMoreInfo, "Partitioned Biomass behavior cannot be applied to seedlings.");
...
void clFoliarChemistry::SetupGrid()
{
  ...
  if (NULL == mp_oGrid) {
    ...
    for (i = 0; i < m_iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "N_", i);
      mp_iNCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    //Register the data member called "P_x"
    for (i = 0; i < m_iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "P_", i);
      mp_iPCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    //Register the data member called "SLA_x"
    for (i = 0; i < m_iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "SLA_", i);
      mp_iSLACodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    //Register the data member called "lignin_x"
    for (i = 0; i < m_iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "lignin_", i);
      mp_iLigninCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    //Register the data member called "fiber_x"
    for (i = 0; i < m_iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "fiber_", i);
      mp_iFiberCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    //Register the data member called "cellulose_x"
    for (i = 0; i < m_iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "cellulose_", i);
      mp_iCelluloseCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    //Register the data member called "tannins_x"
    for (i = 0; i < m_iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "tannins_", i);
      mp_iTanninsCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    //Register the data member called "phenolics_x"
    for (i = 0; i < m_iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "phenolics_", i);
      mp_iPhenolicsCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
  }
  else {
    ...
    for (i = 0; i < m_iNumTotalSpecies; i++) {
      ...
      sprintf(cLabel, "%s%d", "N_", i);
      mp_iNCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
      if (-1 == mp_iNCodes[i]) {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clFoliarChemistry::SetupGrid" );
      ...
      sprintf(cLabel, "%s%d", "P_", i);
      mp_iPCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
      if (-1 == mp_iPCodes[i]) {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clFoliarChemistry::SetupGrid" );
        sprintf( stcErr.cMoreInfo, "%s%s%s",
      ...
      sprintf(cLabel, "%s%d", "SLA_", i);
      mp_iSLACodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
      if (-1 == mp_iSLACodes[i]) {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clFoliarChemistry::SetupGrid" );
        sprintf( stcErr.cMoreInfo, "%s%s%s",
      ...
      sprintf(cLabel, "%s%d", "lignin_", i);
      mp_iLigninCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
      ...
      sprintf(cLabel, "%s%d", "fiber_", i);
      mp_iFiberCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
      ...
      sprintf(cLabel, "%s%d", "cellulose_", i);
      mp_iCelluloseCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
      ...
      sprintf(cLabel, "%s%d", "tannins_", i);
      mp_iTanninsCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
      ...
      sprintf(cLabel, "%s%d", "phenolics_", i);
      mp_iPhenolicsCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
...
void clFoliarChemistry::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clFoliarChemistry::Action" );
    throw( stcErr );
  }
}
