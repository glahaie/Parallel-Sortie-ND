  ...
  clMortalityBase( p_oSimManager )
{
  try
  {
    strcpy( m_cNameString, "insect infestation mortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clInsectInfestationMortality::clInsectInfestationMortality" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if ( 0 > p_fTempValues[i].val || 1 < p_fTempValues[i].val ) {
        ...
        strcpy( stcErr.cFunction, "clInsectInfestationMortality::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "All values for mortality intercept must be between 0 and 1." );
        throw( stcErr );
      }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if ( 0 > p_fTempValues[i].val || 1 < p_fTempValues[i].val ) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clInsectInfestationMortality::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "All values for max mortality must be between 0 and 1." );
        throw( stcErr );
      }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if ( 0 == p_fTempValues[i].val) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clInsectInfestationMortality::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "Values for insect mortality X0 cannot be 0." );
        throw( stcErr );
      }
...
deadCode clInsectInfestationMortality::DoMort( clTree * p_oTree, const float & fDiam, const short int & iSpecies )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clInsectInfestationMortality::DoMort" );
    throw( stcErr );
  }
}
