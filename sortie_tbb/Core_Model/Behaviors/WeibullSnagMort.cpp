...
clMortalityBase( p_oSimManager )
{
  try
  {
    ...
    strcpy( m_cNameString, "weibsnagmortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clWeibullSnagMort::clWeibullSnagMort" );
    throw( stcErr );
  }
}
...
void clWeibullSnagMort::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clWeibullSnagMort::DoShellSetup" );
    throw( stcErr );
  }
}
...
void clWeibullSnagMort::ValidateTypes()
{
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    if ( clTreePopulation::snag != mp_whatSpeciesTypeCombos[i].iType )
    {
      ...
      strcpy( stcErr.cMoreInfo, "Snag weibull mortality can only be applied to snags." );
      strcpy( stcErr.cFunction, "clWeibullSnagMort::ValidateTypes" );
      throw( stcErr );
    }
  }
}
  ...
  try
  {
    ...
    for (i = 0; i < m_iNumBehaviorSpecies; i++) {
      if ( mp_fSnagSizeClasses[0][i] < 0 || mp_fSnagSizeClasses[1][i] < 0 || mp_fSnagSizeClasses[1][i] < mp_fSnagSizeClasses[0][i] )
      {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clWeibullSnagMort::ReadParameterFile" );
        strcpy( stcErr.cMoreInfo, "Snag size classes must be positive and cannot overlap." );
    ...
    for (i = 0; i < m_iNumSizeClasses; i++) {
      for (j = 0; j < m_iNumBehaviorSpecies; j++) {
         if (0 > mp_fAParameter[i][j] && (0 >= mp_fBParameter[i][j] || mp_fBParameter[i][j] - floor(mp_fBParameter[i][j]) != 0)) {
           ...
           stcErr.iErrorCode = BAD_DATA;
           strcpy( stcErr.cFunction, "clWeibullSnagMort::ReadParameterFile" );
           strcpy( stcErr.cMoreInfo, "All values for \"a\" must be a positive number." );
           delete[] p_fTempValues;
           throw( stcErr );
         }
      }
...
void clWeibullSnagMort::GetAgeCodes()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clWeibullSnagMort::GetAgeCodes" );
    throw( stcErr );
  }
}
...
void clWeibullSnagMort::FillDeathProbArray()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clWeibullSnagMort::CalculateDeathProbabilities" );
    throw( stcErr );
  }
}
...
deadCode clWeibullSnagMort::DoMort( clTree * p_oTree, const float & fDbh, const short int & iSpecies )
{
  ...
  {
    modelErr stcErr; stcErr.iErrorCode = UNKNOWN; strcpy( stcErr.cFunction, "clWeibullSnagMort::DoMort" ); throw( stcErr );
  }
}
