  ...
  clMortalityBase( p_oSimManager )
{
  try
  {
    strcpy( m_cNameString, "browsed stochastic mortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clBrowsedStochasticMortality::clBrowsedStochasticMortality" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      if ( 0 > p_fTempValues[i].val || 1 < p_fTempValues[i].val )
      {
        ...
        strcpy( stcErr.cFunction, "clBrowsedStochasticMortality::DoShellSetup" );
...
deadCode clBrowsedStochasticMortality::DoMort( clTree * p_oTree, const float & fDiam, const short int & iSpecies )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clBrowsedStochasticMortality::DoMort" );
    throw( stcErr );
  }
}
