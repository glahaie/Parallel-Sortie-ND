...
clStormLight::clStormLight( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  //Set the namestring
  strcpy( m_cNameString, "StormLight" );
...
void clStormLight::GetData( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clStormLight::GetData" );
    ...
    if (m_fMinCanopyTrees < 0) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clStormLight::GetData" );
      sprintf( stcErr.cMoreInfo, "%s", "Minimum number of trees for full canopy cannot be less than 0." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
...
void clStormLight::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clStormLight::Action" );
    throw( stcErr );
  }
}
