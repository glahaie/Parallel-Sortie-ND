...
clGapLight::clGapLight(clSimManager * p_oSimManager) :
  ...
  //Set the namestring
  strcpy(m_cNameString, "gaplightshell");
  m_sXMLRoot = "GapLight";
  ...
  try
  {
    ...
    else
    {
      ...
      if ( -1 == m_iGapCode )
      {
        ...
        strcpy( stcErr.cMoreInfo, "Gap Light grid was incorrectly set up in the parameter file.  Missing bool \"Is Gap\"." );
        throw( stcErr );
      }
    }
  }
...
void clGapLight::CalculateGapStatus( clTreePopulation * p_oPop )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clGapLight::CalculateGapStatus" );
    throw( stcErr );
  }
}
...
float clGapLight::CalcLightValue( clTree * p_oTree, clTreePopulation * p_oPop )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clGapLight::CalcLightValue" );
    throw( stcErr );
  }
}
