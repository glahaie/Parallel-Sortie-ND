  ...
  clMortalityBase( p_oSimManager )
{
  ...
  {
    //Set namestring
    strcpy( m_cNameString, "WeibullClimatemortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clWeibullClimateSurvival::clWeibullClimateSurvival" );
    throw( stcErr );
  }
}
    ...
    for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
      if ( clTreePopulation::sapling != mp_whatSpeciesTypeCombos[i].iType
           && clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType )
           {
             ...
             strcpy( stcErr.cFunction, "clWeibullClimateSurvival::ReadParameterFile" );
...
void clWeibullClimateSurvival::ValidateData()
{
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "All values for max potential survival must be between 0 and 1." );
        throw( stcErr );
      }
  ...
  {
    strcpy( err.cFunction, "clWeibullClimateSurvival::ValidateData" );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clWeibullClimateSurvival::ValidateData" );
    throw( stcErr );
  }
}
