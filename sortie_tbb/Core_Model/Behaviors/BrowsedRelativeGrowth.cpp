...
{
  strcpy(m_cNameString, "browsedrelgrowthshell");
...
// SetNameData()
//////////////////////////////////////////////////////////////////////////////
void clBrowsedRelativeGrowth::SetNameData( char * cNameString )
{
  try
  {

    //Check the string passed and set the flags accordingly
    if ( strcmp( "BrowsedRelativeGrowth", cNameString ) == 0 )
    {
      m_iGrowthMethod = diameter_auto;
    }
    else if ( strcmp( "BrowsedRelativeGrowth diam only", cNameString ) == 0 )
    {
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clBrowsedRelativeGrowth::SetNameData" );
    throw( stcErr );
  }
}
...
void clBrowsedRelativeGrowth::DoShellSetup( DOMDocument * p_oDoc )
{
  ...
  try
  {
    ...
    for (i = 0; i < m_iNumSpeciesTypeCombos; i++ )
    {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clBrowsedRelativeGrowth::DoShellSetup" );
...
float clBrowsedRelativeGrowth::CalcDiameterGrowthValue( clTree * p_oTree, clTreePopulation * p_oPop, float fHeightGrowth )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clBrowsedRelativeGrowth::CalcGrowthValue" );
    throw( stcErr );
  }
}
