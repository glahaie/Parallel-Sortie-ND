...
clQualityVigorClassifier::clQualityVigorClassifier(clSimManager *p_oSimManager) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ){
  ...
  strcpy(m_cNameString, "QualityVigorClassifier");
 ...
 try {
   ...
   for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
     if ( clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType )
     {
       ...
       strcpy( stcErr.cFunction, "clQualityVigorClassifier::RegisterTreeDataMembers" );
       strcpy( stcErr.cMoreInfo, "This behavior can only be applied to adults." );
       throw( stcErr );
     }
...
void clQualityVigorClassifier::ReadParameterFile(DOMDocument * p_oDoc, clTreePopulation *p_oPop)
{
  ...
  catch (...)
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clGermination::GetData");
    throw(stcErr);
  }
}
...
void clQualityVigorClassifier::EvolveVigorQuality(clTreePopulation *p_oPop, clTree *p_oTree) {
  ...
  switch (iClass) {
    ...
    default:
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clQualityVigorClassifier::EvolveVigorQuality" );
      strcpy( stcErr.cMoreInfo, "Unexpected class number." );
      throw( stcErr );
  }
