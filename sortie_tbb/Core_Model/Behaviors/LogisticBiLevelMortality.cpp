...
clLogisticBiLevelMortality::clLogisticBiLevelMortality(clSimManager *p_oSimManager) :
 ...
 try {
   ...
   strcpy(m_cNameString, "logistic bilevel mortshell");
   ...
   strcpy(stcErr.cFunction, "clLogisticBiLevelMortality::clLogisticBiLevelMortality");
   throw(stcErr);
 }
}
    ...
    if (NULL != mp_oStormLight) {
      ...
      for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
        if (p_fTempValues[i].val < 0 || p_fTempValues[i].val > 100) {
          ...
          strcpy(stcErr.cFunction, "clLogisticBiLevelMortality::DoShellSetup");
...
deadCode clLogisticBiLevelMortality::DoMort(clTree *p_oTree, const float &fDiam,
  ...
  try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clLogisticBiLevelMortality::DoMort");
   throw(stcErr);
 }
}
