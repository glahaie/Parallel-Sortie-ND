  ...
  clBehaviorBase( p_oSimManager ), clLightBase(p_oSimManager)
{
  ...
  //Set the namestring
  strcpy( m_cNameString, "averagelightshell" );
...
void clAverageLight::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    if ( !mp_oGLIMapGrid )
    {
      ...
      strcpy( stcErr.cFunction, "clAverageLight::DoShellSetup" );
    ...
    if ( -1 == m_iMapLightCode )
    {
      ...
      strcpy( stcErr.cFunction, "clAverageLight::DoShellSetup" );
      strcpy( stcErr.cMoreInfo,
           "GLI Map grid set up incorrectly." );
      throw( stcErr );
    }
...
void clAverageLight::CalculateAverage(clTreePopulation *p_oPop)
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clAverageLight::CalculateAverage" );
    throw( stcErr );
  }
}
