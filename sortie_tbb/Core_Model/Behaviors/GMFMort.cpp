...
clGMFMort::clGMFMort(clSimManager *p_oSimManager) :
 ...
 try {
   ...
   strcpy(m_cNameString, "gmfmortshell");
   ...
   strcpy(stcErr.cFunction, "clGMFMort::clGMFMort");
   throw(stcErr);
 }
}
...
void clGMFMort::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
 try {
   ...
   if (5 != mp_oSimManager->GetNumberOfYearsPerTimestep()) {
     ...
     strcpy(stcErr.cMoreInfo, "GMF Mortality cannot be used if the timestep length is not 5 years.");
     strcpy(stcErr.cFunction, "clGMFMort::DoShellSetup");
     throw(stcErr);
   }
     ...
     for (j = 0; j < iTotalTypes; j++)
       if (mp_bUsesThisMortality[i][j]) {
         ...
         if (-1 == mp_iGrowthCodes[i][j]) {
           modelErr stcErr;
           strcpy(stcErr.cFunction, "clGMFMort::GetGrowthVariableCodes");
           sprintf(stcErr.cMoreInfo, "%s%d%s%d%s%s",
                  "Type/species combo species=", i, " type=",
                  j, " does not have a growth behavior compatible with",
