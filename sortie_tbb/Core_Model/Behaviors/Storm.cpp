...
clStorm::clStorm( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clStorm::clStorm" );
    throw( stcErr );
  }
}
...
void clStorm::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clStorm::GetData" );
    throw( stcErr );
  }
...
void clStorm::ReadParFile( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    {
      sprintf( cLabel, "%s%d%s", "st_s", i + 1, "ReturnInterval" );
      FillSingleValue( p_oDocElement, cLabel, &fTemp, true );
      //Validate that all values are greater than or equal to 0
      if ( fTemp < 0 )
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clStorm::ReadParFile" );
        strcpy( stcErr.cMoreInfo, "Storm return intervals cannot be negative." );
    ...
    else
    {
      ...
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clStorm::ReadParFile" );
      sprintf( stcErr.cMoreInfo, "%s%d", "Unrecognized value for susceptibility: ", iTemp );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
    ...
    else
    {
      ...
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clStorm::ReadParFile" );
      sprintf( stcErr.cMoreInfo, "%s%d", "Unrecognized value for stochasticity: ", iTemp );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
    ...
    if ( stochastic == m_iStochasticity )
    {
      ...
      else
      {
        ...
        sprintf( stcErr.cMoreInfo, "%s%d",
             "Unrecognized value for damage pattern probability distribution function: ", iTemp );
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      }
      ...
      {
        FillSingleValue( p_oDocElement, "st_standardDeviation", & mp_fStdDev, true );
      }
    }
    ...
    if (fabs(m_fSineD) > VERY_SMALL_VALUE && fabs(m_fSSTPeriod) < VERY_SMALL_VALUE) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clStorm::ReadParFile" );
      strcpy( stcErr.cMoreInfo, "SST periodicity cannot be zero." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
...
void clStorm::DoGridSetup()
{
  try
  {
    ...
    if ( mapped == m_iSusceptibility )
    {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clStorm::DoGridSetup" );
        strcpy( stcErr.cMoreInfo, "A grid map for the grid \"Storm Susceptibility\" is required for this file." );
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      }
      ...
      if ( -1 == m_iSusceptIndexCode )
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clStorm::DoGridSetup" );
        strcpy( stcErr.cMoreInfo, "The grid map for the grid \"Storm Susceptibility\" is missing the data member\"index\"." );
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      }
    }
    ...
    if ( NULL != mp_oStormGrid )
    {
      ...
      if ( -1 == m_i1DmgIndexCode)
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clStorm::DoGridSetup" );
        strcpy( stcErr.cMoreInfo, "The grid map for the grid \"Storm Damage\" is missing the data member\"1dmg_index\"." );
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      }
      if ( -1 == m_iDmgIndexCode)
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clStorm::DoGridSetup" );
        strcpy( stcErr.cMoreInfo, "The grid map for the grid \"Storm Damage\" is missing the data member\"dmg_index\"." );
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      }
      if ( -1 == m_iStormTimeCode)
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clStorm::DoGridSetup" );
        strcpy( stcErr.cMoreInfo, "The grid map for the grid \"Storm Damage\" is missing the data member\"stormtime\"." );
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      }
    }
...
void clStorm::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clStorm::Action" );
    throw( stcErr );
  }
}
