...
class clStorm : virtual public clBehaviorBase {
  ...
  /**This is for testing purposes - whenever there's a heavy stochastic
  * component to the operation of a behavior the only effective testing happens
  * when the testing class can get in and rig events to happen*/
  friend class clTestStorm;
  ...
  */
  inline float NormalDraw(const float &fMean) {
    float fVal = fMean + clModelMath::NormalRandomDraw(mp_fStdDev);
  ...
  */
  inline float LognormalDraw(const float &fMean) {
    return clModelMath::LognormalRandomDraw(fMean, mp_fStdDev);
  };
};
//---------------------------------------------------------------------------
#endif // Storm_H
