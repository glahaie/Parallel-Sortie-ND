...
clMicroEstablishment::clMicroEstablishment(clSimManager * p_oSimManager) :
  ...
  try {
    strcpy(m_cNameString, "MicroEstablishment");
    ...
    strcpy(stcErr.cFunction, "clMicroEstablishment::clMicroEstablishment");
    throw(stcErr);
  }
}
...
void clMicroEstablishment::GetParameterFileData(xercesc::DOMDocument * p_oDoc) {
  ...
  if (m_fMoundProportion < 0 || m_fMoundProportion > 1) {
    ...
    strcpy(stcErr.cFunction, "clMicroEstablishment::GetParameterFileData");
    strcpy(stcErr.cMoreInfo,
        "Proportion of plot that is mound must be between 0 and 1.");
    throw(stcErr);
  }
}
...
void clMicroEstablishment::GetData(xercesc::DOMDocument * p_oDoc) {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clMicroEstablishment::GetData");
    throw(stcErr);
  }
}
...
void clMicroEstablishment::SetupGrids() {
  ...
  if (!mp_oSubstrateGrid) {
    ...
    strcpy(stcErr.cFunction, "clMicroEstablishment::SetUpGrids");
  ...
  for (i = 0; i < number_substrates; i++) {
    if (-1 == mp_iSubstrateCodes[i]) {
      ...
      strcpy(stcErr.cFunction, "clMicroEstablishment::SetUpGrids");
      strcpy(stcErr.cMoreInfo,
          "Unexpected grid data return code for Substrate.");
      throw(stcErr);
    }
  }
  ...
  if (-1 == m_iCohFreshLogCode || -1 == m_iCohAgeCode) {
    ...
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clMicroEstablishment::SetUpGrids");
    strcpy(stcErr.cMoreInfo, "Unexpected grid data return code for Substrate.");
    throw(stcErr);
  }
...
void clMicroEstablishment::GetTreeDataMemberCodes() {
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if (-1 == mp_iCounterCodes[i] || -1 == mp_iZCodes[i]) {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "clMicroEstablishment::GetTreeDataMemberCodes");
      strcpy(stcErr.cMoreInfo,
          "Microtopographic Establishment must be used with the light filter behavior.");
      stcErr.iErrorCode = BAD_DATA;
      throw(stcErr);
    }
  }
...
void clMicroEstablishment::Action() {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clMicroEstablishment::Action");
    throw(stcErr);
  }
}
