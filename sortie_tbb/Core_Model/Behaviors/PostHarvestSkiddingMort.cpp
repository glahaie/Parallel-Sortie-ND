...
clMortalityBase(p_oSimManager) {
  try {
    ...
    strcpy(m_cNameString, "PostHarvestSkiddingmortshell");
    ...
    strcpy(stcErr.cFunction, "clPostHarvestSkiddingMort::clPostHarvestSkiddingMort");
    throw(stcErr);
  }
}
...
void clPostHarvestSkiddingMort::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
        ...
        clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType)
    {
      ...
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fPreHarvestBackgroundMort[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_preHarvestBackgroundMort must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fWindthrowHarvestBasicProb[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_windthrowHarvestBasicProb must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fSnagRecruitHarvestBasicProb[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_snagRecruitHarvestBasicProb must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fWindthrowSizeEffect[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_windthrowSizeEffect must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fWindthrowHarvestIntensityEffect[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_windthrowHarvestIntensityEffect must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fSnagRecruitHarvestIntensityEffect[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_snagRecruitHarvestIntensityEffect must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fWindthrowCrowdingEffect[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_windthrowCrowdingEffect must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fSnagRecruitCrowdingEffect[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_snagRecruitCrowdingEffect must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fWindthrowHarvestRateParam[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_windthrowHarvestRateParam must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fSnagRecruitHarvestRateParam[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_snagRecruitHarvestRateParam must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fWindthrowBackgroundProb[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_windthrowBackgroundProb must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  for (i = 0; i < m_iNumBehaviorSpecies; i++) {
    ...
    if ( mp_fSnagRecruitBackgroundProb[p_fTempValues[i].code] < 0 ) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_snagRecruitBackgroundProb must be non-negative." );
      throw( stcErr );
    }
  }
  ...
  if ( m_fCrowdingEffectRadius <= 0 ) {
    ...
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clPostHarvestSkiddingMort::DoShellSetup" );
    strcpy( stcErr.cMoreInfo, "The Post-harvest skidding mortality parameter mo_crowdingEffectRadius must be greater than zero." );
    throw( stcErr );
  }
...
void clPostHarvestSkiddingMort::PreMortCalcs( clTreePopulation *p_oPop ) {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clPostHarvestSkiddingMort::PreMortCalcs");
    throw(stcErr);
  }
}
...
void clPostHarvestSkiddingMort::CalcTimeSinceHarvest() {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clPostHarvestSkiddingMort::CalcTimeSinceHarvest");
    throw(stcErr);
  }
}
...
void clPostHarvestSkiddingMort::SetupTimeSinceHarvestGrid() {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clPostHarvestSkiddingMort::SetupTimeSinceHarvestGrid");
    throw(stcErr);
  }
}
