...
clMerchValueCalculator::clMerchValueCalculator( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
	strcpy( m_cNameString, "MerchValueCalculator" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clMerchValueCalculator::clMerchValueCalculator" );
    throw( stcErr );
  }
}
  ...
  try {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      ...
      if (78 == iFormClass ||
        ...
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clMerchValueCalculator::GetParameterFileData" );
        sprintf( stcErr.cMoreInfo, "%s%f%s%d%s", "Invalid form class value \"",
          mp_fFormClass[i], "\" for species ", mp_iWhatSpecies[i],
...
void clMerchValueCalculator::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clMerchValueCalculator::GetData" );
    throw( stcErr );
  }
}
...
float clMerchValueCalculator::GetTreeValue(const float &fDBH, const int &iSpecies)
{
  ...
  //Form class 85
    ...
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clMerchValueCalculator::GetTreeValue" );
    sprintf( stcErr.cMoreInfo, "%s%f%s%d%s", "Invalid form class value \"",
        mp_fFormClass[iSpecies], "\" for species ", iSpecies,
        ".  Valid values are: 78, 79, 80, 81, 84, and 85.");
    stcErr.iErrorCode = BAD_DATA;
    throw( stcErr );
  }
...
void clMerchValueCalculator::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clMerchValueCalculator::Action" );
    throw( stcErr );
  }
}
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    ...
    if ( clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType
         ...
         && clTreePopulation::snag != mp_whatSpeciesTypeCombos[i].iType )
         {
           ...
           modelErr stcErr;
           strcpy( stcErr.cFunction, "clMerchValueCalculator::RegisterTreeDataMembers" );
