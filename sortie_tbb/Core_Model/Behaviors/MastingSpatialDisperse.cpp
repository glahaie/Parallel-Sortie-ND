  ...
  clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clDisperseBase( p_oSimManager)
{
  try
  {
    strcpy( m_cNameString, "MastingSpatialDisperse" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clMastingSpatialDisperse::clMastingSpatialDisperse" );
    throw( stcErr );
  }
}
...
void clMastingSpatialDisperse::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clMastingSpatialDisperse::DoShellSetup" );
    throw( stcErr );
  }
}
  ...
  try {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
      if ( p_iTemp[i].val != weibull && p_iTemp[i].val != lognormal )
      {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clMastingSpatialDisperse::GetParameterFileData" );
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (deterministic_pdf == p_iTemp[i].val)
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clMastingSpatialDisperse::GetParameterFileData" );
        sprintf( stcErr.cMoreInfo, "%s%d", "Invalid STR pdf code ", p_iTemp[i].val );
        throw( stcErr );
      }
    }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
      if (p_fTemp[i].val < 0 || p_fTemp[i].val > 1) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clMastingSpatialDisperse::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "Fraction participating must be between 0 and 1.");
        throw( stcErr );
      }
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    ...
    if ( iNumFunctionSpecies > 0 )
    {
      ...
      for ( i = 0; i < iNumFunctionSpecies; i++ ) {
        ...
        if (p_fTemp[i].val >= 50) {
          modelErr stcErr;
          stcErr.iErrorCode = BAD_DATA;
          strcpy( stcErr.cFunction, "clMastingSpatialDisperse::CalcSeedCDF" );
...
void clMastingSpatialDisperse::DisperseOneParentSeeds( clTree * p_oTree, clTreePopulation * p_oPop, clPlot * p_oPlot, float fDbh )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSpatialDisperse::DisperseOneParentSeeds" );
    throw( stcErr );
  }
}
...
void clMastingSpatialDisperse::CalcMastCDF( xercesc::DOMDocument * p_oDoc )
{
  ...
  try {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
      if (p_fTemp[i].val == 0) {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clMastingSpatialDisperse::CalcMastCDF" );
        strcpy( stcErr.cMoreInfo, "Mast \"a\" cannot be 0.");
        throw( stcErr );
      }
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
  ...
  catch ( ... )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSpatialDisperse::CalcMastCDF" );
    throw( stcErr );
  }
}
