...
clShortOutput::clShortOutput( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  try
  {
    strcpy( m_cNameString, "ShortOutput" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clShortOutput::clShortOutput" );
    throw( stcErr );
  }
}
...
void clShortOutput::GetData( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    char * cPos; //for string searching
    ...
    //Get the filename
    FillSingleValue( p_oElement, "so_filename", m_cFileName, MAX_FILENAME_SIZE, true );

    //Make sure the file extension is on - if not, append it
    cPos = strstr( m_cFileName, SHORT_OUTPUT_FILE_EXT );
    if ( NULL == cPos )
    {
      //Make sure we don't exceed string length - chop it if necessary to allow
      //enough space for string concatenation
      if ( strlen( m_cFileName ) >= ( MAX_FILENAME_SIZE - strlen( SHORT_OUTPUT_FILE_EXT ) ) )
        m_cFileName[MAX_FILENAME_SIZE - strlen( SHORT_OUTPUT_FILE_EXT ) - 1] = '\0';
      strcat( m_cFileName, SHORT_OUTPUT_FILE_EXT );
    }
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clShortOutput::GetData" );
    throw( stcErr );
  }
}
...
void clShortOutput::ExtractLiveTreeInfo( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    for ( i = 0; i < iNumTypes; i++ ) {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clShortOutput::ExtractTreeInfo" );
        stcErr.iErrorCode = BAD_DATA;
        sprintf( stcErr.cMoreInfo, "%s%s", "Unrecognized or unsupported type - ", cData );
        throw( stcErr );
      }
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clShortOutput::ExtractLiveTreeInfo" );
    throw( stcErr );
  }
}
...
void clShortOutput::ExtractDeadTreeInfo( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    for ( i = 0; i < iNumTypes; i++ ) {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clShortOutput::ExtractDeadTreeInfo" );
        stcErr.iErrorCode = BAD_DATA;
        sprintf( stcErr.cMoreInfo, "%s%s", "Unrecognized or unsupported type - ", cData );
        throw( stcErr );
      }
...
void clShortOutput::ExtractSubplotInfo( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    for ( i = 0; i < m_iNumSubplotsToSave; i++ )
    {
      ...
      //Get subplot name
      FillSingleValue( p_oSubplot, "so_subplotName", mp_subplots[i].cSubplotName, MAX_SUBPLOT_NAME_SIZE, true );
      ...
      if ( 0 == p_oGridCellList->getLength() )
      {
        ...
        strcpy( stcErr.cFunction, "clShortOutput::ExtractSubplotInfo" );
...
void clShortOutput::WriteFileHeader()
{
  try
  {
    ...
    fstream out( m_cFileName, ios::out | ios::trunc );
    char cABAString[80],
         cABATotalString[80],
         cRBAString[80],
         cADNString[80],
    ...
    //Write subplot list
    for ( i = 0; i < m_iNumSubplotsToSave; i++ )
      out << i + 1 << " = " << mp_subplots[i].cSubplotName << "\n";
    ...
    if (!DoesFileExist(m_cFileName)) {
      modelErr stcErr;
      stcErr.iErrorCode = DATA_MISSING;
      strcpy( stcErr.cFunction, "clShortOutput::WriteFileHeader" );
      sprintf( stcErr.cMoreInfo, "%s%s%s", "SORTIE could not write the file \"",
              m_cFileName, "\". Check for invalid directories." );
      throw( stcErr );
    }
  }
  catch ( modelErr & err )
...
void clShortOutput::GetLiveTreeStats() {
 try {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clShortOutput::GetLiveTreeStats" );
    throw( stcErr );
  }
}
...
void clShortOutput::GetDeadTreeStats() {
 try {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clShortOutput::GetDeadTreeStats" );
    throw( stcErr );
  }
}
...
void clShortOutput::WriteTimestepData()
{
  ...
  {
    using namespace std;
    fstream out( m_cFileName, ios::app | ios::out );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clShortOutput::WriteTimestepData" );
    throw( stcErr );
  }
}
