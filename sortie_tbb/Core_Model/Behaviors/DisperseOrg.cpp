...
clDisperseOrg::clDisperseOrg(clDisperseBase * p_oHookedShell)
{
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "clDisperseOrg::clDisperseOrg");
...
void clDisperseOrg::DoSetup(clSimManager * p_oSimManager, xercesc::DOMDocument * p_oDoc)
{
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "clDisperseOrg::DoSetup");
      strcpy(stcErr.cMoreInfo, "Null pointer was passed to the function.");
      stcErr.iErrorCode = CANT_FIND_OBJECT;
      throw(stcErr);
    }
...
void clDisperseOrg::GetSeedGridObject(clSimManager * p_oSimManager)
{
  ...
  {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clDisperseOrg::GetSeedGridObject");
    strcpy(stcErr.cMoreInfo, "Can't find grid \"Dispersed Seeds\".");
    stcErr.iErrorCode = CANT_FIND_OBJECT;
    throw(stcErr);
  }
  ...
  for (i = 0; i < m_iTotalSpecies; i++)
  {
    ...
    mp_iCodes[i] = mp_oSeedsGrid->GetFloatDataCode(cLabel);
    if (-1 == mp_iCodes[i])
    {
      modelErr stcErr;
...
void clDisperseOrg::GetParameterFileData(clSimManager * p_oSimManager, xercesc::DOMDocument * p_oDoc)
{
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "clDisperseOrg::GetParameterFileData");
      sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized value for di_seedDistributionMethod: ", iTemp);
      stcErr.iErrorCode = BAD_DATA;
      throw(stcErr);
    }
...
void clDisperseOrg::GetDisperseObjects(clSimManager * p_oSimManager)
{
  try
  {
    ...
    char cBehaviorName[MAX_NAMESTRING_SIZE], //behavior namestrings
         cShellMarker1[] = "disperse",
         cShellMarker2[] = "Disperse";
    ...
    for (i = 0; i < iNumBehaviors; i++)
    {
      ...
      if (NULL != strstr(cBehaviorName, cShellMarker1) ||
          NULL != strstr(cBehaviorName, cShellMarker2)) {
        m_iNumDispObjects++;
      }
    }
  ...
  catch (...)
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clDisperseOrg::PopulateDisperseFunctionsTable");
    throw(stcErr);
  }
}
...
void clDisperseOrg::AdjustSeedNumbers()
{
  ...
  catch (...)
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clDisperseOrg::AdjustSeedNumbers");
    throw(stcErr);
  }
}
...
void clDisperseOrg::DoDisperse()
{
  ...
  catch (...)
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clDisperseOrg::DoDisperse");
    throw(stcErr);
  }
}
