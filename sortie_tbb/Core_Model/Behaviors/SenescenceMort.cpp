...
clSenescenceMort::clSenescenceMort( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clMortalityBase( p_oSimManager )
{
  try
  {
    ...
    strcpy( m_cNameString, "senescencemortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clSenescenceMort::clSenescenceMort" );
    throw( stcErr );
  }
}
...
deadCode clSenescenceMort::DoMort(clTree * p_oTree, const float & fDiam, const short int & iSpecies )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSenescenceMort::DoMort" );
    throw( stcErr );
  }
}
...
void clSenescenceMort::CalculateMortalityProbability()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSenescenceMort::CalculateMortalityProbability" );
    throw( stcErr );
  }
}
