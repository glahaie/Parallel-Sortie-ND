  ...
  clBehaviorBase( p_oSimManager ), clLightBase(p_oSimManager)
{
  ...
  //Set the namestring
  strcpy( m_cNameString, "densitylightshell" );
...
void clDensityLight::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    if (0 == m_fC) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clDensityLight::GetData" );
      strcpy( stcErr.cMoreInfo, "Density light \"c\" parameter cannot equal 0." );
    ...
    if (m_iChangeThreshold < 0) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clDensityLight::GetData" );
      strcpy( stcErr.cMoreInfo, "Density light change threshold cannot be less than 0." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
    ...
    if (m_fMinDbh < 0) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clDensityLight::GetData" );
      strcpy( stcErr.cMoreInfo, "Density light minimum DBH cannot be less than 0." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
...
void clDensityLight::UpdateGridValues(clTreePopulation * p_oPop)
{
  try
  {
    ...
    char cQuery[50]; //format search string into this
    ...
    sprintf(cQuery, "%s%i%s%i", "type=",clTreePopulation::sapling,",",clTreePopulation::adult);
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDensityLight::Action" );
    throw( stcErr );
  }
}
