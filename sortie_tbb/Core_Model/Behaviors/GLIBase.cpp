...
{
    AddTreeToGliFishEye( fTargetX, fTargetY, fTargetHeight, p_oNeighbor, p_oPlot,
            p_oPop, p_oAllom, mp_fPhoto);
}
...
void clGLIBase::AddTreeToGliFishEye(const float &fTargetX, const float
    &fTargetY, const float &fTargetHeight, clTree *p_oNeighbor, clPlot *p_oPlot,
    clTreePopulation *p_oPop, clAllometry *p_oAllom, float** mp_ffPhoto)
{
  try {
    ...
    if (fDistToNearEdge > 0) {
      ...
      GLIFisheyeNoCanopyIntersect(fTargetX, fTargetY, fNeighX, fNeighY,
          ...
          fDistToNearEdge, p_oNeighbor, p_oPlot, p_oAllom, mp_ffPhoto);
    ...
    ************************************************/
      ...
      GLIFisheyeCanopyIntersect(fTargetX, fTargetY, fNeighX, fNeighY,
          fNeighCanrad, fNeighHeight, iSpecies, fTargetHeight,
          p_oNeighbor, p_oPlot, p_oAllom, mp_ffPhoto);
    ...
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clTradLight::FishEye");
...
void clGLIBase::GLIFisheyeNoCanopyIntersect(const float &fTargetX, const float
  ...
  clTree *p_oNeighbor, clPlot *p_oPlot, clAllometry *p_oAllom, float** mp_ffPhoto) {
 ...
 try {
   ...
   while (1) {
     ...
     if (fC >= 0.0) {
       ...
       for (i = iAltStart; i <= iAltEnd; i++)
         mp_ffPhoto[i][iAziChunk] *= fLightExtCoeff;
   ...
   modelErr stcErr;
   strcpy(stcErr.cFunction, "clTradLight::GLIFisheyeNoCanopyIntersect");
   stcErr.iErrorCode = UNKNOWN;
   throw(stcErr);
 }
}
...
void clGLIBase::GLIFisheyeCanopyIntersect(const float &fTargetX, const float
  ...
  clAllometry *p_oAllom, float** mp_ffPhoto) {
 try {
     ...
     for (i = 0; i < iHalfAzi; i++) {
       ...
       for (j = 0; j < 2; j++) {
         ...
         for (k = iAltStart; k <= iAltEnd; k++)
           mp_ffPhoto[k][iAziChunk] *= fLightExtCoeff;
