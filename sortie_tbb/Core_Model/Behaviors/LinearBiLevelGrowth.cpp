     ...
     clGrowthBase( p_oSimManager )
{
  ...
  try
  {
    ...
    strcpy( m_cNameString, "linearbilevelgrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clLinearBiLevelGrowth::clLinearBiLevelGrowth" );
    throw( stcErr );
  }
}
    ...
    if (NULL != mp_oStormLight) {
      ...
      for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
        if (p_fTempValues[i].val < 0 || p_fTempValues[i].val > 100) {
          ...
          strcpy(stcErr.cFunction, "clLinearBiLevelGrowth::DoShellSetup");
...
// SetNameData()
/////////////////////////////////////////////////////////////////////////////*/
void clLinearBiLevelGrowth::SetNameData(char *cNameString) {
 try {

   //Check the string passed and set the flags accordingly
   if (strcmp("LinearBilevelGrowth", cNameString) == 0) {
     m_iGrowthMethod = diameter_auto;
   } else if (strcmp("LinearBilevelGrowth diam only", cNameString) == 0) {
     ...
     sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized behavior name \"",
        cNameString, "\".");
     strcpy(stcErr.cFunction, "clLinearBiLevelGrowth::SetNameData");
     throw(stcErr);
   }
 }
...
float clLinearBiLevelGrowth::CalcDiameterGrowthValue(clTree *p_oTree, clTreePopulation *p_oPop, float fHeightGrowth) {
  try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clLinearBiLevelGrowth::CalcDiameterGrowthValue");
   throw(stcErr);
 }
}
