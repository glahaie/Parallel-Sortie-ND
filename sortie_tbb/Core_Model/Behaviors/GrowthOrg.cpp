...
clGrowthOrg::clGrowthOrg( clGrowthBase * p_oHookedShell )
{
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clGrowthOrg::clGrowthOrg" );
...
void clGrowthOrg::DoSetup( clSimManager * p_oSimManager, xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clGrowthOrg::DoSetup" );
      strcpy( stcErr.cMoreInfo, "Null pointer was passed to the function." );
      stcErr.iErrorCode = CANT_FIND_OBJECT;
      throw( stcErr );
    }
      ...
      if ( NULL != strstr( cBehaviorName, cShellMarker ) )
      {
        ...
        if ( clGrowthBase::diameter_auto == iGrowthType || clGrowthBase::diameter_only == iGrowthType )
        {
          ...
          for ( j = 0; j < iNumCombos; j++ )
          {
            ...
            {
              modelErr stcErr;
              strcpy( stcErr.cFunction, "clGrowthOrg::PopulateGrowthTables" );
      ...
      for ( j = 0; j < m_iTotalTypes; j++ )
      {
        ...
        if ( ( NULL == mp_oDiameterGrowthTable[i] [j] && NULL != mp_oHeightGrowthTable[i] [j] )
             ...
             && NULL == mp_oHeightGrowthTable[i] [j] ) )
             {
               ...
               modelErr stcErr;
               strcpy( stcErr.cFunction, "clGrowthOrg::PopulateGrowthTables" );
               sprintf( stcErr.cMoreInfo, "%s%d%s%d%s", "Type/species combo species=", i, " type=", j,
...
void clGrowthOrg::GetLightVariableCodes()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clGrowthOrg::GetLightVariableCodes" );
    throw( stcErr );
  }
}
    ...
    for ( i = 0; i < m_iTotalSpecies; i++ )
      for ( j = 0; j < m_iTotalTypes; j++ )
        if ( NULL != mp_oDiameterGrowthTable[i] [j] )
        {
          ...
          {
            modelErr stcErr;
            strcpy( stcErr.cFunction, "clGrowthOrg::GetDiamVariableCodes" );
            sprintf( stcErr.cMoreInfo, "%s%d%s%d%s", "Type/species combo species=", i, " type=", j,
    ...
    for ( i = 0; i < m_iTotalSpecies; i++ )
      for ( j = 0; j < m_iTotalTypes; j++ )
        if ( NULL != mp_oHeightGrowthTable[i] [j] )
        {
          ...
          {
            modelErr stcErr;
            strcpy( stcErr.cFunction, "clGrowthOrg::GetHeightVariableCodes" );
            sprintf( stcErr.cMoreInfo, "%s%d%s%d%s", "Type/species combo species=", i, " type=", j,
...
void clGrowthOrg::DoGrowthAssignments()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clGrowthOrg::DoGrowthAssignments" );
    throw( stcErr );
  }
}
...
void clGrowthOrg::DoTreeDataMemberRegistrations( clSimManager * p_oSimManager, clTreePopulation * p_oPop )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clGrowthOrg::DoTreeDataMemberRegistrations" );
    throw( stcErr );
  }
}
