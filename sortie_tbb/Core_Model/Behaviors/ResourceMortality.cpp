...
clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clMortalityBase( p_oSimManager )
{
  try
  {
    ...
    strcpy( m_cNameString, "resourcemortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clResourceMortality::clResourceMortality" );
    throw( stcErr );
  }
}
...
void clResourceMortality::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clResourceMortality::DoShellSetup" );
    throw( stcErr );
  }
}
...
void clResourceMortality::GetResourceGrid()
{
  ...
  {
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clResourceMortality::GetResourceGrid" );
    ...
    for ( i = 0; i < m_iNumTotalSpecies; i++ )
      for ( j = 0; j < iTotalTypes; j++ )
        if ( mp_bUsesThisMortality[i] [j] )
        {
          ...
          {
            modelErr stcErr;
            strcpy( stcErr.cFunction, "clResourceMortality::GetTreeDataMemberCodes" );
...
deadCode clResourceMortality::DoMort(clTree * p_oTree, const float & fDbh, const short int & iSpecies )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clResourceMortality::DoMort" );
    throw( stcErr );
  }
}
