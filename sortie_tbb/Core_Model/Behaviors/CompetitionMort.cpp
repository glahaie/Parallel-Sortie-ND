...
clCompetitionMort::clCompetitionMort(clSimManager *p_oSimManager) :
 ...
 try {
   ...
   strcpy(m_cNameString, "competitionmortshell");
   ...
   strcpy(stcErr.cFunction, "clCompetitionMort::clCompetitionMort");
   throw(stcErr);
 }
}
 ...
 try {
   ...
   for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
      if ( clTreePopulation::seedling == mp_whatSpeciesTypeCombos[i].iType )
      {
        ...
        strcpy( stcErr.cFunction, "clCompetitionMort::DoShellSetup" );
   ...
   //Capture the values from the parameter file
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      if ( mp_fCompMort[mp_iIndexes[p_fTempValues[i].code]] <= 0 )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clCompetitionMort::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "The Competition mortality parameter mp_fCompMort must be a positive number." );
        throw( stcErr );
      }
    }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      if ( mp_fCompMortMax[mp_iIndexes[p_fTempValues[i].code]] <= 0 )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clCompetitionMort::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "The Competition mortality parameter mp_fCompMortMax must be a positive number." );
        throw( stcErr );
      }
    }
     ...
     for (j = 0; j < iTotalTypes; j++)
       if (mp_bUsesThisMortality[i][j]) {
         ...
         if (-1 == mp_iGrowthCodes[i][j]) {
           modelErr stcErr;
           strcpy(stcErr.cFunction, "clCompetitionMort::GetGrowthVariableCodes");
           sprintf(stcErr.cMoreInfo, "%s%d%s%d%s%s",
                  "Type/species combo species=", i, " type=",
                  j, " does not have a light behavior compatible with",
                  " its mortality behavior.");
           stcErr.iErrorCode = BAD_DATA;
           throw(stcErr);
         }
       }
...
/*   if ( fRelativeIncrement > 1 )
  {
               modelErr stcErr;
       stcErr.iErrorCode = BAD_DATA;
       strcpy( stcErr.cFunction, "clCompetitionMort::DoMort" );
       strcpy( stcErr.cMoreInfo, "Relative Increment is greater than one" );
       throw( stcErr );
  } */
