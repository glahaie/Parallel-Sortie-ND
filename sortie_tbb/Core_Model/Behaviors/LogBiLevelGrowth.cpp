  ...
  clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clGrowthBase( p_oSimManager )
{
  ...
  try
  {
    ...
    strcpy( m_cNameString, "logbilevelgrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clLogBiLevelGrowth::clLogBiLevelGrowth" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (p_fTempValues[i].val == 0) {
        ...
        strcpy(stcErr.cFunction, "clLogBiLevelGrowth::DoShellSetup");
        strcpy(stcErr.cMoreInfo, "X0 values cannot equal 0.");
        throw(stcErr);
      }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (p_fTempValues[i].val == 0) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clLogBiLevelGrowth::DoShellSetup");
        strcpy(stcErr.cMoreInfo, "Xb values cannot equal 0.");
        throw(stcErr);
      }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (p_fTempValues[i].val < 0) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clLogBiLevelGrowth::DoShellSetup");
        strcpy(stcErr.cMoreInfo, "Max growth values must be greater than 0.");
        throw(stcErr);
      }
