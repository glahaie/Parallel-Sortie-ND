...
clSnagDecomp::clSnagDecomp(clSimManager * p_oSimManager) :
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "SnagDecayClassDynamics" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clSnagDecomp::clSnagDecomp" );
    throw( stcErr );
  }
}
...
void clSnagDecomp::GetData(xercesc::DOMDocument * p_oDoc) {
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clSnagDecomp::GetData" );
      strcpy( stcErr.cMoreInfo, "Max break height must be greater than or equal to min break height, and both values must be non-negative." );
    ...
    for ( j = 0; j <= 5; j++) {
      ...
      for (k = j; k <=5; k++) {
        if (k == 0) {
          ...
          if ( mp_fSnagTransProb[j][k] < 0.0 )
          {
            modelErr stcErr;
            strcpy( stcErr.cFunction, "clSnagDecomp::GetData" );
            strcpy( stcErr.cMoreInfo, "Values for transition probabilites cannot be less than 0." );
            stcErr.iErrorCode = BAD_DATA;
            throw( stcErr );
          } //end if
      ...
      if ( fabs(fCumulativeProb - 1.0) > 0.0001 )
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clSnagDecomp::GetData" );
        strcpy( stcErr.cMoreInfo, "Sum of transition probabilities must add up to 1 within each column." );
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      } //end if
...
void clSnagDecomp::CalcGridValues() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSnagDecomp::CalcGridValues" );
    throw( stcErr );
  }
}
...
void clSnagDecomp::Action() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSnagDecomp::Action" );
    throw( stcErr );
  }
}
  ...
  for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
    ...
    if (mp_whatSpeciesTypeCombos[i].iType == clTreePopulation::adult) {
      ...
      if (mp_iDeadCodes[mp_whatSpeciesTypeCombos[i].iSpecies] [mp_whatSpeciesTypeCombos[i].iType]
        ...
        modelErr stcErr;
        strcpy(stcErr.cFunction, "clSnagDecomp::RegisterTreeDataMembers");
        sprintf(stcErr.cMoreInfo, "%s%d%s%d%s", "Type/species combo species=",
