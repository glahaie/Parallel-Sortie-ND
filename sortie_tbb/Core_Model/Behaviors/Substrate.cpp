...
clSubstrate::clSubstrate( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "Substrate" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clSubstrate::clSubstrate" );
    throw( stcErr );
  }
}
...
void clSubstrate::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSubstrate::GetData" );
    throw( stcErr );
  }
}
...
void clSubstrate::GetParameterFileData( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    if ( fTemp < 0 )
    {
      ...
      strcpy( stcErr.cFunction, "clSubstrate::GetParameterFileData" );
...
void clSubstrate::SetupSubstrateGrids()
{
  try
  {
    ...
    else
    {
      ...
      if ( ( fXCellLength > 0 && mp_oSubstrateGrid->GetLengthXCells() != fXCellLength )
           || ( fYCellLength > 0 && mp_oSubstrateGrid->GetLengthYCells() != fYCellLength ) )
           {
             ...
             modelErr stcErr;
             stcErr.iErrorCode = BAD_DATA;
             strcpy( stcErr.cFunction, "clSubstrate::SetupSubstrateGrid" );
             sprintf( stcErr.cMoreInfo, "%s%s%s", "Grid map values for grid ",
                  "\"Substrate\" contain cell lengths which do not match the allowed ", "values." );
             throw( stcErr );
      }
          ...
          || -1 == m_iTipupCode || -1 == m_iFreshLogCode || -1 == m_iDecLogCode )
           {
             ...
             sprintf( stcErr.cMoreInfo, "%s%s", "Grid map setup values for grid ",
                  "\"Substrate\" did not contain all required grid data members." );
             throw( stcErr );
      }
    ...
    if ( NULL == mp_oCalcGrid )
    {
      ...
      {
        sprintf( cLabel, "%s%d", "freshlog_", i );
        mp_iFLogCalcsCode[i] = mp_oCalcGrid->RegisterFloat( cLabel );
        sprintf( cLabel, "%s%d", "declog_", i );
        mp_iDecLogCalcsCode[i] = mp_oCalcGrid->RegisterFloat( cLabel );
      }
    }
    else
    {
      ...
      if ( (mp_oCalcGrid->GetLengthXCells() != mp_oSubstrateGrid->GetLengthXCells())
           || ( mp_oCalcGrid->GetLengthYCells() != mp_oSubstrateGrid->GetLengthYCells()) )
           {
             ...
             strcpy( stcErr.cFunction, "clSubstrate::SetupSubstrateGrid" );
             sprintf( stcErr.cMoreInfo, "%s%s%s", "Grid map values for grid ",
                  "\"substratecalcs\" contain cell lengths which do not match ",
                  "the allowed values." );
             throw( stcErr );
      }
      ...
      {
        sprintf( cLabel, "%s%d", "freshlog_", i );
        mp_iFLogCalcsCode[i] = mp_oCalcGrid->GetFloatDataCode( cLabel );
        ...
        sprintf( cLabel, "%s%d", "declog_", i );
        mp_iDecLogCalcsCode[i] = mp_oCalcGrid->GetFloatDataCode( cLabel );
...
void clSubstrate::PopulateInitialConditions()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSubstrate::PopulateInitialConditions" );
    throw( stcErr );
  }
}
...
void clSubstrate::GetDeadCodes()
{
  try
  {
    ...
    for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
    {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clSubstrate::GetDeadCodes" );
        strcpy( stcErr.cMoreInfo, "Substrate cannot be applied to seedlings." );
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      }
...
void clSubstrate::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSubstrate::Action" );
    throw( stcErr );
  }
}
...
void clSubstrate::HarvestSubstrate()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSubstrate::HarvestSubstrate" );
    throw( stcErr );
  }
}
...
void clSubstrate::MortalitySubstrate()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSubstrate::MortalitySubstrate" );
    throw( stcErr );
  }
}
...
void clSubstrate::AddNewDeadTrees()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSubstrate::AddNewDeadTrees" );
    throw( stcErr );
  }
}
...
void clSubstrate::AdjustSubstrateForMortality()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSubstrate::AdjustSubstrateForMortality" );
    throw( stcErr );
  }
}
...
void clSubstrate::DecaySubstrate()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSubstrate::DecaySubstrate" );
    throw( stcErr );
  }
}
