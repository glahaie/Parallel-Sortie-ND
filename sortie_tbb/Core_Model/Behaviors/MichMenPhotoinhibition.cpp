  ...
  clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clGrowthBase( p_oSimManager )
{
  try
  {
    strcpy( m_cNameString, "michmenphotogrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clMichMenPhotoinhibition::clMichMenPhotoinhibition" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (p_fTempValues[i].val == 0) {
        ...
        strcpy(stcErr.cFunction, "clMichMenPhotoinhibition::DoShellSetup");
...
float clMichMenPhotoinhibition::CalcHeightGrowthValue(clTree *p_oTree, clTreePopulation *p_oPop, float fDiameterGrowth) {
  ...
  return fAmountHeightIncrease;
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clMichMenPhotoinhibition::CalcHeightGrowthValue");
   throw(stcErr);
 }
}
