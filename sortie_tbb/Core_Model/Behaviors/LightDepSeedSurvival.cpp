     ...
     clLightBase( p_oSimManager), clGLIBase( p_oSimManager )
{
  try
  {
    strcpy( m_cNameString, "LightDependentSeedSurvival" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clLightDepSeedSurvival::clLightDepSeedSurvival" );
    throw( stcErr );
  }
}
...
void clLightDepSeedSurvival::GetParameterFileData( xercesc::DOMDocument * p_oDoc )
{
  ...
  for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
  {
    if ( mp_fOptimumGLI[i] < 0 || mp_fOptimumGLI[i] > 100 )
    {
      ...
      strcpy( stcErr.cFunction, "clLightDepSeedSurvival::GetParameterFileData" );
      strcpy( stcErr.cMoreInfo, "Optimum GLI must be between 0 and 100." );
      throw( stcErr );
    }
  }
  ...
  if ( !m_bUseStormLight )
  {
    ...
    for ( i = 0; i < m_iNumTotalSpecies; i++ )
    {
      for ( j = 0; j < 3; j++ )
      {
        if ( mp_fLightExtCoeff[i] [j] < 0 || mp_fLightExtCoeff[i] [j] > 1 )
        {
          ...
          stcErr.iErrorCode = BAD_DATA;
          strcpy( stcErr.cFunction, "clLightDepSeedSurvival::GetParameterFileData" );
          strcpy( stcErr.cMoreInfo, "All light extinction coefficients must be between 0 and 1." );
          throw( stcErr );
        }
      }
    }
    ...
    if ( m_fLightHeight < 0 )
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clLightDepSeedSurvival::GetParameterFileData" );
      strcpy( stcErr.cMoreInfo, "Height of light calculations must be greater than 0." );
      throw( stcErr );
    }
    ...
    if ( m_iNumAltAng < 1 || m_iNumAziAng < 1 )
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clLightDepSeedSurvival::GetParameterFileData" );
      strcpy( stcErr.cMoreInfo, "Number of altitude and azimuth sky divisions must be at least 1." );
      throw( stcErr );
...
float clLightDepSeedSurvival::GetGLI( clTreePopulation * p_oPop, const float & fX, const float & fY )
{
  ...
  //TBB FIX - just in case
  //Initialize photo array
  float** mp_ffPhoto = createMPPhotoMatrix(m_iNumAltAng, m_iNumAziAng);
//   for ( i = 0; i < m_iNumAltAng; i++ )
//     for ( j = 0; j < m_iNumAziAng; j++ )
//       mp_fPhoto[i] [j] = 1.0;
  ...
  while ( p_oNeighbor != NULL )
  {
    ...
    if ( clTreePopulation::seedling != p_oNeighbor->GetType() )
      ...
      AddTreeToGliFishEye( fX, fY, m_fLightHeight, p_oNeighbor, p_oPlot, p_oPop, p_oAllom, mp_ffPhoto );
  ...
  for ( i = 0; i < m_iNumAltAng; i++ )
    for ( j = 0; j < m_iNumAziAng; j++ )
      fGli += ( mp_ffPhoto[i] [j] * mp_fBrightness[i] [j] );
  fGli *= 100;

  deleteMPPhotoMatrix(mp_ffPhoto, m_iNumAltAng);
  return fGli;
}
...
void clLightDepSeedSurvival::SetupGrid()
{
  ...
  if ( NULL == mp_oSeedGrid )
  {
    ...
    strcpy( stcErr.cFunction, "clLightDepSeedSurvival::SetupGrids" );
    strcpy( stcErr.cMoreInfo, "Disperse behaviors are required if establishment is to be used." );
    throw( stcErr );
  }
  ...
  for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
  {
    ...
    if ( -1 == mp_iSeedGridCode[i] )
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clLightDepSeedSurvival::SetupGrids" );
      strcpy( stcErr.cMoreInfo, "Unexpected grid data return code for Dispersed Seeds." );
      throw( stcErr );
    }
  }
  ...
  if ( m_bUseStormLight )
  {
    ...
    if ( NULL == mp_oStormLightGrid )
    {
      ...
      stcErr.iErrorCode = CANT_FIND_OBJECT;
      strcpy( stcErr.cFunction, "clLightDepSeedSurvival::SetupGrids" );
      strcpy( stcErr.cMoreInfo, "The Storm Light behavior is required for Storm Light Seed Dependent Survival." );
      throw( stcErr );
    }
    ...
    if ( -1 == m_iLightCode )
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clLightDepSeedSurvival::SetupGrids" );
      strcpy( stcErr.cMoreInfo, "Unexpected grid data return code for Storm Light." );
      throw( stcErr );
    }
  }
}
...
void clLightDepSeedSurvival::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clLightDepSeedSurvival::Action" );
    throw( stcErr );
  }
}
...
// SetNameData()
//////////////////////////////////////////////////////////////////////////////
void clLightDepSeedSurvival::SetNameData( char * cNameString )
{
  try
  {

    //Check the string passed and set the flags accordingly
    if ( strcmp( "LightDependentSeedSurvival", cNameString ) == 0 )
    {
      m_bUseStormLight = false;
    }
    else if ( strcmp( "StormLightDependentSeedSurvival", cNameString ) == 0 )
    {
    ...
    else
    {
      ...
      strcpy( stcErr.cFunction, "clLightDepSeedSurvival::SetNameData" );
      throw( stcErr );
    }
  }
  catch ( modelErr & err )
