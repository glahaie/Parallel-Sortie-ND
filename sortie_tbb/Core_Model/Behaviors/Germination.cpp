...
clGermination::clGermination(clSimManager * p_oSimManager) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  try
  {
    strcpy(m_cNameString, "Germination");
  ...
  catch (...)
  {
    ...
    strcpy(stcErr.cFunction, "clGermination::clGermination");
    throw(stcErr);
  }
}
  ...
  try
  {
    ...
    for (i = 0; i < m_iNumBehaviorSpecies; i++)
    {
      if (mp_fProportionGerminating[mp_iWhatSpecies[i]] < 0 || mp_fProportionGerminating[mp_iWhatSpecies[i]] > 1)
      {
        ...
        strcpy(stcErr.cMoreInfo, "All values in proportion germinating must be between 0 and 1.");
    ...
    if (mp_oSeedGrid == NULL)
    {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "clGermination::GetData");
      strcpy(stcErr.cMoreInfo, "Disperse behaviors must be used with germination.");
    ...
    for (i = 0; i < m_iNumBehaviorSpecies; i++)
    {
      ...
      if (-1 == mp_iSeedGridCode[mp_iWhatSpecies[i]])
      {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clGermination::SetupGrids");
