...
clLaggedPostHarvestGrowth::clLaggedPostHarvestGrowth( clSimManager * p_oSimManager ) :
  ...
  {
    //Set namestring
    strcpy( m_cNameString, "laggedpostharvestgrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clLaggedPostHarvestGrowth::clLaggedPostHarvestGrowth" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
          ...
          && clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType )
      {
        ...
        strcpy( stcErr.cFunction, "clLaggedPostHarvestGrowth::ReadParameterFile" );
...
void clLaggedPostHarvestGrowth::ValidateData()
{
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "All values for maximum growth constant must be non-negative." );
        throw( stcErr );
      }
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "All values for maximum growth dbh effect must be non-negative." );
        throw( stcErr );
      }
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "All values for NCI constant must be non-negative." );
        throw( stcErr );
      }
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "All values for NCI dbh effect must be non-negative." );
        throw( stcErr );
      }
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "Time since harvest rate parameter must be greater than 0." );
        throw( stcErr );
      }
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cMoreInfo, "The NCI distance radius must be greater than 0." );
      throw( stcErr );
    }
  ...
  {
    strcpy( err.cFunction, "clLaggedPostHarvestGrowth::ValidateData" );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clLaggedPostHarvestGrowth::ValidateData" );
    throw( stcErr );
  }
}
...
void clLaggedPostHarvestGrowth::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clLaggedPostHarvestGrowth::DoShellSetup" );
    throw( stcErr );
  }
}
...
// SetNameData()
//////////////////////////////////////////////////////////////////////////////
void clLaggedPostHarvestGrowth::SetNameData( char * cNameString )
{
  try
  {

    //Check the string passed and set the flags accordingly
    if ( strcmp( "LaggedPostHarvestGrowth", cNameString ) == 0 )
    {
      m_iGrowthMethod = diameter_auto;
    }
    else if ( strcmp( "LaggedPostHarvestGrowth diam only", cNameString ) == 0 )
    {
    ...
    else
    {
      ...
      strcpy( stcErr.cFunction, "clLaggedPostHarvestGrowth::SetNameData" );
      throw( stcErr );
    }
  }
  catch ( modelErr & err )
...
float clLaggedPostHarvestGrowth::CalcDiameterGrowthValue( clTree * p_oTree, clTreePopulation * p_oPop, float fHeightGrowth )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clLaggedPostHarvestGrowth::CalcDiameterGrowthValue" );
    throw( stcErr );
  }
}
...
void clLaggedPostHarvestGrowth::PreGrowthCalcs( clTreePopulation * p_oPop )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clLaggedPostHarvestGrowth::PreCalcGrowth" );
    throw( stcErr );
  }
}
...
float clLaggedPostHarvestGrowth::LocalBasalAreaAroundTree( clTree *p_oTree ) {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clLaggedPostHarvestGrowth::LocalBasalAreaAroundTree");
    throw(stcErr);
  }
}
...
void clLaggedPostHarvestGrowth::CalcTimeSinceHarvest() {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clLaggedPostHarvestGrowth::CalcTimeSinceHarvest");
    throw(stcErr);
  }
}
...
void clLaggedPostHarvestGrowth::SetupTimeSinceHarvestGrid() {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clLaggedPostHarvestGrowth::SetupTimeSinceHarvestGrid");
    throw(stcErr);
  }
}
...
int clLaggedPostHarvestGrowth::GetTimeSinceHarvest( clTree *p_oTree ) {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clLaggedPostHarvestGrowth::GetTimeSinceHarvest");
    throw(stcErr);
  }
}
...
void clLaggedPostHarvestGrowth::RegisterTreeDataMembers()
{
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
        ...
        clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType)
    {
      ...
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clSnagDecomp::RegisterTreeDataMembers" );
      strcpy( stcErr.cMoreInfo, "This behavior can only be applied to saplings and adults." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
