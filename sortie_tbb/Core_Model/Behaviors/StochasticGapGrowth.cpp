...
clStochasticGapGrowth::clStochasticGapGrowth(clSimManager * p_oSimManager) :
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "StochasticGapGrowth" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clStochasticGapGrowth::clStochasticGapGrowth" );
    throw( stcErr );
  }
}
...
void clStochasticGapGrowth::GetData(xercesc::DOMDocument * p_oDoc) {
  ...
  try
  {
    ...
    if ( !mp_oGapLight )
    {
      ...
      strcpy( stcErr.cFunction, "clStochasticGapGrowth::GetData" );
      strcpy( stcErr.cMoreInfo, "You must use the \"Gap Light\" behavior with \"Stochastic Gap Growth\"." );
      throw( stcErr );
    }
    ...
    if ( -1 == m_iIsGapCode )
    {
      ...
      strcpy( stcErr.cFunction, "clGapLight::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "Gap Light grid was incorrectly set up.  Missing bool \"Is Gap\"." );
      throw( stcErr );
    }
...
void clStochasticGapGrowth::Action() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clStochasticGapGrowth::Action" );
    throw( stcErr );
  }
}
