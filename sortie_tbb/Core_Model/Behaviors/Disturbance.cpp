...
clDisturbance::clDisturbance(clSimManager * p_oSimManager) :
  ...
  try {
    ...
    strcpy(stcErr.cFunction, "clDisturbance::clDisturbance");
    throw(stcErr);
  }
}
...
void clDisturbance::SetNameData(char * cNameString) {
  try {
    ...
    if (strcmp("Harvest", cNameString) == 0) {
      m_iReasonCode = harvest;
      m_bIsHarvest = true;
      m_sXMLRoot = "Harvest";
    } else if (strcmp("EpisodicMortality", cNameString) == 0) {
      ...
      sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized namestring \"",
          cNameString, "\".");
      strcpy(stcErr.cFunction, "clDisturbance::SetNameData");
      throw(stcErr);
    }
...
void clDisturbance::ReadHarvestParameterFileData(xercesc::DOMDocument * p_oDoc) {
  ...
  *p_fRanges; //for sorting cut ranges
  ...
  char *cData, cTemp[100]; //for extracting text data

  //See if there's any harvest data in the parameter file
  sprintf(cTemp, "%s%d", "Harvest", m_iBehaviorListNumber);
  sVal = XMLString::transcode(cTemp);
  ...
  for (i = 0; i < iNumHarvestEvents; i++) {
    ...
    if (iTimestep <= 0 || iTimestep > iNumberTimesteps) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clDisturbance::ReadHarvestParameterFileData");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid harvest cut timestep ",
          iTimestep);
      throw(stcErr);
    }
    ...
    for (j = 0; j < iNumChildren; j++) {
      ...
      if (-1 == iTemp) { //throw an error
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clDisturbance::ReadHarvestParameterFileData");
        strcpy(stcErr.cMoreInfo, "Unrecognized species in harvest.");
        throw(stcErr);
      }
    ...
    //Get the cut type
    FillSingleValue(p_oElement, "ha_cutType", cTemp, 100, true);
    //See if it's a value we recognize
    if (strcmp(cTemp, "gap") == 0)
      iCutType = gap;
    else if (strcmp(cTemp, "partial") == 0)
      iCutType = partial;
    else if (strcmp(cTemp, "clear") == 0)
      iCutType = clear;
    else {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clDisturbance::ReadHarvestParameterFileData");
      strcpy(stcErr.cMoreInfo, "Unrecognized value in ha_cutType.");
      throw(stcErr);
    }
    ...
    if (partial == iCutType) {
      FillSingleValue(p_oElement, "ha_cutAmountType", cTemp, 100, true);
      //See if it's a value we recognize
      if (strcmp(cTemp, "percent of basal area") == 0)
        p_oNewPackage->SetValue(m_iAmountTypeCode, percentBA);
      else if (strcmp(cTemp, "percent of density") == 0)
        p_oNewPackage->SetValue(m_iAmountTypeCode, percentDen);
      else if (strcmp(cTemp, "absolute basal area") == 0)
        p_oNewPackage->SetValue(m_iAmountTypeCode, absBA);
      else if (strcmp(cTemp, "absolute density") == 0)
        p_oNewPackage->SetValue(m_iAmountTypeCode, absDen);
      else {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clDisturbance::ReadHarvestParameterFileData");
        strcpy(stcErr.cMoreInfo, "Unrecognized value in ha_cutAmountType.");
        throw(stcErr);
      }
    } else {
    ...
    if (iNumChildren > m_iNumAllowedCutRanges) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clDisturbance::ReadHarvestParameterFileData");
      strcpy(stcErr.cMoreInfo, "Too many dbh cut ranges.");
      throw(stcErr);
    }
    ...
    if (partial == iCutType && 0 == iNumChildren) {
      ...
      strcpy(stcErr.cFunction, "clDisturbance::ReadHarvestParameterFileData");
      strcpy(stcErr.cMoreInfo, "ha_dbhRange");
      throw(stcErr);
    }
    ...
    for (j = 0; j < iNumChildren; j++) {
      ...
      if (fTemp < 0) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clDisturbance::ReadHarvestParameterFileData");
        strcpy(stcErr.cMoreInfo, "Harvest cut range value cannot be negative.");
        throw(stcErr);
      }
    }
    ...
    if (iNumChildren > m_iNumAllowedPriorities) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clDisturbance::ReadHarvestParameterFileData");
      strcpy(stcErr.cMoreInfo, "Too many cut priorities.");
      throw(stcErr);
    }
    ...
    if (iNumChildren > 0 && iCutType == percentDen) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clDisturbance::ReadHarvestParameterFileData");
      strcpy(stcErr.cMoreInfo, "Cut priorities cannot be used with percent density cuts.");
      throw(stcErr);
    }
    for (j = 0; j < iNumChildren; j++) {
      p_oNode = p_oCutSpecificsList->item(j);
      p_oChildElement = (DOMElement *) p_oNode;
      FillSingleValue(p_oChildElement, "ha_name", cTemp,
          MAX_TREE_LABEL_SIZE, true);
      if (strlen(cTemp) == 0) {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clDisturbance::ReadHarvestParameterFileData");
        strcpy(stcErr.cMoreInfo, "Priority name is missing.");
        throw(stcErr);
      }
      p_oNewPackage->SetValue(mp_iPriorityNameCodes[j], cTemp);

      FillSingleValue(p_oChildElement, "ha_type", cTemp, 100, true);
      //See if it's a value we recognize
      if (strcmp(cTemp, "float") == 0)
        iTemp = floatType;
      else if (strcmp(cTemp, "int") == 0)
        iTemp = intType;
      else if (strcmp(cTemp, "bool") == 0)
        iTemp = boolType;
      else {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clDisturbance::ReadHarvestParameterFileData");
        strcpy(stcErr.cMoreInfo, "Unrecognized value in ha_type.");
        throw(stcErr);
      }
...
void clDisturbance::ReadMortEpParameterFileData(xercesc::DOMDocument * p_oDoc) {
  ...
  *p_fRanges; //for sorting cut ranges
  ...
  iNumberTimesteps = mp_oSimManager->GetNumberOfTimesteps(), iNumSpecies =
      ...
      iTemp; //for extracting integer data from parameter file
  short int i, j, k; //loop counters
  char cTemp[100], //for extracting text data
  *cData;

  //See if there's any mortality episode data in the parameter file
  sprintf(cTemp, "%s%d", "EpisodicMortality", m_iBehaviorListNumber);
  sVal = XMLString::transcode(cTemp);
  p_oCutEventsList = p_oDoc->getElementsByTagName(sVal);
  XMLString::release(&sVal);
  if (0 == p_oCutEventsList->getLength())
  ...
  for (i = 0; i < iNumDisturbanceEvents; i++) {
    ...
    if (iTimestep <= 0 || iTimestep > iNumberTimesteps) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clDisturbance::ReadMortEpParameterFileData");
      sprintf(stcErr.cMoreInfo, "%s%d",
          "Invalid mortality episode cut timestep ", iTimestep);
      throw(stcErr);
    }
    ...
    for (j = 0; j < iNumChildren; j++) {
      ...
      if (-1 == iTemp) { //throw an error
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clDisturbance::ReadMortEpParameterFileData");
        strcpy(stcErr.cMoreInfo, "Unrecognized species in mortality episode.");
        throw(stcErr);
      }
      p_oNewPackage->SetValue(mp_iSpeciesCodes[iTemp], true);
    }
    ...
    //Get the amount type
    FillSingleValue(p_oElement, "ds_cutAmountType", cTemp, 100, true);
    //See if it's a value we recognize
    if (strcmp(cTemp, "percent of basal area") == 0)
      p_oNewPackage->SetValue(m_iAmountTypeCode, percentBA);
    else if (strcmp(cTemp, "percent of density") == 0)
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clDisturbance::ReadMortEpParameterFileData");
      strcpy(stcErr.cMoreInfo, "Unrecognized value in ds_cutAmountType.");
      throw(stcErr);
    }
    ...
    if (0 == iNumChildren) {
      ...
      stcErr.iErrorCode = DATA_MISSING;
      strcpy(stcErr.cFunction, "clDisturbance::ReadMortEpParameterFileData");
      strcpy(stcErr.cMoreInfo, "ds_dbhRange");
      throw(stcErr);
    }
...
void clDisturbance::GetData(xercesc::DOMDocument * p_oDoc) {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clDisturbance::GetData");
    throw(stcErr);
  }
}
      ...
      for (k = 0; k < m_iNumAllowedCutRanges; k++) {
        ...
        if (fLowDbh != 0 && fHiDbh != 0 && fAmount != 0) {
          ...
          if (partial != iCutType && k > 0) {
            modelErr stcErr;
            stcErr.iErrorCode = BAD_DATA;
            strcpy(stcErr.cFunction, "clDisturbance::ValidatePackages");
            strcpy(stcErr.cMoreInfo,
          ...
          if (fHiDbh <= fLowDbh) {
            ...
            stcErr.iErrorCode = BAD_DATA;
            strcpy(stcErr.cFunction, "clDisturbance::ValidatePackages");
            sprintf(stcErr.cMoreInfo, "%s%s", "The low dbh range value in a",
                " harvest cut must be smaller than the high.");
            throw(stcErr);
          }
          ...
          if (fPrevHiDbh > fLowDbh) {
            ...
            stcErr.iErrorCode = BAD_DATA;
            strcpy(stcErr.cFunction, "clDisturbance::ValidatePackages");
            strcpy(stcErr.cMoreInfo, "Harvest cut ranges may not overlap.");
            throw(stcErr);
          }
...
void clDisturbance::ResetResultsGrid() {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clDisturbance::ResetResultsGrid");
    throw(stcErr);
  }
}
  ...
  try {
    ...
    if (m_bIsHarvest) {
      ...
      for (i = 0; i < m_iNumAllowedCutRanges; i++) {
        ...
        mp_iRangeAmountCodes[i] = mp_oMasterCutsGrid->RegisterPackageFloat(
            cFloatLabel);
      }
      ...
      for (i = 0; i < iNumSpecies; i++) {
        ...
        mp_iSpeciesCodes[i] = mp_oMasterCutsGrid->RegisterPackageBool(
            cFloatLabel);
        sprintf(cFloatLabel, "%s%d", "seedling", i);
        mp_iSeedlingCodes[i] = mp_oMasterCutsGrid->RegisterPackageFloat(
            cFloatLabel);
      }

      for (i = 0; i < m_iNumAllowedPriorities; i++) {
        sprintf(cFloatLabel, "priorityname%d", i);
        mp_iPriorityNameCodes[i] = mp_oMasterCutsGrid->RegisterPackageChar(
            cFloatLabel);
        sprintf(cFloatLabel, "prioritytype%d", i);
        mp_iPriorityTypeCodes[i] = mp_oMasterCutsGrid->RegisterPackageInt(
            cFloatLabel);
        sprintf(cFloatLabel, "prioritymin%d", i);
        mp_iPriorityMinCodes[i] = mp_oMasterCutsGrid->RegisterPackageFloat(
            cFloatLabel);
        sprintf(cFloatLabel, "prioritymax%d", i);
        mp_iPriorityMaxCodes[i] = mp_oMasterCutsGrid->RegisterPackageFloat(
            cFloatLabel);
      }
      ...
      for (i = 0; i < iNumSpecies; i++) {
        sprintf(cFloatLabel, "%s%d", "Cut Seedlings_", i);
        mp_iSeedlingCutCodes[i] = mp_oResultsGrid->RegisterInt(cFloatLabel);
      }
    } else {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clDisturbance::SetupGrids");
    throw(stcErr);
  }
}
...
void clDisturbance::Action() {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clDisturbance::Action");
    throw(stcErr);
  }
}
...
void clDisturbance::CutTrees(clPackage * p_oMasterPackage) {
      ...
      if (p_bSpeciesCut[iSp]) {
        ...
        for (j = 0; j < m_iNumAllowedPriorities; j++) {
          if (mp_iPriorityNameCodes[j] > -1) {
            p_oMasterPackage->GetValue(mp_iPriorityNameCodes[j], cPriorityName);
            if (strlen(cPriorityName) > 0) {
              ...
              if (percentBA == iAmountType)
                CutSpeciesPercentBAPriority(iSp, p_cutArea, p_fLoDbh, p_fHiDbh,
                    ...
                    cPriorityName, iPriorityType, fPriorityMin, fPriorityMax);
              else if (absDen == iAmountType)
                CutSpeciesAbsDenPriority(iSp, p_cutArea, p_fLoDbh, p_fHiDbh,
                    p_fAmountToRemove, p_fAmountRemoved, cPriorityName,
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clDisturbance::CutTrees");
    throw(stcErr);
  }
}
...
void clDisturbance::CutSpeciesPercentBAPriority(int iSp, stcGridList * & p_cutArea,
    float *p_fLoDbh, float *p_fHiDbh, float *p_fAmountToRemove,
    float *p_fAmountRemoved, float *p_fTotalBasalArea, char *cPriorityName,
    ...
    for (i = 0; i < m_iNumAllowedCutRanges; i++) {
      if (fDbh >= p_fLoDbh[i] && fDbh <= p_fHiDbh[i]) {
        ...
        if (floatType == iPriorityType) {
          iCode = mp_oPop->GetFloatDataCode(cPriorityName, iSp, p_oTree->GetType());
          if (iCode > -1) {
          ...
          iCode = mp_oPop->GetIntDataCode(cPriorityName, iSp, p_oTree->GetType());
          if (iCode > -1) {
          ...
          iCode = mp_oPop->GetBoolDataCode(cPriorityName, iSp, p_oTree->GetType());
          if (iCode > -1) {
...
void clDisturbance::CutSpeciesAbsBAPriority(int iSp, stcGridList * & p_cutArea,
    float *p_fLoDbh, float *p_fHiDbh, float *p_fAmountToRemove,
    float *p_fAmountRemoved, char *cPriorityName, int &iPriorityType,
