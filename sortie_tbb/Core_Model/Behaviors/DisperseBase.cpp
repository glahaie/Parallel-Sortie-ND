...
clDisperseBase::clDisperseBase(clSimManager *p_oSimManager) :
  ...
  try {
    ...
    strcpy(stcErr.cFunction, "clDisperseBase::clDisperseBase");
    throw(stcErr);
  }
}
...
void clDisperseBase::GetData(xercesc::DOMDocument *p_oDoc) {
 try {
   ...
   DoShellSetup(p_oDoc);
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clGrowthBase::GetData");
    throw(stcErr);
  }
}
...
void clDisperseBase::TimestepCleanup()
{
  ...
  catch (...)
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clDisperseBase::TimestepCleanup");
    throw(stcErr);
  }
}
...
void clDisperseBase::SetUpBase()
{
  try
  {
    ...
    char cLabel[MAX_GRID_LABEL_SIZE], cGridName[] = "Dispersed Seeds";
    ...
    mp_oSeedGrid = mp_oSimManager->GetGridObject(cGridName);
    ...
    if (mp_oSeedGrid != NULL)
    {
      ...
      for (i = 0; i < m_iTotalSpecies; i++)
      {
        ...
        mp_iNumSeedsCode[i] = mp_oSeedGrid->GetFloatDataCode(cLabel);
    ...
    else
    {
      ...
      mp_oSeedGrid = mp_oSimManager->CreateGrid(cGridName, //grid name
      ...
      {
        sprintf(cLabel, "%s%d", "seeds_", i);
        mp_iNumSeedsCode[i] = mp_oSeedGrid->RegisterFloat(cLabel);
      }
...
void clDisperseBase::Action() {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clDisperseBase::Action");
    throw(stcErr);
  }
}
