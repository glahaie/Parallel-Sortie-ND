...
clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clMortalityBase( p_oSimManager )
{
  try
  {
    ...
    strcpy( m_cNameString, "expresourcemortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clExpResourceMortality::clExpResourceMortality" );
    throw( stcErr );
  }
}
...
void clExpResourceMortality::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clExpResourceMortality::DoShellSetup" );
    throw( stcErr );
  }
}
...
void clExpResourceMortality::GetResourceGrid()
{
  ...
  {
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clExpResourceMortality::GetResourceGrid" );
    strcpy( stcErr.cMoreInfo, "Can't find required grid object \"Resources\"." );
  ...
  if ( -1 == m_iResourceCode )
  {
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clExpResourceMortality::GetResourceGrid" );
    strcpy( stcErr.cMoreInfo, "Grid object \"Resources\" is set up incorrectly." );
    stcErr.iErrorCode = CANT_FIND_OBJECT;
    throw( stcErr );
  }
}
    ...
    for ( i = 0; i < m_iNumTotalSpecies; i++ )
      for ( j = 0; j < iTotalTypes; j++ )
        if ( mp_bUsesThisMortality[i] [j] )
        {
          ...
          {
            modelErr stcErr;
            strcpy( stcErr.cFunction, "clExpResourceMortality::GetTreeDataMemberCodes" );
            sprintf( stcErr.cMoreInfo, "%s%d%s%d%s%s", "Type/species combo species=", i, " type=", j,
                 " does not have a growth behavior compatible with",
...
deadCode clExpResourceMortality::DoMort(clTree * p_oTree, const float & fDbh, const short int & iSpecies )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clExpResourceMortality::DoMort" );
    throw( stcErr );
  }
}
