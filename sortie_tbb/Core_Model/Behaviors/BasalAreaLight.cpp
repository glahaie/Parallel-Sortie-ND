  ...
  clBehaviorBase( p_oSimManager ), clLightBase(p_oSimManager)
{
  ...
  //Set the namestring
  strcpy( m_cNameString, "basalarealightshell" );
...
void clBasalAreaLight::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  ...
  try
  {
    ...
    if (0 == m_fConiferC) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clBasalAreaLight::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "Basal area light conifer \"c\" parameter cannot equal 0." );
    ...
    if (0 == m_fAngiospermC) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clBasalAreaLight::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "Basal area light angiosperm \"c\" parameter cannot equal 0." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
    ...
    if (m_fChangeThreshold < 0) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clBasalAreaLight::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "Basal area light change threshold cannot be less than 0." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
    ...
    if (m_fMinDbh < 0) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clBasalAreaLight::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "Basal area light minimum DBH cannot be less than 0." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
    ...
    if (m_fRadius < 0) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clBasalAreaLight::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "Basal area light radius cannot be less than 0." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
    ...
    for (i = 0; i < iNumSpecies; i++) {
      if (angiosperm == p_iTemp[i]) mp_iSpeciesTypes[i] = angiosperm;
      	...
      	modelErr stcErr;
        strcpy( stcErr.cFunction, "clBasalAreaLight::DoShellSetup" );
        sprintf( stcErr.cMoreInfo, "%s%d", "Unrecognized basal area light tree type: ", p_iTemp[i] );
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      }
    }
...
void clBasalAreaLight::UpdateGridValues(clTreePopulation * p_oPop)
{
  try
  {
    ...
    for ( iX = 0; iX < iNumXCells; iX++ )
    {
      for ( iY = 0; iY < iNumYCells; iY++ )
      {
        ...
        sprintf( cQuery, "%s%f%s%f%s%f%s%f", "distance=", m_fRadius, "FROM x=",
           fX, "y=", fY, "::height=", 0.0 );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clBasalAreaLight::Action" );
    throw( stcErr );
  }
}
