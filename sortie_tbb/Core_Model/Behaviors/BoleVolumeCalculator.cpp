...
clBoleVolumeCalculator::clBoleVolumeCalculator( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "TreeBoleVolumeCalculator" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clBoleVolumeCalculator::clBoleVolumeCalculator" );
    throw( stcErr );
  }
}
  ...
  try {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (mp_fFormClass[i] < 60 || mp_fFormClass[i] > 100) {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clBoleVolumeCalculator::GetParameterFileData" );
...
void clBoleVolumeCalculator::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clBoleVolumeCalculator::GetData" );
    throw( stcErr );
  }
}
...
void clBoleVolumeCalculator::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clBoleVolumeCalculator::Action" );
    throw( stcErr );
  }
}
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    ...
    if ( clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType
         ...
         && clTreePopulation::snag != mp_whatSpeciesTypeCombos[i].iType )
         {
           ...
           modelErr stcErr;
           strcpy( stcErr.cFunction, "clBoleVolumeCalculator::RegisterTreeDataMembers" );
