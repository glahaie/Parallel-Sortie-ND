...
clDensitySelfThinningGompertz::clDensitySelfThinningGompertz(clSimManager *p_oSimManager) :
 ...
 try {
   ...
   strcpy(m_cNameString, "densitygompertzmortshell");
   ...
   strcpy(stcErr.cFunction, "clDensitySelfThinningGompertz::clDensitySelfThinningGompertz");
   throw(stcErr);
 }
}
  ...
  try {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      if ( mp_fMinHeight[mp_iIndexes[p_fTempValues[i].code]] < 0 )
      {
        ...
        strcpy( stcErr.cFunction, "clDensitySelfThinningGompertz::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "The Gompertz Density Self Thinning parameter minimum height must be a positive number." );
        throw( stcErr );
      }
    }
    //G
    ...
    if ( m_fRadius <= 0 )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDensitySelfThinningGompertz::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "The Gompertz Density Self Thinning parameter search radius must be a positive number." );
        throw( stcErr );
      }
...
float clDensitySelfThinningGompertz::CalculateNeighborhoodTreeCount( clTree * p_oTree)
{
  try
  {
    ...
    sprintf( cQuery, "%s%f%s%f%s%f%s%f", "distance=", m_fRadius, "FROM x=", fTargetX,
         "y=", fTargetY, "::height=", mp_fMinHeight[mp_iIndexes[iTargetSpecies]] );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDensitySelfThinningGompertz::CalculateNeighborhoodTreeCountAndMeanDiam10" );
    throw( stcErr );
  }
}
...
deadCode clDensitySelfThinningGompertz::DoMort(clTree *p_oTree, const float &fDbh, const short int &iSpecies) {
 try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clDensitySelfThinningGompertz::DoMort");
   throw(stcErr);
 }
}
