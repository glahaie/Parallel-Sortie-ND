  ...
  clMortalityBase( p_oSimManager )
{
  ...
  {
    //Set namestring
    strcpy( m_cNameString, "TempDependentNeighborhoodmortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clTempDependentNeighborhoodSurvival::clTempDependentNeighborhoodSurvival" );
    throw( stcErr );
  }
}
...
void clTempDependentNeighborhoodSurvival::ReadParameterFile( xercesc::DOMDocument * p_oDoc, clTreePopulation *p_oPop )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTempDependentNeighborhoodSurvival::ReadParameterFile" );
    throw( stcErr );
  }
}
...
void clTempDependentNeighborhoodSurvival::ValidateData()
{
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "N values cannot be 0." );
        throw( stcErr );
      }
    }
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cMoreInfo, "Neighbor search radius cannot be less than 0." );
      throw( stcErr );
    }
  }
  catch ( modelErr & err )
  {
    strcpy( err.cFunction, "clTempDependentNeighborhoodSurvival::ValidateData" );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTempDependentNeighborhoodSurvival::ValidateData" );
    throw( stcErr );
  }
}
...
void clTempDependentNeighborhoodSurvival::SetupGrid(clTreePopulation *p_oPop)
{
  ...
  if (NULL == mp_oGrid) {
    ...
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "survival_", i);
      mp_iGridSurvivalCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    ...
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "survival_", i);
      mp_iGridSurvivalCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
      if (-1 == mp_iGridSurvivalCodes[i]) {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clTempDependentNeighborhoodSurvival::SetupGrid" );
