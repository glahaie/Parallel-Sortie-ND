...
clStormDamageApplier::clStormDamageApplier( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set namestring
    strcpy( m_cNameString, "StormDamageApplier" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clStormDamageApplier::clStormDamageApplier" );
    throw( stcErr );
  }
}
...
void clStormDamageApplier::RegisterTreeDataMembers()
{
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    if ( clTreePopulation::seedling == mp_whatSpeciesTypeCombos[i].iType )
    {
      ...
      strcpy( stcErr.cFunction, "clStormDamageApplier::RegisterTreeDataMembers" );
      strcpy( stcErr.cMoreInfo, "The storm damage applier behavior cannot be applied to seedlings." );
      throw( stcErr );
    }
...
void clStormDamageApplier::GetParameterFileData( xercesc::DOMDocument * p_oDoc )
{
  ...
  if ( m_iNumYearsToHeal <= 0 )
  {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clStormDamageApplier::GetParameterFileData" );
    strcpy( stcErr.cMoreInfo, "The number of years to heal parameter must be greater than 0." );
...
void clStormDamageApplier::GetData( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    if ( NULL == mp_oStormDamageGrid )
    {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clStormDamageApplier::GetData" );
      strcpy( stcErr.cMoreInfo, "The storm damage applier behavior must be used with the storm behavior." );
      throw( stcErr );
    }
    ...
    if ( -1 == m_iDmgIndexCode )
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clStormDamageApplier::GetData" );
      strcpy( stcErr.cMoreInfo, "The grid \"Storm Damage\" is set up in an unexpected way." );
      throw( stcErr );
    }
  }
  catch ( modelErr & err )
...
void clStormDamageApplier::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clStormDamageApplier::Action" );
    throw( stcErr );
  }
}
