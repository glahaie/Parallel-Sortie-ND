...
clAllometricGrowthIncrementer::clAllometricGrowthIncrementer(
  ...
  try
  {
    strcpy( m_cNameString, "allometric incrementer growthshell" );
    m_bGoLast = true;
  }
  catch ( modelErr & err )
  {
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clAllometricGrowthIncrementer::clAllometricGrowthIncrementer" );
    throw( stcErr );
  }
}
...
// SetNameData()
//////////////////////////////////////////////////////////////////////////////
void clAllometricGrowthIncrementer::SetNameData(char *cNameString) {

  //Check the string passed and set the flags accordingly
  if (strcmp("HeightIncrementer", cNameString) == 0) {
    m_iGrowthMethod = height_only;
  } else if (strcmp("DiameterIncrementer", cNameString) == 0) {
    ...
    sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized behavior name \"",
        cNameString, "\".");
    strcpy(stcErr.cFunction, "clAllometricGrowthIncrementer::SetNameData");
    throw(stcErr);
  }
}
