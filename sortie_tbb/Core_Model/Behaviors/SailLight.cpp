...
clSailLight::clSailLight(clSimManager * p_oSimManager) :
  ...
  //Set the namestring
  strcpy(m_cNameString, "saillightshell");
}
...
void clSailLight::DoShellSetup(xercesc::DOMDocument * p_oDoc) {
  try
  {
    ...
    else
    {
      ...
      strcpy( stcErr.cFunction, "clSailLight::DoShellSetup" );
      sprintf( stcErr.cMoreInfo, "%s%d%s", "li_crownFracOfHeight has invalid value \"", iTemp, "\"." );
      throw( stcErr );
    }
    ...
    if ( iVal != clLightOrg::mid && iVal != clLightOrg::top )
    {
      ...
      sprintf( stcErr.cMoreInfo, "%s%d%s", "li_heightOfFishEyePhoto has invalid value \"", iVal, "\"." );
      throw( stcErr );
    }
...
//     mp_fPhoto[i] [j] = 1.0;
    ...
    while ( p_oNeighbor != NULL )
    {
      ...
      if ( clTreePopulation::seedling != p_oNeighbor->GetType() && p_oNeighbor != p_oTree )
      {
        ...
        AddTreeToSailFishEye( fTargetX, fTargetY, fTreeHeight, p_oNeighbor, p_oPlot, p_oPop, p_oAllom, mp_ffPhoto );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSailLight::CalcLightValue" );
    throw( stcErr );
  }
}
...
void clSailLight::AddTreeToSailFishEye(const float & fTargetX,
  ...
  AddTreeToSailFishEye(fTargetX, fTargetY, fTargetHeight, p_oNeighbor, p_oPlot, p_oPop, p_oAllom, mp_fPhoto);
}

void clSailLight::AddTreeToSailFishEye(const float & fTargetX,
    const float & fTargetY, const float & fTargetHeight, clTree * p_oNeighbor,
    clPlot * p_oPlot, clTreePopulation * p_oPop, clAllometry * p_oAllom, float** mp_ffPhoto) {
  try
  {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSailLight::AddTreeToSailFishEye" );
    throw( stcErr );
  }
}
