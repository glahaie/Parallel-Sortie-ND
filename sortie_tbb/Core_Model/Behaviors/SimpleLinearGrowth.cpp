...
clSimpleLinearGrowth::clSimpleLinearGrowth(clSimManager * p_oSimManager) :
  ...
  try
  {
    ...
    strcpy( m_cNameString, "simplelineargrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clSimpleLinearGrowth::clSimpleLinearGrowth" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
      if (-1 == mp_oGrowthOrg->GetLightCode(mp_whatSpeciesTypeCombos[i].iSpecies,
        ...
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clSimpleLinearGrowth::DoShellSetup" );
...
// SetNameData()
/////////////////////////////////////////////////////////////////////////////*/
void clSimpleLinearGrowth::SetNameData(char *cNameString) {
  try {

    //Check the string passed and set the flags accordingly
    if (strcmp("SimpleLinearGrowth", cNameString) == 0) {
      m_iGrowthMethod = diameter_auto;
    } else if (strcmp("SimpleLinearGrowth diam only", cNameString) == 0) {
      m_iGrowthMethod = diameter_only;
    } else if (strcmp("SimpleLinearGrowth height only", cNameString) == 0) {
      ...
      sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized behavior name \"",
          cNameString, "\".");
      strcpy(stcErr.cFunction, "clSimpleLinearGrowth::SetNameData");
      throw(stcErr);
    }
  }
...
inline float clSimpleLinearGrowth::CalculateFunctionValue(clTree *p_oTree) {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clSimpleLinearGrowth::CalculateFunctionValue");
    throw(stcErr);
  }
}
