...
clQuadratGLILight::clQuadratGLILight(clSimManager * p_oSimManager) :
  ...
  //Set the namestring
  strcpy(m_cNameString, "quadratglilightshell");
...
void clQuadratGLILight::DoShellSetup(xercesc::DOMDocument * p_oDoc) {
  try
  {
    ...
    if ( 0 >= m_iNumAltAng )
    {
      ...
      strcpy( stcErr.cFunction, "clQuadratGLILight::DoShellSetup" );
    ...
    else
    {
      ...
      if ( -1 == m_iGridGliCode )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clQuadratGLILight::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "Quadrat GLI grid was incorrectly set up in the parameter file.  Missing float \"GLI\"." );
        throw( stcErr );
      }
    }
...
catch ( ... )
{
  modelErr stcErr;
  stcErr.iErrorCode = UNKNOWN;
  strcpy( stcErr.cFunction, "clQuadratGLILight::TimestepCleanup" );
  throw( stcErr );
}
}
