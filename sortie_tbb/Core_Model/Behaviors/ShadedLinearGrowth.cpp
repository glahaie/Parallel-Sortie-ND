...
clShadedLinearGrowth::clShadedLinearGrowth( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clGrowthBase( p_oSimManager )
{
  ...
  try
  {
    ...
    strcpy( m_cNameString, "shadedlineargrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clShadedLinearGrowth::clShadedLinearGrowth" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
      if (-1 == mp_oGrowthOrg->GetLightCode(mp_whatSpeciesTypeCombos[i].iSpecies,
        ...
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clShadedLinearGrowth::DoShellSetup" );
...
// SetNameData()
/////////////////////////////////////////////////////////////////////////////*/
void clShadedLinearGrowth::SetNameData(char *cNameString) {
  try {

    //Check the string passed and set the flags accordingly
    if (strcmp("ShadedLinearGrowth", cNameString) == 0) {
      m_iGrowthMethod = diameter_auto;
    } else if (strcmp("ShadedLinearGrowth diam only", cNameString) == 0) {
      m_iGrowthMethod = diameter_only;
    } else if (strcmp("ShadedLinearGrowth height only", cNameString) == 0) {
      ...
      sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized behavior name \"",
          cNameString, "\".");
      strcpy(stcErr.cFunction, "clShadedLinearGrowth::SetNameData");
      throw(stcErr);
    }
  }
