...
{
  //Set the namestring
  strcpy( m_cNameString, "GLIMapCreator" );
...
void clGLIMap::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clGLIMap::GetData" );
    throw( stcErr );
  }
}
...
void clGLIMap::SetUpGrid()
{
  ...
  else
  {
    ...
    if ( -1 == m_iGridGliCode )
    {
      ...
      strcpy( stcErr.cFunction, "clGLIMap::SetUpGrid" );
      strcpy( stcErr.cMoreInfo, "\"GLI Map\" grid was incorrectly set up in the parameter file.  Missing float \"GLI\"." );
      throw( stcErr );
    }
  }
...
void clGLIMap::ReadParameterFileData( xercesc::DOMDocument * p_oDoc )
{
  ...
  if ( 0 >= m_iNumAltAng )
  {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clGLIMap::ReadParameterFileData" );
  ...
  if ( 0 > m_fLightHeight )
  {
    ...
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clGLIMap::ReadParameterFileData" );
    strcpy( stcErr.cMoreInfo, "The height of the GLI point for the GLI map cannot be less than 0." );
    throw( stcErr );
  }
}
...
void clGLIMap::Action()
{
      ...
      for ( iY = 0; iY < iNumYCells; iY++ )
      {
        ...
        while ( p_oNeighbor != NULL )
        {
          ...
          if ( clTreePopulation::seedling != p_oNeighbor->GetType() )
               ...
               AddTreeToGliFishEye( fX, fY, m_fLightHeight, p_oNeighbor, p_oPlot, p_oPop, 
                       p_oAllom, mp_ffPhoto );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clGLIMap::Action" );
    throw( stcErr );
  }
}
