...
clDensitySeedSurvival::clDensitySeedSurvival(clSimManager * p_oSimManager) :
  ...
  try {
    strcpy(m_cNameString, "DensityDependentSeedSurvival");
    ...
    strcpy(stcErr.cFunction, "clDensitySeedSurvival::clDensitySeedSurvival");
    throw(stcErr);
  }
}
...
void clDensitySeedSurvival::GetData(DOMDocument * p_oDoc) {
  try {
    ...
    if (mp_oSeedGrid == NULL) {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "clDensitySeedSurvival::GetData");
    ...
    for (i = 0; i < m_iNumBehaviorSpecies; i++) {
      ...
      mp_iSeedGridCode[mp_iWhatSpecies[i]] = mp_oSeedGrid->GetFloatDataCode(cLabel);
...
void clDensitySeedSurvival::ActionUseTrees() {
  ...
  for (iX = 0; iX < iNumXCells; iX++) {
    for (iY = 0; iY < iNumYCells; iY++) {
      ...
      sprintf( cQuery, "%s%f%s%f%s%f%s%f", "distance=", m_fSearchRadius,
          "FROM x=", fX, "y=", fY, "::height=", fMinHeight);
...
// SetNameData()
//////////////////////////////////////////////////////////////////////////////
void clDensitySeedSurvival::SetNameData(char *cNameString) {
 try {

   //Check the string passed and set the flags accordingly
   if (strcmp("DensityDependentSeedSurvival", cNameString) == 0) {
     m_bSeeds = true;
   } else if (strcmp("ConspecificTreeDensityDependentSeedSurvival", cNameString) == 0) {
     m_bSeeds = false;
   } else {
     modelErr stcErr;
     stcErr.iErrorCode = BAD_DATA;
     sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized behavior name \"",
        cNameString, "\".");
     strcpy(stcErr.cFunction, "clDensitySeedSurvival::SetNameData");
     throw(stcErr);
   }
 }
