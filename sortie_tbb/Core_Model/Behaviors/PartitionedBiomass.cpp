...
clPartitionedBiomass::clPartitionedBiomass( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
	...
	{
		//Set the namestring
		strcpy( m_cNameString, "PartitionedBiomass" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clPartitionedBiomass::clPartitionedBiomass" );
    throw( stcErr );
  }
}
...
// SetNameData()
/////////////////////////////////////////////////////////////////////////////*/
void clPartitionedBiomass::SetNameData(char *cNameString) {
 try {

   //Check the string passed and set the flags accordingly
   if (strcmp("PartitionedDBHBiomass", cNameString) == 0) {
     m_bIsDbh = true;
   } else if (strcmp("PartitionedHeightBiomass", cNameString) == 0) {
     m_bIsDbh = false;
   }
   else {
     modelErr stcErr;
     ...
     sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized behavior name \"",
        cNameString, "\".");
     strcpy(stcErr.cFunction, "clPartitionedBiomass::SetNameData");
     throw(stcErr);
   }
 }
...
void clPartitionedBiomass::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clPartitionedBiomass::GetData" );
    throw( stcErr );
  }
}
...
void clPartitionedBiomass::GetAppliesTo()
{
  ...
  for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
    if (clTreePopulation::seedling == mp_whatSpeciesTypeCombos[i].iType) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clPartitionedBiomass::GetAppliesTo" );
  ...
  if (NULL == mp_oGrid) {
    ...
    for (i = 0; i < m_iNumTotalSpecies; i++) {
      ...
      if (-1 == mp_iLeafCodes[i]) {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clPartitionedBiomass::SetupGrid" );
...
void clPartitionedBiomass::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clPartitionedBiomass::Action" );
    throw( stcErr );
  }
}
