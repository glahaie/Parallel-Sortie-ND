...
clLogisticGrowth::clLogisticGrowth( clSimManager * p_oSimManager ) :
  ...
  try
  {
    ...
    strcpy( m_cNameString, "logisticgrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clLogisticGrowth::clLogisticGrowth" );
    throw( stcErr );
  }
}
...
void clLogisticGrowth::DoShellSetup( DOMDocument * p_oDoc )
{
  try
  {
    ...
    for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
      if (-1 == mp_oGrowthOrg->GetLightCode(mp_whatSpeciesTypeCombos[i].iSpecies,
        ...
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clLogisticGrowth::DoShellSetup" );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clLogisticGrowth::GetNameData" );
    throw( stcErr );
  }
}
...
// SetNameData()
//////////////////////////////////////////////////////////////////////////////
void clLogisticGrowth::SetNameData(char *cNameString) {
  try {

    //Check the string passed and set the flags accordingly
    if (strcmp("LogisticGrowth", cNameString) == 0) {
      m_iGrowthMethod = diameter_auto;
    } else if (strcmp("LogisticGrowth diam only", cNameString) == 0) {
      m_iGrowthMethod = diameter_only;
    } else if (strcmp("LogisticGrowth height only", cNameString) == 0) {
      ...
      sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized behavior name \"",
          cNameString, "\".");
      strcpy(stcErr.cFunction, "clLogisticGrowth::SetNameData");
      throw(stcErr);
    }
  }
...
float clLogisticGrowth::CalculateFunctionValue(clTree *p_oTree) {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clLogisticGrowth::CalculateFunctionValue");
    throw(stcErr);
  }
}
