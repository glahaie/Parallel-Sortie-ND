     ...
     clGrowthBase( p_oSimManager )
{
  ...
  try
  {
    ...
    strcpy( m_cNameString, "prstormbilevelgrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clPRStormBiLevelGrowth::clPRStormBiLevelGrowth" );
    throw( stcErr );
  }
}
...
void clPRStormBiLevelGrowth::DoShellSetup( DOMDocument * p_oDoc )
{
  ...
  try
  {
    ...
    if (NULL == mp_oStorm) {
      ...
      strcpy(stcErr.cFunction, "clPRStormBiLevelGrowth::DoShellSetup");
      strcpy(stcErr.cMoreInfo, "The storm behavior must be used with PR Storm Bi-Level Growth.");
      throw(stcErr);
    }
    ...
    if (-1 == m_iStormtimeCode) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPRStormBiLevelGrowth::DoShellSetup");
      strcpy(stcErr.cMoreInfo, "The \"stormtime\" was not found in the \"Storm Damage\" grid.");
      throw(stcErr);
    }
    ...
    if (NULL == mp_oStormLight) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPRStormBiLevelGrowth::DoShellSetup");
      strcpy(stcErr.cMoreInfo, "The storm light behavior must be used with PR Storm Bi-Level Growth.");
      throw(stcErr);
    }
    ...
    if (-1 == m_iLightCode) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPRStormBiLevelGrowth::DoShellSetup");
      strcpy(stcErr.cMoreInfo, "The \"Light\" was not found in the \"Storm Light\" grid.");
      throw(stcErr);
    }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (p_fTempValues[i].val < 0 || p_fTempValues[i].val > 100) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clPRStormBiLevelGrowth::DoShellSetup");
        strcpy(stcErr.cMoreInfo, "All values in high-light growth threshold must be between 0 and 100.");
        throw(stcErr);
      }
...
float clPRStormBiLevelGrowth::CalcDiameterGrowthValue(clTree *p_oTree, clTreePopulation *p_oPop, float fHeightGrowth) {
  try {
    ...
    float fLightLevel, fDiam, fX, fY, fNumYears, fHeightGrowth, fHeight;
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clPRStormBiLevelGrowth::CalcDiameterGrowthValue");
   throw(stcErr);
 }
}
