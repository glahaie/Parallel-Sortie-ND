  ...
  clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clDisperseBase( p_oSimManager)
{
  try
  {
    strcpy( m_cNameString, "spatialdisperse" );
    m_iNumFunctions = 2;
    m_iNumCovers = 2;
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clSpatialDispersal::clSpatialDispersal" );
    throw( stcErr );
  }
}
...
void clSpatialDispersal::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSpatialDispersal::DoShellSetup" );
    throw( stcErr );
  }
}
...
void clSpatialDispersal::PopulateUsedTable( clTreePopulation * p_oPop )
{
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    ...
    if ( mp_whatSpeciesTypeCombos[i].iType != clTreePopulation::sapling
         ...
         {
           modelErr stcErr;
           strcpy( stcErr.cFunction, "clSpatialDisperse::PopulateUsedTable" );
           strcpy( stcErr.cMoreInfo, "Dispersal may only be applied to saplings and adults." );
  ...
  try {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
      if ( p_iTemp[i].val < 0 || p_iTemp[i].val >= m_iNumFunctions )
      {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clSpatialDispersal::GetParameterFileData" );
        sprintf( stcErr.cMoreInfo, "%s%d", "Invalid disperse function code ", p_iTemp[i].val );
        throw( stcErr );
      }
      else
      {
    ...
    if ( iNumFunctionSpecies > 0 )
    {
      ...
      for ( i = 0; i < iNumFunctionSpecies; i++ ) {
        ...
        if (p_fTemp[i].val >= 50) {
          ...
          stcErr.iErrorCode = BAD_DATA;
          strcpy( stcErr.cFunction, "clSpatialDispersal::GetParameterFileData" );
          strcpy( stcErr.cMoreInfo, "One or more weibull theta values is too large and will cause math errors.");
          throw( stcErr );
        }
      }
      ...
      for ( i = 0; i < iNumFunctionSpecies; i++ ) {
        ...
        if (p_fTemp[i].val > 25) {
          ...
          stcErr.iErrorCode = BAD_DATA;
          strcpy( stcErr.cFunction, "clSpatialDispersal::GetParameterFileData" );
          strcpy( stcErr.cMoreInfo, "One or more beta values is too large and will cause math errors.");
          throw( stcErr );
        }
      }
...
void clSpatialDispersal::CalculateProbabilityDistribution( float * p_fProbArray, int iMaxDistance, int iSpecies,
     function iFunction, cover iCover )
     {
       try
       {
         ...
         for ( iDistance = 1; iDistance < iMaxDistance; iDistance++ )
         {
           if ( p_fProbArray[iDistance] < p_fProbArray[iDistance - 1] )
           {
             modelErr stcErr;
             stcErr.iErrorCode = BAD_DATA;
             strcpy( stcErr.cFunction, "clDisperseBase::CalculateProb..." );
             strcpy( stcErr.cMoreInfo, "Probability array does not increase sequentially." );
             throw( stcErr );
           }
         }
       }
       ...
       catch ( ... )
       {
         modelErr stcErr;
         stcErr.iErrorCode = UNKNOWN;
         strcpy( stcErr.cFunction, "clSpatialDisperse::CalculateProbability..." );
         throw( stcErr );
       }
}
...
void clSpatialDispersal::AddSeeds()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSpatialDisperse::AddSeeds" );
    throw( stcErr );
  }
}
...
void clSpatialDispersal::SpatialDisperse( clTree * p_oTree, clTreePopulation * p_oPop, clPlot * p_oPlot, float fDbh )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSpatialDispersal::Disperse" );
    throw( stcErr );
  }
}
...
void clSpatialDispersal::NonSpatialDisperse( clTree * p_oTree, clTreePopulation * p_oPop, clPlot * p_oPlot, float fDbh )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSpatialDisperse::NonSpatialDisperse" );
    throw( stcErr );
  }
}
...
void clSpatialDispersal::CalculateGapStatus( clTreePopulation * p_oPop )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSpatialDisperse::CalculateGapStatus" );
    throw( stcErr );
  }
}
...
// SetNameData()
////////////////////////////////////////////////////////////////////////////
void clSpatialDispersal::SetNameData( char * cNameString )
{
  try
  {
    if ( strcmp( cNameString, "GapDisperse" ) == 0 )
    {
      m_bIsGap = true;
      m_sXMLRoot = "GapDisperse";

    }
    else if ( strcmp( cNameString, "NonGapDisperse" ) == 0 )
    {
      m_bIsGap = false;
    ...
    { //error!
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clSpatialDispersal::SetNameData" );
      const char* cWarning = "Unrecognized name ";
      sprintf( stcErr.cMoreInfo, "%s%s", cWarning, cNameString );
      throw( stcErr );
    }
  }
  catch ( modelErr & err )
