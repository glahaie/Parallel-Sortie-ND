...
clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clMortalityBase( p_oSimManager )
{
  ...
  {
    //Set namestring
    strcpy( m_cNameString, "ncimortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clNCIMort::clNCIMort" );
    throw( stcErr );
  }
}
...
void clNCIMort::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clNCIMort::DoShellSetup" );
    throw( stcErr );
  }
}
...
void clNCIMort::ReadParameterFile( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
      if ( clTreePopulation::sapling != mp_whatSpeciesTypeCombos[i].iType
           && clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType )
           {
             ...
             strcpy( stcErr.cFunction, "clNCIMort::ReadParameterFile" );
    ...
    {
      sprintf( cName, "%s%s%s", "mo_nci", p_oPop->TranslateSpeciesCodeToName( i ), "NeighborLambda" );
      FillSpeciesSpecificValue( p_oElement, cName, "mo_nlVal", p_fTempValues, m_iNumBehaviorSpecies, p_oPop, true );
...
void clNCIMort::GetTreeMemberCodes()
{
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
               ...
               [mp_whatSpeciesTypeCombos[i].iType - clTreePopulation::sapling] )
    {
      ...
      strcpy( stcErr.cMoreInfo, "Can't find the \"dead\" data member." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
  }
...
void clNCIMort::ValidateData()
{
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "All values for max survival probability must be between 0 and 1." );
        throw( stcErr );
      }
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "Size effect mode values cannot be 0." );
        throw( stcErr );
      }
  ...
  {
    strcpy( err.cFunction, "clNCIMort::ValidateData" );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clNCIMort::ValidateData" );
    throw( stcErr );
  }
}
...
void clNCIMort::PreMortCalcs( clTreePopulation * p_oPop )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clNCIMort::PreMortGrowth" );
    throw( stcErr );
  }
}
