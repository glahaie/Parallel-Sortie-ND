...
clTreeRemover::clTreeRemover(clSimManager *p_oSimManager) :
 ...
 try {
   ...
   strcpy(m_cNameString, "removedead");
   ...
   strcpy(stcErr.cFunction, "clTreeRemover::clTreeRemover");
   throw(stcErr);
 }
}
 ...
 try {
   ...
   for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
     ...
     if (-1 == mp_iDeadCodes[mp_whatSpeciesTypeCombos[i].iSpecies]
                           [mp_whatSpeciesTypeCombos[i].iType]) {
       modelErr stcErr;
       strcpy(stcErr.cFunction, "clTreeRemover::GetData");
       sprintf(stcErr.cMoreInfo, "%s%d%s%d%s%s",
         "Type/species combo species=", mp_whatSpeciesTypeCombos[i].iSpecies,
         " type=", mp_whatSpeciesTypeCombos[i].iType, " cannot be used with",
         " this behavior.");
...
void clTreeRemover::Action() {
 try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clTreeRemover::Action");
   throw(stcErr);
 }
}
