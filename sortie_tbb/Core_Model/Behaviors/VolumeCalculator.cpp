...
clVolumeCalculator::clVolumeCalculator( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "TreeVolumeCalculator" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clVolumeCalculator::clVolumeCalculator" );
    throw( stcErr );
  }
}
...
void clVolumeCalculator::GetData( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clVolumeCalculator::GetData" );
      strcpy( stcErr.cMoreInfo, "Stump height cannot be less than 0." );
    ...
    if ( m_fMinUsableDiam < 0 )
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clVolumeCalculator::GetData" );
      strcpy( stcErr.cMoreInfo, "Minimum usable diameter cannot be less than 0." );
      stcErr.iErrorCode = BAD_DATA;
      delete[] p_fTempValues;
      throw( stcErr );
    }
    ...
    if ( m_fSegmentLength <= 0 )
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clVolumeCalculator::GetData" );
      strcpy( stcErr.cMoreInfo, "Segment length cannot be less than 0." );
      stcErr.iErrorCode = BAD_DATA;
      delete[] p_fTempValues;
      throw( stcErr );
    }
...
void clVolumeCalculator::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clVolumeCalculator::Action" );
    throw( stcErr );
  }
}
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    ...
    if ( clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType
         ...
         && clTreePopulation::snag != mp_whatSpeciesTypeCombos[i].iType )
         {
           ...
           modelErr stcErr;
           strcpy( stcErr.cFunction, "clVolumeCalculator::RegisterTreeDataMembers" );
