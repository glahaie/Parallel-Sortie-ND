      ...
      clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clMortalityBase(p_oSimManager)
{
 try {
   ...
   strcpy(m_cNameString, "stochasticmortshell");
   ...
   strcpy(stcErr.cFunction, "clStochasticMort::clStochasticMort");
   throw(stcErr);
 }
}
...
void clStochasticMort::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
 try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clStochasticMort::DoShellSetup");
   throw(stcErr);
 }
}
...
deadCode clStochasticMort::DoMort(clTree *p_oTree, const float &fDiam,
 ...
 try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clStochasticMort::DoMort");
   throw(stcErr);
 }
}
