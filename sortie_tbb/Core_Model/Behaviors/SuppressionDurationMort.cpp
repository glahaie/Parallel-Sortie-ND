  ...
  clMortalityBase( p_oSimManager )
{
  try
  {
    strcpy( m_cNameString, "suppressiondurationmortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clSuppressionDurationMort::clSuppressionDurationMort" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if ( 0 > p_fTempValues[i].val || 1 < p_fTempValues[i].val ) {
        ...
        strcpy( stcErr.cFunction, "clSuppressionDurationMort::DoShellSetup" );
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if ( 0 == p_fTempValues[i].val) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clSuppressionDurationMort::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "Values for suppression duration mortality X0 cannot be 0." );
        throw( stcErr );
      }
...
deadCode clSuppressionDurationMort::DoMort( clTree * p_oTree, const float & fDiam, const short int & iSpecies )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clSuppressionDurationMort::DoMort" );
    throw( stcErr );
  }
}
