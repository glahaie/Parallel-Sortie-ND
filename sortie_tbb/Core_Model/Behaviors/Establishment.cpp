...
clEstablishment::clEstablishment( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  try
  {
    strcpy( m_cNameString, "Establishment" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clEstablishment::clEstablishment" );
    throw( stcErr );
  }
}
...
void clEstablishment::GetData( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    if ( NULL == mp_oSeedGrid )
    {
      ...
      strcpy( stcErr.cFunction, "clEstablishment::GetData" );
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      if ( -1 == mp_iSeedGridCode[mp_iWhatSpecies[i]] )
      {
        ...
        strcpy( stcErr.cFunction, "clEstablishment::GetData" );
        strcpy( stcErr.cMoreInfo, "A disperse behavior is required for all species using establishment." );
        throw( stcErr );
      }
    }
  }
...
void clEstablishment::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clEstablishment::Action" );
    throw( stcErr );
  }
}
