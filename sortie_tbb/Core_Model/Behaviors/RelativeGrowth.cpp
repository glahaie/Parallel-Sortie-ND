...
clRelativeGrowth::clRelativeGrowth(clSimManager *p_oSimManager) :
 ...
 {
    strcpy(m_cNameString, "relativegrowthshell");
    //Version 1.1
...
// SetNameData()
//////////////////////////////////////////////////////////////////////////////
void clRelativeGrowth::SetNameData( char * cNameString )
{
  try
  {

    //Check the string passed and set the flags accordingly
    if ( strcmp( "RelRadialGrowth", cNameString ) == 0 )
    {
    ...
    else if ( strcmp( "RelBAGrowth", cNameString ) == 0 )
    {
    ...
    else if ( strcmp( "RelUnlimGrowth", cNameString ) == 0 )
    {
      m_bConstRadialLimited = false;
      m_bConstBasalAreaLimited = false;
      m_iGrowthMethod = diameter_auto;
    ...
    else if ( strcmp( "RelRadialGrowth diam only", cNameString ) == 0 )
    {
      m_bConstRadialLimited = true;
      m_bConstBasalAreaLimited = false;
    ...
    else if ( strcmp( "RelBAGrowth diam only", cNameString ) == 0 )
    {
      m_bConstRadialLimited = false;
      m_bConstBasalAreaLimited = true;
      m_iGrowthMethod = diameter_only;
    ...
    else if ( strcmp( "RelUnlimGrowth diam only", cNameString ) == 0 )
    {
      m_bConstRadialLimited = false;
      m_bConstBasalAreaLimited = false;
      m_iGrowthMethod = diameter_only;
    ...
    else if ( strcmp( "RelativeHeight", cNameString ) == 0 )
    {
      m_bConstRadialLimited = false;
      m_bConstBasalAreaLimited = false;
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clRelativeGrowth::SetNameData" );
    throw( stcErr );
  }
}
...
void clRelativeGrowth::DoShellSetup( DOMDocument * p_oDoc )
{
  try
  {
    ...
    for ( int i = 0; i < m_iNumSpeciesTypeCombos; i++ )
    {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clRelativeGrowth::DoShellSetup" );
        sprintf( stcErr.cMoreInfo, "%s%d%s%d%s",
                 "Type/species combo species=",
