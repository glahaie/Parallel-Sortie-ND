...
clRandomBrowse::clRandomBrowse( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "RandomBrowse" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clRandomBrowse::clRandomBrowse" );
    throw( stcErr );
  }
}
...
void clRandomBrowse::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy(stcErr.cFunction, "clRandomBrowse::GetData");
      sprintf(stcErr.cMoreInfo, "%s%d", "Unrecognized value for di_randBrowsePDF: ", iTemp);
    ...
    for ( i = 0; i < m_iNumSpecies; i++ ) {
      if (mp_fBrowseProb[i] < 0 || mp_fBrowseProb[i] > 1) {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clRandomBrowse::GetData" );
        sprintf( stcErr.cMoreInfo, "%s%f%s%d%s",
                 "Invalid browse probability value \"",
                 mp_fBrowseProb[i],
                 "\" for species ",
                 i,
...
void clRandomBrowse::Action()
{
  ...
  catch ( ... )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clRandomBrowse::Action" );
    throw( stcErr );
  }
}
