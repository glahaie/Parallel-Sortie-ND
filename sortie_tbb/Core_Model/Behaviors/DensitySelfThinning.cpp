...
clDensitySelfThinning::clDensitySelfThinning(clSimManager *p_oSimManager) :
 ...
 try {
   ...
   strcpy(m_cNameString, "densityselfthinningmortshell");
   ...
   strcpy(stcErr.cFunction, "clDensitySelfThinning::clDensitySelfThinning");
   throw(stcErr);
 }
}
...
void clDensitySelfThinning::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
  ...
  try {
    ...
    if ( m_fNumberYearsPerTimestep != 1 )
    {
      ...
      strcpy( stcErr.cFunction, "clDensitySelfThinning::DoShellSetup" );
      strcpy( stcErr.cMoreInfo, "This behavior can only be used with a one year timestep" );
      throw( stcErr );
     }
    ...
    for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
      if ( clTreePopulation::adult == mp_whatSpeciesTypeCombos[i].iType )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDensitySelfThinning::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "This behavior cannot be applied to Adults" );
        throw( stcErr );
      }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      if ( mp_fSelfThinRadius[mp_iIndexes[p_fTempValues[i].code]] <= 0 )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDensitySelfThinning::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "The Density Self Thinning parameter SelfThinRadius must be a positive number." );
        throw( stcErr );
      }
    }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      if ( mp_fMinDensityForMort[mp_iIndexes[p_fTempValues[i].code]] < 0 )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDensitySelfThinning::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "The Density Self Thinning parameter MinDensityForMort must be greater than or equal to 0." );
        throw( stcErr );
      }
    }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      if ( mp_fSelfThinAsymptote[mp_iIndexes[p_fTempValues[i].code]] <= 0 )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDensitySelfThinning::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "The Density Self Thinning parameter SelfThinAsymptote must be a positive number." );
        throw( stcErr );
      }
    }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      if ( mp_fSelfThinDensityEffect[mp_iIndexes[p_fTempValues[i].code]] <= 0 )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDensitySelfThinning::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "The Density Self Thinning parameter SelfThinDensityEffect must be a positive number." );
        throw( stcErr );
      }
    }
...
void clDensitySelfThinning::CalculateNeighborhoodTreeCountAndMeanDiam10( clTree * p_oTree, float *p_fTreeCount, float *p_fMeanDiam10)
{
  try
  {
    clPopulationBase *p_oTemp =
           mp_oSimManager->GetPopulationObject("treepopulation");
    ...
    char cQuery[200]; //format search strings into this
    ...
    sprintf( cQuery, "%s%f%s%f%s%f%s%f", "distance=", mp_fSelfThinRadius[mp_iIndexes[iTargetSpecies]], "FROM x=", fTargetX,
         "y=", fTargetY, "::height=", 0.0 );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDensitySelfThinning::CalculateNeighborhoodTreeCountAndMeanDiam10" );
    throw( stcErr );
  }
}
...
deadCode clDensitySelfThinning::DoMort(clTree *p_oTree, const float &fDbh, const short int &iSpecies) {
 try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clDensitySelfThinning::DoMort");
   throw(stcErr);
 }
}
