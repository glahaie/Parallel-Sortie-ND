     ...
     clLightBase( p_oSimManager), clGLIBase( p_oSimManager )
{
  try
  {
    strcpy( m_cNameString, "EpiphyticEstablishment" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clEpiphyticEstablishment::clEpiphyticEstablishment" );
    throw( stcErr );
  }
}
...
void clEpiphyticEstablishment::GetParameterFileData( xercesc::DOMDocument * p_oDoc )
{
  ...
  if ( m_iNumAltAng < 1 || m_iNumAziAng < 1 )
  {
    ...
    strcpy( stcErr.cFunction, "clEpiphyticEstablishment::GetParameterFileData" );
...
void clEpiphyticEstablishment::DoLightSetup()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clEpiphyticEstablishment::DoLightSetup" );
    throw( stcErr );
  }
}
    ...
    const float & fX, const float & fY, const float &fHeight )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clEpiphyticEstablishment::GetGLI" );
    throw( stcErr );
  }
}
...
void clEpiphyticEstablishment::GetTreeDataMemberCodes()
{
  ...
  for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
    ...
    if (clTreePopulation::adult != iTp && clTreePopulation::sapling != iTp) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clEpiphyticEstablishment::GetTreeDataMemberCodes" );
    ...
    if (-1 == mp_iDeadCodes[iSp][iTp]) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clEpiphyticEstablishment::GetTreeDataMemberCodes" );
      strcpy( stcErr.cMoreInfo, "All trees must have a mortality behavior assigned." );
      throw( stcErr );
    }
  }
}
...
void clEpiphyticEstablishment::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clEpiphyticEstablishment::Action" );
    throw( stcErr );
  }
}
