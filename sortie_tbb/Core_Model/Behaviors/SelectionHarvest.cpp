...
: clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ) {
  try {
    strcpy(m_cNameString, "SelectionHarvest");
    ...
    strcpy(stcErr.cFunction, "clSelectionHarvest::clSelectionHarvest");//(JM)
    throw(stcErr);
  }
}
...
void clSelectionHarvest::GetData(DOMDocument *p_oDoc) {
  try {
    ...
    if(0==m_iNumAllowedCutRanges)
    {
      ...
      strcpy(stcErr.cFunction, "clSelectionHarvest::GetData");
      sprintf(stcErr.cMoreInfo, "%s", "Invalid number of cut ranges.  At least 1 cut range must be specified.");
      throw(stcErr);
    }
    ...
    for(i=0; i < m_iNumAllowedCutRanges; i++)
    {
      ...
      if(mp_fTargetBA[i] < 0)
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clSelectionHarvest::GetData");
        sprintf(stcErr.cMoreInfo, "%s", "Invalid Target Basal Area value.  Value must be greater than or equal to zero.");
        throw(stcErr);
      }
      ...
      if(mp_fHighDBH[i] < 0 || mp_fLowDBH[i] < 0)
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clSelectionHarvest::GetData");
        sprintf(stcErr.cMoreInfo, "%s", "Invalid bound on size class.  Values must be greater than or equal to zero.");
        throw(stcErr);
      }
      ...
      if(mp_fLowDBH[i] >= mp_fHighDBH[i])
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clSelectionHarvest::GetData");
        sprintf(stcErr.cMoreInfo, "%s", "Invalid bound on size class.  The lower bound must be smaller than the upper bound.");
        throw(stcErr);
      }
      ...
      if(i!=0)
      {
        if(mp_fLowDBH[i] <= mp_fHighDBH[i-1])
        {
          ...
          stcErr.iErrorCode = BAD_DATA;
          strcpy(stcErr.cFunction, "clSelectionHarvest::GetData");
          sprintf(stcErr.cMoreInfo, "%s", "Invalid bound on size class.  The values must not overlap.");
          throw(stcErr);
        }
      }
    }
    ...
    float *p_fLoDbh, float *p_fHiDbh)
{
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clSelectionHarvest::GetBasalArea");
    throw(stcErr);
  }
...
void clSelectionHarvest::GetHarvestGrids() {
  try {
    ...
    if (!mp_oMasterCutsGrid)
    {
      ...
      strcpy(stcErr.cFunction, "clSelectionHarvest::GetHarvestGrids");
      strcpy(stcErr.cMoreInfo, "Master cuts grid required.");
      throw(stcErr);
    }
    ...
    if (-1 == m_iAmountTypeCode || -1 == m_iMasterIDCode || -1 == m_iMasterTimestepCode || -1 == m_iCutTypeCode)
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clSelectionHarvest::GetHarvestGrids");
      strcpy(stcErr.cMoreInfo, "Unexpected master grid data return code.");
      throw(stcErr);
    }
    for (i = 0; i < iNumSpecies; i++)
    {
      if (-1 == mp_iSpeciesCodes[i])
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clSelectionHarvest::GetHarvestGrids");
        strcpy(stcErr.cMoreInfo, "Unexpected master grid data return code for species.");
        throw(stcErr);
      }
    }
    for (i = 0; i < m_iNumAllowedCutRanges; i++)
    {
                                                                ...
                                                                || -1 == mp_iRangeAmountCodes[i])
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clSelectionHarvest::GetHarvestGrids");
        strcpy(stcErr.cMoreInfo, "Unexpected master grid data return code for range.");
        throw(stcErr);
      }
    }
    ...
    if (!mp_oMasterCutsGrid)
    {
      ...
      stcErr.iErrorCode = CANT_FIND_OBJECT;
      strcpy(stcErr.cFunction, "clSelectionHarvest::GetHarvestGrids");
      strcpy(stcErr.cMoreInfo, "Cut events grid required.");
      throw(stcErr);
    }
        ...
        while (p_oPackage) {
          ...
          while (p_oNextPackage) {
            ...
            if (iID == iNextID) {
              ...
              modelErr stcErr;
              stcErr.iErrorCode = BAD_DATA;
              strcpy(stcErr.cFunction, "clSelectionHarvest::ValidatePackages");
...
void clSelectionHarvest::GetTimeSinceHarvest()
{
  try
  {
    ...
    if (!mp_oResultsGrid)
    {
      modelErr stcErr;
      stcErr.iErrorCode = CANT_FIND_OBJECT;
      strcpy(stcErr.cFunction, "clSelectionHarvest::GetTimeSinceHarvest");
      strcpy(stcErr.cMoreInfo, "Results cuts grid required.");
      throw(stcErr);
    }
