...
clStormDirectKiller::clStormDirectKiller( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set namestring
    strcpy( m_cNameString, "StormDirectKiller" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clStormDirectKiller::clStormDirectKiller" );
    throw( stcErr );
  }
}
...
void clStormDirectKiller::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clStormDirectKiller::GetData" );
    throw( stcErr );
  }
}
...
void clStormDirectKiller::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clStormDirectKiller::Action" );
    throw( stcErr );
  }
}
...
void clStormDirectKiller::GetDeadCodes(clTreePopulation *p_oPop)
{
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    ...
    if (-1 == mp_iDeadCodes[mp_whatSpeciesTypeCombos[i].iType] [mp_whatSpeciesTypeCombos[i].iSpecies]) {
      ...
      strcpy( stcErr.cFunction, "clStormDirectKiller::GetDeadCodes" );
...
void clStormDirectKiller::GetGridInfo()
{
  ...
  if ( NULL == mp_oStormDamageGrid )
  {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clStormDirectKiller::GetGridInfo" );
    strcpy( stcErr.cMoreInfo, "The storm behavior must be used with the storm behavior." );
    throw( stcErr );
  }
