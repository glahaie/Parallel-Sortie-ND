...
clStormKiller::clStormKiller( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set namestring
    strcpy( m_cNameString, "StormKiller" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clStormKiller::clStormKiller" );
    throw( stcErr );
  }
}
...
void clStormKiller::GetDeadCodes(clTreePopulation *p_oPop)
{
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    ...
    if (-1 == mp_iDeadCodes[mp_whatSpeciesTypeCombos[i].iType] [mp_whatSpeciesTypeCombos[i].iSpecies]) {
      ...
      strcpy( stcErr.cFunction, "clStormKiller::GetDeadCodes" );
      strcpy( stcErr.cMoreInfo, "You must use tree mortality behaviors along with the storm killer." );
      throw( stcErr );
    }
  }
}
...
void clStormKiller::GetStmDmgCodes()
{
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    if ( clTreePopulation::seedling == mp_whatSpeciesTypeCombos[i].iType )
    {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clStormKiller::GetStmDmgCodes" );
      strcpy( stcErr.cMoreInfo, "The storm killer behavior cannot be applied to seedlings." );
      throw( stcErr );
    }
    ...
    if (-1 == mp_iStmDmgCodes[mp_whatSpeciesTypeCombos[i].iType] [mp_whatSpeciesTypeCombos[i].iSpecies]) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clStormKiller::GetStmDmgCodes" );
      strcpy( stcErr.cMoreInfo, "You must use the storm damage applier behavior along with the storm killer." );
      throw( stcErr );
    }
  }
}
...
void clStormKiller::GetParameterFileData( xercesc::DOMDocument * p_oDoc )
{
  ...
  try {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      if ( p_fTempValues[i].val < 0 || p_fTempValues[i].val > 1 )
      {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clStormKiller::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "All values for proportion of fully-damaged trees that tip-up must be between 0 and 1." );
        throw( stcErr );
      }
    ...
    if ( m_iSnagYears < 0 )
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clStormKiller::GetParameterFileData" );
      strcpy( stcErr.cMoreInfo, "The number of years snags last parameter cannot be less than 0." );
      throw( stcErr );
    }
  ...
  catch ( ... )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clStormKiller::GetData" );
    throw( stcErr );
  }
}
...
void clStormKiller::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clStormKiller::Action" );
    throw( stcErr );
  }
}
