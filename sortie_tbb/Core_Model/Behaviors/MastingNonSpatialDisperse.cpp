...
clMastingNonSpatialDisperse::clMastingNonSpatialDisperse(
  ...
  try
  {
    strcpy( m_cNameString, "MastingNonSpatialDisperse" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clMastingNonSpatialDisperse::clMastingNonSpatialDisperse" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if ( p_iTempValues[i].val != inverse_gaussian_pdf &&
        ...
        strcpy( stcErr.cFunction, "clMastingNonSpatialDisperse::DoShellSetup" );
        sprintf( stcErr.cMoreInfo, "%s%d", "Invalid disperse function code ", p_iTempValues[i].val );
        throw( stcErr );
      } else {
    ...
    if ( iNumFunctionSpecies > 0 )
    {
      ...
      for ( i = 0; i < iNumFunctionSpecies; i++ ) {
        if (p_fTempValues[i].val <= 0) {
          ...
          stcErr.iErrorCode = BAD_DATA;
          strcpy( stcErr.cFunction, "clMastingNonSpatialDisperse::DoShellSetup" );
          strcpy( stcErr.cMoreInfo, "Inverse gaussian mu values must be greater than zero.");
          throw( stcErr );
        }
      }
      ...
      for ( i = 0; i < iNumFunctionSpecies; i++ ) {
        if (p_fTempValues[i].val <= 0) {
          ...
          stcErr.iErrorCode = BAD_DATA;
          strcpy( stcErr.cFunction, "clMastingNonSpatialDisperse::DoShellSetup" );
          strcpy( stcErr.cMoreInfo, "Inverse gaussian lambda values must be greater than zero.");
          throw( stcErr );
        }
      }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      if (false == p_bDone[i]) {
        ...
        if (iDraw != 0 && iDraw != 1) { //error condition
          modelErr stcErr;
          stcErr.iErrorCode = UNKNOWN;
          strcpy( stcErr.cMoreInfo, "Invalid binomial random draw." );
          strcpy( stcErr.cFunction, "clMastingNonSpatialDisperse::AddSeeds" );
          throw( stcErr );
        }
...
void clMastingNonSpatialDisperse::ComputeBasalArea() {
  ...
  catch ( ... )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clMastingNonSpatialDisperse::ComputeBasalArea" );
    throw( stcErr );
  }
}
