...
{
  //Set the namestring
  strcpy( m_cNameString, "GLIPointCreator" );
...
void clGLIPoints::GetData( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    fstream oOut( m_cFileName, ios::trunc | ios::out );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clGLIPoints::GetData" );
    throw( stcErr );
  }
}
...
void clGLIPoints::ReadParameterFileData( xercesc::DOMDocument * p_oDoc )
{
  ...
  //Get the filename
  FillSingleValue( p_oElement, "li_GLIPointsFilename", m_cFileName, MAX_FILENAME_SIZE, true );
  //Make sure the file extension is on - if not, append it
  cPos = strstr( m_cFileName, TEXT_FILE_EXT );
  if ( NULL == cPos )
  {
    //Make sure we don't exceed string length - chop it if necessary to allow
    //enough space for string concatenation
    if ( strlen( m_cFileName ) >= ( MAX_FILENAME_SIZE - strlen( TEXT_FILE_EXT ) ) )
      m_cFileName[MAX_FILENAME_SIZE - strlen( TEXT_FILE_EXT ) - 1] = '\0';
    strcat( m_cFileName, TEXT_FILE_EXT );
  }
  ...
  if ( 0 == m_iNumPoints )
  {
    ...
    strcpy( stcErr.cFunction, "clGLIPoints::ReadParameterFileData" );
    strcpy( stcErr.cMoreInfo, "li_GLIPoint" );
    throw( stcErr );
  }
  ...
  for ( j = 0; j < m_iNumPoints; j++ )
  {
         ...
         mp_pointsList[j].fY >= fPlotLenY || mp_pointsList[j].fY < 0 )
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clGLIPoints::ReadParameterFileData" );
      sprintf( stcErr.cMoreInfo, "%s%f%s%f", "Point value outside plot - ", mp_pointsList[j].fX, ", ", mp_pointsList[j].fY );
    ...
    if ( mp_pointsList[j].fHeight < 0 )
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clGLIPoints::ReadParameterFileData" );
      strcpy( stcErr.cMoreInfo, "Height values cannot be negative." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
  ...
  if ( 0 >= m_iNumAltAng )
  {
    ...
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clGLIPoints::ReadParameterFileData" );
    strcpy( stcErr.cMoreInfo, "The number of sky altitude divisions must be greater than 0." );
    throw( stcErr );
  }
  ...
  if ( 0 >= m_iNumAziAng )
  {
    ...
    stcErr.iErrorCode = BAD_DATA;
    strcpy( stcErr.cFunction, "clGLIPoints::ReadParameterFileData" );
    strcpy( stcErr.cMoreInfo, "The number of sky azimuth divisions must be greater than 0." );
    throw( stcErr );
  }
}
...
void clGLIPoints::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clGLIPoints::Action" );
    throw( stcErr );
  }
}
