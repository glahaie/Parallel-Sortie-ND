  ...
  clGrowthBase( p_oSimManager )
{
  ...
  {
    //Set namestring
    strcpy( m_cNameString, "ncibagrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clNCIBAGrowth::clNCIBAGrowth" );
    throw( stcErr );
  }
}
    ...
    for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
      if ( clTreePopulation::sapling != mp_whatSpeciesTypeCombos[i].iType
           && clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType )
           {
             ...
             strcpy( stcErr.cFunction, "clNCIBAGrowth::ReadParameterFile" );
...
void clNCIBAGrowth::ValidateData()
{
  ...
  {
    strcpy( err.cFunction, "clNCIBAGrowth::ValidateData" );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clNCIBAGrowth::ValidateData" );
    throw( stcErr );
  }
}
...
void clNCIBAGrowth::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clNCIBAGrowth::DoShellSetup" );
    throw( stcErr );
  }
}
...
void clNCIBAGrowth::SetNameData( char * cNameString )
{
  try
  {
    ...
    if ( strcmp( "NCIBAGrowth", cNameString ) == 0 )
    {
      m_iGrowthMethod = diameter_auto;
    }
    else if ( strcmp( "NCIBAGrowth diam only", cNameString ) == 0 )
    {
    ...
    else
    {
      ...
      strcpy( stcErr.cFunction, "clNCIBAGrowth::SetNameData" );
      throw( stcErr );
    }
  }
  catch ( modelErr & err )
...
float clNCIBAGrowth::CalcDiameterGrowthValue( clTree * p_oTree, clTreePopulation * p_oPop, float fHeightGrowth )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clNCIBAGrowth::CalcDiameterGrowthValue" );
    throw( stcErr );
  }
}
...
void clNCIBAGrowth::PreGrowthCalcs( clTreePopulation * p_oPop )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clNCIBAGrowth::PreCalcGrowth" );
    throw( stcErr );
  }
}
