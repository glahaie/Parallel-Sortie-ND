...
clSubstrateDepSeedSurvival::clSubstrateDepSeedSurvival(
  ...
  try {
    strcpy(m_cNameString, "Substrate Dependent Seed Survival");
...
// SetNameData
///////////////////////////////////////////////////////////////////////////////
void clSubstrateDepSeedSurvival::SetNameData(char * cNameString) {
  try {
    if (strcmp(cNameString, "NoGapSubstrateSeedSurvival") == 0) {
      m_bUsesGap = false;
      m_bUsesMicro = false;
    } else if (strcmp(cNameString, "GapSubstrateSeedSurvival") == 0) {
      m_bUsesGap = true;
      m_bUsesMicro = false;
    } else if (strcmp(cNameString, "MicrotopographicSubstrateSeedSurvival")
        == 0) {
      m_bUsesMicro = true;
      m_bUsesGap = false;
    } else { //error!
      ...
      strcpy(stcErr.cFunction, "clSubstrateDepSeedSurvival::SetNameData");
      char msg[] = "Unrecognized name ";
      strcpy(stcErr.cMoreInfo, strcat(msg, cNameString));
      throw(stcErr);
    }
  } catch (modelErr& err) {
    throw(err);
  ...
  try {
    ...
    if (m_bUsesMicro) {
      ...
      if (m_fMoundProp < 0 || m_fMoundProp > 1) {
        ...
        strcpy(stcErr.cFunction,
            "clSubstrateDepSeedSurvival::GetParameterFileData");
        strcpy(stcErr.cMoreInfo, "Mound proportion must be between 0 and 1.");
        stcErr.iErrorCode = BAD_DATA;
        throw(stcErr);
      }
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clSubstrateDepSeedSurvival::GetParameterFileData");
    throw(stcErr);
  }
}
...
void clSubstrateDepSeedSurvival::GetData(xercesc::DOMDocument * p_oDoc) {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clSubstrateDepSeedSurvival::GetData");
    throw(stcErr);
  }
}
...
void clSubstrateDepSeedSurvival::SetupGrids() {
  try {
    ...
    if (!mp_oSubstrateGrid) {
      ...
      strcpy(stcErr.cFunction, "clSubstrateDepSeedSurvival::SetupGrids");
      strcpy(stcErr.cMoreInfo, "Substrate behavior required.");
      throw(stcErr);
    }
    ...
    if (NULL == mp_oSubstrateFavGrid) {
      ...
      for (i = 0; i < iNumSpecies; i++) {
        sprintf(cLabel, "%s%d", "Favorability Index", i);
        mp_iSubstrFavCode[i] = mp_oSubstrateFavGrid->RegisterFloat(cLabel);
      }
    }
