...
clGrowthBase( p_oSimManager), clMichMenBase( p_oSimManager )
{
  ...
  strcpy( m_cNameString, "doublemmrelgrowthshell" );
...
// SetNameData()
//////////////////////////////////////////////////////////////////////////////
void clDoubleMMRelGrowth::SetNameData( char * cNameString )
{
  try
  {

    //Check the string passed and set the flags accordingly
    if ( strcmp( "DoubleResourceRelative", cNameString ) == 0 )
    {
      m_iGrowthMethod = diameter_auto;
    }
    else if ( strcmp( "DoubleResourceRelative diam only", cNameString ) == 0 )
    {
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clDoubleMMRelGrowth::SetNameData" );
    throw( stcErr );
  }
}
...
void clDoubleMMRelGrowth::DoShellSetup( DOMDocument * p_oDoc )
{
  ...
  try
  {
    ...
    for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
    {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clDoubleMMRelGrowth::DoShellSetup" );
