...
clPlant::clPlant(clSimManager *p_oSimManager)
  ...
  try {
    ...
    strcpy(stcErr.cFunction, "clPlant::clPlant");
    throw(stcErr);
  }
}
 ...
 try {
   ...
   for (i = 0; i < iNumPlantEvents; i++) {
     ...
     if (iTimestep <= 0 || iTimestep > iNumberTimesteps) {
       ...
       strcpy(stcErr.cFunction, "clPlant::GetData");
       sprintf(stcErr.cMoreInfo, "%s%d", "Invalid plant timestep ", iTimestep);
       throw(stcErr);
     }
     ...
     for (j = 0; j < iNumChildren; j++) {
       ...
       if (-1 == iTemp) {  //throw an error
         ...
         stcErr.iErrorCode = BAD_DATA;
         strcpy(stcErr.cFunction, "clPlant::GetData");
         strcpy(stcErr.cMoreInfo, "Unrecognized species in plant.");
         throw(stcErr);
       }
     ...
     FillSingleValue(p_oElement, "pl_spaceType", cTemp, 100, true);
     //See if it's a value we recognize
     if (strcmp(cTemp, "gridded") == 0) iTemp = gridded;
     else if (strcmp(cTemp, "random") == 0) iTemp = random;
     else {
       modelErr stcErr;
       stcErr.iErrorCode = BAD_DATA;
       strcpy(stcErr.cFunction, "clPlant::GetData");
       strcpy(stcErr.cMoreInfo, "Unrecognized value in pl_spaceType.");
       throw(stcErr);
     }
   ...
   while (p_oPackage) {
     ...
     if (gridded == iSpaceType) {
       ...
       if (fTemp <= 0 || fTemp > mp_oPlantEventsGrid->GetLengthXCells() - 1) {
         modelErr stcErr;
         stcErr.iErrorCode = BAD_DATA;
         strcpy(stcErr.cFunction, "clPlant::ValidatePackages");
         sprintf(stcErr.cMoreInfo, "%s%f",
           "Plant grid spacing must be greater than 0 and less than ",
     ...
     for (i = 0; i < iNumSpecies; i++) {
       ...
       if (fTemp < 0) {
         ...
         stcErr.iErrorCode = BAD_DATA;
         strcpy(stcErr.cFunction, "clPlant::ValidatePackages");
         strcpy(stcErr.cMoreInfo, "Plant amounts must be greater than 0.");
         throw(stcErr);
       }
     ...
     if (fabs(fPercentagesTotal - 100) > 0.01) {
       ...
       strcpy(stcErr.cMoreInfo,
                      "Plant amounts for gridded spacings must add up to 100.");
       throw(stcErr);
     }
       ...
       while (p_oPackage) {
         ...
         while (p_oNextPackage) {
             ...
             if (iID == iNextID) {
               ...
               strcpy(stcErr.cFunction, "clPlant::ValidatePackages");
               strcpy(stcErr.cMoreInfo,
                 "A planting may not be applied twice to the same grid cell.");
               throw(stcErr);
             }
...
void clPlant::TimestepCleanup() {
 try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clPlant::TimestepCleanup");
   throw(stcErr);
 }
}
...
void clPlant::SetupPlantGrids() {
 try {
   ...
   for (i = 0; i < iNumSpecies; i++) {
     sprintf(cFloatLabel, "%s%d", "amtPlant", i);
     mp_iAmtPlantCodes[i] =
             mp_oPlantMasterGrid->RegisterPackageFloat(cFloatLabel);
   }
   ...
   //Register the data members
   for (i = 0; i < iNumSpecies; i++) {
     sprintf(cFloatLabel, "%s%d", "planted", i);
     mp_iPlantedCodes[i] = mp_oPlantResultsGrid->RegisterInt(cFloatLabel);
   }
 } //end of try block
 catch (modelErr&err) {throw(err);}
 catch (modelMsg &msg) {throw(msg);} //non-fatal error
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clPlant::SetupPlantGrids");
   throw(stcErr);
 }
}
...
//       break;  //we've done them all this timestep
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clPlant::Action");
   throw(stcErr);
 }
}
...
void clPlant::PlantTrees(clPackage *p_oMasterPackage) {
 try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clPlant::PlantTrees");
   throw(stcErr);
 }
}
...
int clPlant::AssemblePlantArea(clPackage *p_oMasterPackage, const int
 ...
 try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clPlant::AssemblePlantArea");
   throw(stcErr);
 }
}
//----------------------------------------------------------------------------
