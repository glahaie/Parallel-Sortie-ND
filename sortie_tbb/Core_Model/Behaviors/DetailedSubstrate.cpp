...
clDetailedSubstrate::clDetailedSubstrate( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "DetailedSubstrate" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clDetailedSubstrate::clDetailedSubstrate" );
    throw( stcErr );
  }
}
...
void clDetailedSubstrate::GetData(xercesc::DOMDocument * p_oDoc) {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDetailedSubstrate::GetData" );
    throw( stcErr );
  }
}
...
// -------------
// April 28, 2004 - Submitted in beta version (LEM)
// May 14, 2004 - Fixed a bug by compounding the tree fall and uproot chance
//                by the number of years per timestep (LEM)
// April 22, 2005 - Added new snag uproot parameter
// May 26, 2005 - Changed all the XML names for version 2.0 (LEM)
////////////////////////////////////////////////////////////////////////////
void clDetailedSubstrate::GetParameterFileData(xercesc::DOMDocument * p_oDoc) {
  try
  {
    ...
    static const char * p_cSizes[] = {"Small","Large"}; //translates array indexes to size label in parameter names
    char cConcatenatedString[50];
    ...
    for (k=1;k<=5;k++) {
      sprintf(cConcatenatedString,"su_propFallenTreesLogDecayClass%d",k);
      FillSingleValue( p_oElement, cConcatenatedString, & mp_fPropFallenTreesLogDecayClass[k-1], true );

      sprintf(cConcatenatedString,"su_propFallenSnagsLogDecayClass%d",k);
      FillSingleValue( p_oElement, cConcatenatedString, & mp_fPropFallenSnagsLogDecayClass[k-1], true );
    }
    ...
    for (i = 0; i < m_iNumLogSpGroupsUsed; i++) { //i is SpGroup (0-2)
      ...
      for (j = 0; j <= 1; j++) { //j is index of size class (small=0, large=1)
        ...
        for (k = 1; k <= 5; k++) { //k is decay class (1-5)
          sprintf(cConcatenatedString,"su_initialLogSpGroup%d%sDecayClass%d",i+1,p_cSizes[j],k);
          FillSingleValue( p_oElement, cConcatenatedString, & (mp_fInitLog[i][j][k-1]), true );

          sprintf(cConcatenatedString,"su_logSpGroup%d%sDecayClass%dDecayAlpha",i+1,p_cSizes[j],k);
          FillSingleValue( p_oElement, cConcatenatedString, & (mp_fLogDecayAlpha[i][j][k-1]), true );

          sprintf(cConcatenatedString,"su_logSpGroup%d%sDecayClass%dDecayBeta",i+1,p_cSizes[j],k);
          FillSingleValue( p_oElement, cConcatenatedString, & (mp_fLogDecayBeta[i][j][k-1]), true );
        } //end for k
      } //end for j
    }// end for i
    ...
    if ( mp_oHarvestGrid )
    {
      ...
      for (i = 0; i < m_iNumLogSpGroupsUsed; i++) { //i is SpGroup (0-2)
        ...
        for (j = 0; j <= 1; j++) { //j is index of size class (small=0, large=1)
          ...
          for (k = 1; k <= 5; k++) { //k is decay class (1-5)
            sprintf(cConcatenatedString,"su_partialCutLogSpGroup%d%sDecayClass%d",i+1,p_cSizes[j],k);
            FillSingleValue( p_oElement, cConcatenatedString, & (mp_fPartCutLog[i][j][k-1]), true );
            
            sprintf(cConcatenatedString,"su_gapCutLogSpGroup%d%sDecayClass%d",i+1,p_cSizes[j],k);
            FillSingleValue( p_oElement, cConcatenatedString, & (mp_fGapCutLog[i][j][k-1]), true );
            
            sprintf(cConcatenatedString,"su_clearCutLogSpGroup%d%sDecayClass%d",i+1,p_cSizes[j],k);
            FillSingleValue( p_oElement, cConcatenatedString, & (mp_fClearCutLog[i][j][k-1]), true );
          } //end for k
        } //end for j
      } //end for i
    } //end if harvest
    ...
    for (i = 0; i < m_iNumTotalSpecies; i++) {
      if ( mp_iLogSpGroupAssignment[i] < 0 || mp_iLogSpGroupAssignment[i] > 2)
      {
        ...
        strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "Species must be assigned a group numbered 1-3." );
        throw( stcErr );
      }
    }
    ...
    if (bBadDecayValue)
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
      strcpy( stcErr.cMoreInfo, "Substrate decay parameter(s) outside of valid range." );
      throw( stcErr );
    }
    ...
    if ( fTemp > 1.0 )
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
      strcpy( stcErr.cMoreInfo, "Initial conditions substrate proportions must add up to be less than 1" );
      throw( stcErr );
    }
    ...
    if (mp_fInitialLogMeanDiameter[0] > m_fLogSizeClassBoundary)
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
      strcpy( stcErr.cMoreInfo, "su_initialSmallLogMeanDiameter must be less than su_logSizeClassBoundary" );
      throw( stcErr );
    }
    if (mp_fInitialLogMeanDiameter[1] < m_fLogSizeClassBoundary)
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
      strcpy( stcErr.cMoreInfo, "su_initialLargeLogMeanDiameter must be greater than su_logSizeClassBoundary" );
      throw( stcErr );
    }
    ...
    if ( fabs(fTemp) > 0.0001 )
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
      strcpy( stcErr.cMoreInfo, "Proportion of fallen live trees in decay classes 1-5 must sum to 1." );
      throw( stcErr );
    }
    ...
    if ( fabs(fTemp) > 0.0001 )
    {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
      strcpy( stcErr.cMoreInfo, "Proportion of fallen snags in decay classes 1-5 must sum to 1." );
      throw( stcErr );
    }
    ...
    if ( mp_oHarvestGrid )
    {
      ...
      if ( fTemp > 1.0 )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "Partial cut substrate proportions must add up to be less than 1" );
        throw( stcErr );
      }
      ...
      if ( fTemp > 1.0 )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "Gap cut substrate proportions must add up to be less than 1" );
        throw( stcErr );
      }
      ...
      if ( fTemp > 1.0 )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "Clear cut substrate proportions must add up to be less than 1" );
        throw( stcErr );
      }
      ...
      if (mp_fPartCutLogMeanDiameter[0] > m_fLogSizeClassBoundary)
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "su_partialCutSmallLogMeanDiameter must be less than su_logSizeClassBoundary" );
        throw( stcErr );
      }
      if (mp_fPartCutLogMeanDiameter[1] < m_fLogSizeClassBoundary)
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "su_partialCutLargeLogMeanDiameter must be greater than su_logSizeClassBoundary" );
        throw( stcErr );
      }
      ...
      if (mp_fGapCutLogMeanDiameter[0] > m_fLogSizeClassBoundary)
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "su_gapCutSmallLogMeanDiameter must be less than su_logSizeClassBoundary" );
        throw( stcErr );
      }
      if (mp_fGapCutLogMeanDiameter[1] < m_fLogSizeClassBoundary)
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "su_gapCutLargeLogMeanDiameter must be greater than su_logSizeClassBoundary" );
        throw( stcErr );
      }
      ...
      if (mp_fClearCutLogMeanDiameter[0] > m_fLogSizeClassBoundary)
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "su_clearCutSmallLogMeanDiameter must be less than su_logSizeClassBoundary" );
        throw( stcErr );
      }
      if (mp_fClearCutLogMeanDiameter[1] < m_fLogSizeClassBoundary)
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::GetParameterFileData" );
        strcpy( stcErr.cMoreInfo, "su_clearCutLargeLogMeanDiameter must be greater than su_logSizeClassBoundary" );
        throw( stcErr );
      }
...
// Edit history:
// -------------
// September 6, 2005 - Created to more easily trap for the condition in which
// one of the exponents is 0; plus to be more efficient (LEM)
////////////////////////////////////////////////////////////////////////////
...
void clDetailedSubstrate::CalculateDecayProportions() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDetailedSubstrate::CalculateDecayProportions" );
    throw( stcErr );
  }
}
...
// -------------
// April 28, 2004 - Submitted in beta version (LEM)
// July 21, 2004 - Added the ability for the calcs grid to read a map (LEM)
// May 26, 2005 - Changed the names of the grid data members for 2.0 (LEM)
// November 10, 2005 - Added decayed logs (LEM)
////////////////////////////////////////////////////////////////////////////
void clDetailedSubstrate::SetupSubstrateGrids() {
  try
  {
    ...
    int i,j,k,t; //loop counters
    char cLabel[30]; //label for log codes
    ...
    if ( NULL == mp_oSubstrateGrid )
    {
      ...
      for (i = 0; i < iNumLogSpGroupsUsed; i++) {
        for (j = 0; j <= 1; j++) {
          for (k = 1; k <= 5; k++) {
            sprintf(cLabel,"%s%d%s%s%d","loggroup",i+1,p_cSizes[j],"decay",k);
            mp_iLogCodes[i][j][k-1] = mp_oSubstrateGrid->RegisterFloat(cLabel);
            
            sprintf(cLabel,"vloggroup%d%sdecay%d",i+1,p_cSizes[j],k);
            mp_iLogVolCodes[i][j][k-1] = mp_oSubstrateGrid->RegisterFloat(cLabel);
          } //end for k
        } //end for j
      } //end for i
    }
    else //The substrate grid was created from a map file. Validate it.
    {
          ...
          || ( fYCellLength > 0 && mp_oSubstrateGrid->GetLengthYCells() != fYCellLength ) )
      {
        ...
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::SetupSubstrateGrid" );
        sprintf( stcErr.cMoreInfo, "%s%s%s", "Grid map values for grid ",
            "\"DetailedSubstrate\" contain cell lengths which do not match the allowed ", "values." );
        throw( stcErr );
      }
      ...
      for (i = 0; i < iNumLogSpGroupsUsed; i++) {
        for (j = 0; j <= 1; j++) {
          for (k = 1; k <= 5; k++) {
            sprintf(cLabel,"loggroup%d%sdecay%d",i+1,p_cSizes[j],k);
            mp_iLogCodes[i][j][k-1] = mp_oSubstrateGrid->GetFloatDataCode(cLabel);
            if (mp_iLogCodes[i][j][k-1]== (-1))
              iNoLogCode = -1;
            sprintf(cLabel,"vloggroup%d%sdecay%d",i+1,p_cSizes[j],k);
            mp_iLogVolCodes[i][j][k-1] = mp_oSubstrateGrid->GetFloatDataCode(cLabel);
          ...
          -1 == m_iTotalLogVolCode)
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::SetupSubstrateGrid" );
        sprintf( stcErr.cMoreInfo, "%s%s", "Grid map setup values for grid ",
            "\"DetailedSubstrate\" did not contain all required grid data members." );
        throw( stcErr );
      }
    ...
    if ( NULL == mp_oCalcGrid )
    {
      ...
      for (i = 0; i < iNumLogSpGroupsUsed; i++) {
        for (j = 0; j <= 1; j++) {
          for (k = 1; k <= 5; k++) {
            sprintf( cLabel, "newloggroup%d%sdecay%d", i+1,p_cSizes[j],k);
            mp_iNewLogCalcsCode[i][j][k-1] = mp_oCalcGrid->RegisterFloat(cLabel);

            for ( t = 0; t < m_iMaxDecayTimesteps; t++ ) {
              sprintf( cLabel, "loggroup%d%sdecay%d_%d", i+1,p_cSizes[j],k, t );
              mp_iLogCalcsCode[i][j][k-1][t] = mp_oCalcGrid->RegisterFloat( cLabel );
    ...
    else //Calc grid was created.
    {
          ...
          || ( mp_oCalcGrid->GetLengthYCells() != mp_oSubstrateGrid->GetLengthYCells()) )
      {
        ...
        strcpy( stcErr.cFunction, "clDetailedSubstrate::SetupSubstrateGrid" );
        sprintf( stcErr.cMoreInfo, "%s%s%s", "Grid map values for grid ",
            "\"detailedsubstratecalcs\" contain cell lengths which do not match ",
            "the allowed values." );
        throw( stcErr );
      }
      ...
      if (-1 == m_iNewTipupCalcsCode) {
        ...
        strcpy( stcErr.cFunction, "clDetailedSubstrate::SetupSubstrateGrid" );
        sprintf( stcErr.cMoreInfo, "%s%s", "Grid map setup values for grid ",
            "\"substratecalcs\" did not contain all required grid data members." );
        throw( stcErr );
      }
      ...
      for (i = 0; i < iNumLogSpGroupsUsed; i++) {
        ...
        for (j = 0; j <= 1; j++) {
          ...
          for (k = 1; k <= 5; k++) {
            //mp_iLogCalcsCode[i][j][k-1] = new int [m_iMaxDecayTimesteps];
            sprintf( cLabel, "newloggroup%d%sdecay%d", i+1,p_cSizes[j],k);
            mp_iNewLogCalcsCode[i][j][k-1] = mp_oCalcGrid->GetFloatDataCode(cLabel);
            ...
            if (-1 == mp_iNewLogCalcsCode[i][j][k-1]) {
              ...
              strcpy( stcErr.cFunction, "clDetailedSubstrate::SetupSubstrateGrid" );
              sprintf( stcErr.cMoreInfo, "%s%s", "Grid map setup values for grid ",
                  "\"detailedsubstratecalcs\" did not contain all required grid data members." );
              throw( stcErr );
            }
            
            for ( t = 0; t < m_iMaxDecayTimesteps; t++ ) {
              sprintf( cLabel, "loggroup%d%sdecay%d_%d", i+1,p_cSizes[j],k, t );
              mp_iLogCalcsCode[i][j][k-1][t] = mp_oCalcGrid->GetFloatDataCode( cLabel );
    ...
    if ( mp_oHarvestGrid )
    {
      ...
      if ( m_iHarvestTypeCode == -1 )
      {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::SetupSubstrateGrid" );
        strcpy( stcErr.cMoreInfo, "Harvest grid set up in unexpected way." );
        throw( stcErr );
      }
    }
...
void clDetailedSubstrate::SetInitialSubstrate() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDetailedSubstrate::SetInitialSubstrate" );
    throw( stcErr );
  }
}
...
// -------------
// April 28, 2004 - Submitted in beta version (LEM)
// May 14, 2004 - Renamed this and pointed it at the "dead" tree data member
//                instead of "lgm" (LEM)
////////////////////////////////////////////////////////////////////////////
...
void clDetailedSubstrate::GetCodes() {
  try
  {
    ...
    char cDeadLabel[] = "dead", //labels used to tell what type of dead tree we have.
         cFallLabel[] = "Fall",
         cNewBreakLabel[] = "NewBreakHeight",
         cOldBreakLabel[] = "SnagOldBreakHeight";
    ...
    for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ ) {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clDetailedSubstrate::GetCodes" );
        strcpy( stcErr.cMoreInfo, "Detailed substrate cannot be applied to seedlings." );
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      }

      mp_iFallCodes[mp_whatSpeciesTypeCombos[i].iSpecies] = p_oPop->GetBoolDataCode( cFallLabel, mp_whatSpeciesTypeCombos[i].iSpecies, clTreePopulation::adult );

      mp_iOldBreakCodes[mp_whatSpeciesTypeCombos[i].iSpecies] = p_oPop->GetFloatDataCode( cOldBreakLabel, mp_whatSpeciesTypeCombos[i].iSpecies, clTreePopulation::snag );

      mp_iNewBreakCodes[mp_whatSpeciesTypeCombos[i].iSpecies][mp_whatSpeciesTypeCombos[i].iType] =
      p_oPop->GetFloatDataCode( cNewBreakLabel, mp_whatSpeciesTypeCombos[i].iSpecies, mp_whatSpeciesTypeCombos[i].iType );

      mp_iDeadCodes[mp_whatSpeciesTypeCombos[i].iSpecies] [mp_whatSpeciesTypeCombos[i].iType] =
      p_oPop->GetIntDataCode( cDeadLabel, mp_whatSpeciesTypeCombos[i].iSpecies, mp_whatSpeciesTypeCombos[i].iType );
...
void clDetailedSubstrate::Action() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDetailedSubstrate::Action" );
    throw( stcErr );
  }
}
...
void clDetailedSubstrate::AddHarvestSubstrate() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDetailedSubstrate::AddHarvestSubstrate" );
    throw( stcErr );
  }
}
...
void clDetailedSubstrate::CalcMainGridTotals() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDetailedSubstrate::CalcMainGridTotals" );
    throw( stcErr );
  }
}
...
// -------------
// April 28, 2004 - Submitted in beta version (LEM)
// November 10, 2005 - Changed decayed log decay from straight proportion to
// the same equation as all the other substrate types (LEM)
////////////////////////////////////////////////////////////////////////////
void clDetailedSubstrate::DecaySubstrate() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDetailedSubstrate::DecaySubstrate" );
    throw( stcErr );
  }
}
...
void clDetailedSubstrate::CalcVolumeAfterDecay() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDetailedSubstrate::CalcVolumeAfterDecay" );
    throw( stcErr );
  }
}
...
void clDetailedSubstrate::MortalitySubstrate() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDetailedSubstrate::MortalitySubstrate" );
    throw( stcErr );
  }
}
...
// -------------
// April 28, 2004 - Submitted in beta version (LEM)
// May 14, 2004 - Killed trees that fell (LEM)
// April 22, 2005 - Allowed snags to uproot with their own probability (LEM)
// May 26, 2005 - Allowed directional tree fall (LEM)
////////////////////////////////////////////////////////////////////////////
void clDetailedSubstrate::AddNewDeadTrees() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDetailedSubstrate::AddNewDeadTrees" );
    throw( stcErr );
  }
}
...
void clDetailedSubstrate::AdjustSubstrateForMortality() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clDetailedSubstrate::AdjustSubstrateForMortality" );
    throw( stcErr );
  }
}
