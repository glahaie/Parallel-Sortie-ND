...
clTreeAgeCalculator::clTreeAgeCalculator(clSimManager * p_oSimManager) :
  ...
  try
  {
    ...
    strcpy( m_cNameString, "TreeAgeCalculator" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clTreeAgeCalculator::clTreeAgeCalculator" );
    throw( stcErr );
  }
}
...
void clTreeAgeCalculator::GetData(xercesc::DOMDocument * p_oDoc) {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreeAgeCalculator::GetData" );
    throw( stcErr );
  }
}
...
void clTreeAgeCalculator::RegisterTreeDataMembers() {
  ...
  for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
    ...
    if (clTreePopulation::adult != mp_whatSpeciesTypeCombos[i].iType
      ...
      modelErr stcErr;
      strcpy(stcErr.cFunction, "clTreeAgeCalculator::RegisterTreeDataMembers");
...
void clTreeAgeCalculator::Action() {
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clTreeAgeCalculator::Action" );
    throw( stcErr );
  }
}
