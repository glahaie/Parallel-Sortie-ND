     ...
     clMortalityBase( p_oSimManager )
{
  try
  {
    ...
    strcpy( m_cNameString, "stochastic bilevel mortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clStochasticBiLevelMortality::clStochasticBiLevelMortality" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    if (m_bIsGLI) {
      ...
      for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
        ...
        strcpy( stcErr.cFunction, "clStochasticBiLevelMortality::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "A light behavior must be assigned to all trees for Stochastic Bi-Level Mortality." );
        throw( stcErr );
        }
      }
      ...
      else
      {
        ...
        stcErr.iErrorCode = CANT_FIND_OBJECT;
        strcpy( stcErr.cFunction, "clStochasticBiLevelMortality::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "\"Storm Light\" grid is missing." );
        throw( stcErr );
      }
    }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      if ( 0 > p_fTempValues[i].val || 1 < p_fTempValues[i].val )
      {
        ...
        strcpy( stcErr.cFunction, "clStochasticBiLevelMortality::DoShellSetup" );
        strcpy( stcErr.cMoreInfo, "All values for probability of mortality must be between 0 and 1." );
        throw( stcErr );
      }
...
deadCode clStochasticBiLevelMortality::DoMort( clTree * p_oTree, const float & fDiam, const short int & iSpecies )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clStochasticBiLevelMortality::DoMort" );
    throw( stcErr );
  }
}
...
// SetNameData()
////////////////////////////////////////////////////////////////////////////
void clStochasticBiLevelMortality::SetNameData(char *cNameString) {
 if (strcmp(cNameString, "StochasticBiLevelMortality - GLI") == 0)
   m_bIsGLI = true;
 else
   m_bIsGLI = false;
}
