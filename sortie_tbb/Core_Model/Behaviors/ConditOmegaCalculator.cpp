...
clConditOmegaCalculator::clConditOmegaCalculator( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "ConditsOmega" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clConditOmegaCalculator::clConditOmegaCalculator" );
    throw( stcErr );
  }
}
...
void clConditOmegaCalculator::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clConditOmegaCalculator::GetData" );
    throw( stcErr );
  }
}
...
void clConditOmegaCalculator::GetParameterFileData( xercesc::DOMDocument * p_oDoc )
{
  try
  {
    ...
    if ( m_fMaxDistance <= 0 )
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clConditOmegaCalculator::GetData" );
      strcpy( stcErr.cMoreInfo, "The max distance must be greater than zero." );
    ...
    if ( m_fIncrement <= 0 )
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clConditOmegaCalculator::GetData" );
      strcpy( stcErr.cMoreInfo, "The distance increment must be greater than zero." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
    ...
    if (m_fMaxDistance <= m_fIncrement) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clConditOmegaCalculator::GetData" );
      strcpy( stcErr.cMoreInfo, "The distance increment must be less than the max distance." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
  }
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clConditOmegaCalculator::GetParameterFileData" );
    throw( stcErr );
  }
}
...
void clConditOmegaCalculator::SetupGrid()
{
  ...
  for (i = 0; i < m_iNumTotalSpecies; i++) {
    for (j = 0; j < m_iNumIncs; j++) {
      sprintf(cLabel, "%d%s%d", j, "_", i);
      mp_iGridCodes[i][j] = mp_oGrid->RegisterFloat( cLabel );
    }
  }
...
void clConditOmegaCalculator::Action()
{
  ...
  catch ( ... )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clConditOmegaCalculator::Action" );
    throw( stcErr );
  }
}
