...
clWindstorm::clWindstorm( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "Windstorm" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clWindstorm::clWindstorm" );
    throw( stcErr );
  }
}
...
void clWindstorm::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clWindstorm::GetData" );
    throw( stcErr );
  }
}
  ...
  try {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      ...
      if (mp_fMinDBH[p_fTempValues[i].code] < 0) {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clWindstorm::ReadParFile" );
        strcpy( stcErr.cMoreInfo, "Minimum DBH for windstorms cannot be negative." );
    ...
    if (m_iFirstStormTimestep < 0) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clWindstorm::ReadParFile" );
      strcpy( stcErr.cMoreInfo, "Timestep to start windstorms cannot be negative." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
    ...
    if (fTemp < 0 || fTemp > 1) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clWindstorm::ReadParFile" );
      strcpy( stcErr.cMoreInfo, "Windstorm severities must be between 0 and 1." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
    ...
    {
      sprintf( cLabel, "%s%d", "ws_severityReturnInterval", iTemp );
      FillSingleValue( p_oElement, cLabel, &fTemp, true );
...
void clWindstorm::DoGridSetup()
{
  ...
  for ( i = 0; i < m_iTotalNumSpecies; i++ )
  {
    sprintf( cFloatLabel, "%s%d", "ba_", i );
    mp_iBa_xCodes[i] = mp_oResultsGrid->RegisterPackageFloat( cFloatLabel );
    sprintf( cFloatLabel, "%s%d", "density_", i );
    mp_iDensity_xCodes[i] = mp_oResultsGrid->RegisterPackageFloat( cFloatLabel );
  }
...
void clWindstorm::Action()
{
  ...
  catch ( ... )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clWindstorm::Action" );
    throw( stcErr );
  }
}
...
void clWindstorm::GetDeadCodes(clTreePopulation *p_oPop)
{
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    ...
    if (clTreePopulation::seedling == mp_whatSpeciesTypeCombos[i].iType) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clWindstorm::GetDeadCodes" );
      strcpy( stcErr.cMoreInfo, "Windstorm cannot be applied to seedlings." );
      throw( stcErr );
    }
    ...
    if (-1 == mp_iDeadCodes[mp_whatSpeciesTypeCombos[i].iSpecies]
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clWindstorm::GetDeadCodes" );
      strcpy( stcErr.cMoreInfo, "You must use tree mortality behaviors along with Windstorm." );
      throw( stcErr );
    }
  }
}
