...
clGrowthBase( p_oSimManager), clMichMenBase(p_oSimManager) {
  ...
  try {
    ...
    strcpy(m_cNameString, "absolutegrowthshell");
    m_iNewTreeInts = 2;
  }
    ...
    strcpy(stcErr.cFunction, "clAbsoluteGrowth::clAbsoluteGrowth");
    throw(stcErr);
  }
}
...
// SetNameData()
/////////////////////////////////////////////////////////////////////////////*/
void clAbsoluteGrowth::SetNameData(char *cNameString) {
  try {

    //Check the string passed and set the flags accordingly
    if (strcmp("AbsRadialGrowth", cNameString) == 0) {
      m_bConstRadialLimited = true;
      m_bConstBasalAreaLimited = false;
      m_iGrowthMethod = diameter_auto;
      m_sXMLRoot = "AbsRadialGrowth";
    } else if (strcmp("AbsBAGrowth", cNameString) == 0) {
      m_bConstRadialLimited = false;
      m_bConstBasalAreaLimited = true;
      m_iGrowthMethod = diameter_auto;
      m_sXMLRoot = "AbsBAGrowth";
    } else if (strcmp("AbsUnlimGrowth", cNameString) == 0){
      m_bConstRadialLimited = false;
      m_bConstBasalAreaLimited = false;
      m_iGrowthMethod = diameter_auto;
      m_sXMLRoot = "AbsUnlimGrowth";
    } else if (strcmp("AbsRadialGrowth diam only", cNameString) == 0) {
      m_bConstRadialLimited = true;
      m_bConstBasalAreaLimited = false;
      m_iGrowthMethod = diameter_only;
      m_sXMLRoot = "AbsRadialGrowth";
    } else if (strcmp("AbsBAGrowth diam only", cNameString) == 0) {
      m_bConstRadialLimited = false;
      m_bConstBasalAreaLimited = true;
      m_iGrowthMethod = diameter_only;
      m_sXMLRoot = "AbsBAGrowth";
    } else if (strcmp("AbsUnlimGrowth diam only", cNameString) == 0){
      m_bConstRadialLimited = false;
      m_bConstBasalAreaLimited = false;
      m_iGrowthMethod = diameter_only;
      m_sXMLRoot = "AbsUnlimGrowth";
      ...
      sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized behavior name \"",
          cNameString, "\".");
      strcpy(stcErr.cFunction, "clAbsoluteGrowth::SetNameData");
      throw(stcErr);
    }
  }
  catch (modelErr&err) {throw(err);}
  ...
  try {
    ...
    for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
      if (-1 == mp_oGrowthOrg->GetLightCode(mp_whatSpeciesTypeCombos[i].iSpecies,
        ...
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clAbsoluteGrowth::DoShellSetup" );
    ...
    *p_fLightDepMort;     //light dependent mortality - from par file
    ...
    for (i = 0; i < m_iNumBehaviorSpecies; i++) {
      ...
      if (0 < m_iYrsExceedThresholdBeforeSup * p_fMortAtZeroGrowth[i].val &&
        ...
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cMoreInfo, "Cannot calculate suppression thresholds.");
        strcpy(stcErr.cFunction, "clAbsoluteGrowth::CalculateSuppressionThresholds");
        throw(stcErr);
      }
...
void clAbsoluteGrowth::RegisterTreeDataMembers() {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clAbsoluteGrowth::RegisterTreeDataMembers");
    throw(stcErr);
  }
}
