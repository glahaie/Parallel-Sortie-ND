  ...
  clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clGrowthBase( p_oSimManager )
{
  try
  {
    strcpy( m_cNameString, "michmenneggrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clMichMenNegGrowth::clMichMenNegGrowth" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (p_fTempValues[i].val == 0) {
        ...
        strcpy(stcErr.cFunction, "clMichMenNegGrowth::DoShellSetup");
        strcpy(stcErr.cMoreInfo, "Beta values cannot equal 0.");
        throw(stcErr);
      }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (p_fTempValues[i].val < 0 || p_fTempValues[i].val > 1) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clMichMenNegGrowth::DoShellSetup");
        strcpy(stcErr.cMoreInfo, "Autocorrelation probability must be between 0 and 1.");
        throw(stcErr);
      }
...
float clMichMenNegGrowth::CalcHeightGrowthValue(clTree *p_oTree, clTreePopulation *p_oPop, float fDiameterGrowth) {
  ...
  return fAmountHeightIncrease;
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clMichMenNegGrowth::CalcHeightGrowthValue");
   throw(stcErr);
 }
}
