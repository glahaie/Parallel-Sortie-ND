...
clSizeDepLogisticGrowth::clSizeDepLogisticGrowth( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clGrowthBase( p_oSimManager )
{
  ...
  try
  {
    ...
    strcpy( m_cNameString, "sizedeplogisticgrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clSizeDepLogisticGrowth::clSizeDepLogisticGrowth" );
    throw( stcErr );
  }
}
  ...
  try
  {
    ...
    for (i = 0; i < m_iNumSpeciesTypeCombos; i++) {
      if (-1 == mp_oGrowthOrg->GetLightCode(mp_whatSpeciesTypeCombos[i].iSpecies,
        ...
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clSizeDepLogisticGrowth::DoShellSetup" );
...
// SetNameData()
/////////////////////////////////////////////////////////////////////////////*/
void clSizeDepLogisticGrowth::SetNameData(char *cNameString) {
  try {

    //Check the string passed and set the flags accordingly
    if (strcmp("SizeDependentLogisticGrowth", cNameString) == 0) {
      m_iGrowthMethod = diameter_auto;
    } else if (strcmp("SizeDependentLogisticGrowth diam only", cNameString) == 0) {
      m_iGrowthMethod = diameter_only;
    } else if (strcmp("SizeDependentLogisticGrowth height only", cNameString) == 0) {
      ...
      sprintf(stcErr.cMoreInfo, "%s%s%s", "Unrecognized behavior name \"",
          cNameString, "\".");
      strcpy(stcErr.cFunction, "clSizeDepLogisticGrowth::SetNameData");
      throw(stcErr);
    }
  }
...
float clSizeDepLogisticGrowth::CalculateFunctionValue(int iSpecies, float fGLI, float fDiam) {
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clSizeDepLogisticGrowth::CalculateFunctionValue");
    throw(stcErr);
  }
}
