...
clAggregatedMortality::clAggregatedMortality( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager ), clMortalityBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
    strcpy( m_cNameString, "aggregatedmortshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clAggregatedMortality::clAggregatedMortality" );
    throw( stcErr );
  }
}
...
void clAggregatedMortality::DoShellSetup( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clAggregatedMortality::DoShellSetup" );
    throw( stcErr );
  }
}
...
void clAggregatedMortality::ReadParFile( xercesc::DOMDocument * p_oDoc )
{
  ...
  if (m_fMortalityEpisodeProbability < 0) {
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clAggregatedMortality::ReadParFile" );
    strcpy( stcErr.cMoreInfo, "Return interval cannot be negative." );
  ...
  if (m_fMortalityProb < 0 || m_fMortalityProb > 1) {
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clAggregatedMortality::ReadParFile" );
    strcpy( stcErr.cMoreInfo, "Mortality probability must be between 0 and 1." );
    stcErr.iErrorCode = BAD_DATA;
    throw( stcErr );
  }
  ...
  if (m_iNumTreesToClump < 1) {
    modelErr stcErr;
    strcpy( stcErr.cFunction, "clAggregatedMortality::ReadParFile" );
    strcpy( stcErr.cMoreInfo, "Number of trees to aggregate must be greater than 0." );
    stcErr.iErrorCode = BAD_DATA;
    throw( stcErr );
  }
...
void clAggregatedMortality::KillClump(clTreePopulation *p_oPop, clTree *p_oTree, int iNumToKill) {
  ...
  do {
    ...
    sprintf( cQuery, "%s%f%s%f%s%f%s%f", "distance=", fDistance, "FROM x=",
             fTargetX, "y=", fTargetY, "::height=", fHeight );
...
void clAggregatedMortality::GetDeadCodes(clTreePopulation *p_oPop)
{
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    ...
    if (-1 == mp_iDeadCodes[mp_whatSpeciesTypeCombos[i].iSpecies]
      ...
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cFunction, "clAggregatedMortality::GetDeadCodes" );
      strcpy( stcErr.cMoreInfo, "Panic!  Can't get dead data member." );
      throw( stcErr );
    }
  }
}
