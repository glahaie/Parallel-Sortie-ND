...
clNeighborhoodSeedPredation::clNeighborhoodSeedPredation(
  ...
  try
  {
    strcpy( m_cNameString, "NeighborhoodSeedPredation");
  ...
  catch (...)
  {
    ...
    strcpy(stcErr.cFunction, "clNeighborhoodSeedPredation::clNeighborhoodSeedPredation");
    throw(stcErr);
  }
}
...
void clNeighborhoodSeedPredation::GetData(DOMDocument * p_oDoc) {
  ...
  if (m_bIsLinked) {
    if (NULL == mp_oSimManager->GetBehaviorObject("LinkedFunctionalResponseSeedPredation")) {
      ...
      strcpy( stcErr.cFunction, "clNeighborhoodSeedPredation::GetData" );
      strcpy( stcErr.cMoreInfo, "You must use the functional response seed predation behavior." );
      throw( stcErr );
    }
  }
}
...
// SetNameData()
//////////////////////////////////////////////////////////////////////////////
void clNeighborhoodSeedPredation::SetNameData(char * cNameString) {

  //Check the string passed and set the flags accordingly
  if (strcmp("LinkedNeighborhoodSeedPredation", cNameString) == 0) {
    m_bIsLinked = true;
    strcpy(m_cNameString, "NeighborhoodSeedPredation");
  } else {
    m_bIsLinked = false;
...
void clNeighborhoodSeedPredation::DoStandalonePredation() {
  ...
  catch ( ... )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clNeighborhoodSeedPredation::DoStandalonePredation" );
    throw( stcErr );
  }
...
void clNeighborhoodSeedPredation::DoLinkedPredation() {
  ...
  catch ( ... )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clNeighborhoodSeedPredation::DoLinkedPredation" );
    throw( stcErr );
  }
}
...
void clNeighborhoodSeedPredation::SetupGrids()
{
  try
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clNeighborhoodSeedPredation::SetupGrids" );
      strcpy( stcErr.cMoreInfo, "Disperse behaviors must be used with neighborhood seed predation." );
    ...
    if (mp_oOutputGrid == NULL) {
      ...
      {
        sprintf(cLabel, "%s%d", "startseeds_", i);
        mp_iStartSeedsCode[i] = mp_oOutputGrid->RegisterFloat(cLabel);
      }
      for (i = 0; i < iTotalSpecies; i++)
      {
        sprintf(cLabel, "%s%d", "propeaten_", i);
        mp_iPropEatenCode[i] = mp_oOutputGrid->RegisterFloat(cLabel);
      }
      ...
      if (fabs(mp_oOutputGrid->GetLengthXCells() - mp_oSeedGrid->GetLengthXCells()) > 0.001 ||
        ...
        {
          sprintf(cLabel, "%s%d", "startseeds_", i);
          mp_iStartSeedsCode[i] = mp_oOutputGrid->GetFloatDataCode(cLabel);
          sprintf(cLabel, "%s%d", "propeaten_", i);
          mp_iPropEatenCode[i] = mp_oOutputGrid->GetFloatDataCode(cLabel);
        }
      }
    }
  ...
  catch (...)
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clDisperseBase::SetUpBase");
    throw(stcErr);
  }
}
...
void clNeighborhoodSeedPredation::ReadParameterFileData(xercesc::DOMDocument *p_oDoc) {
  ...
  try
  {
    ...
    if (m_bIsLinked) {
      ...
      for ( i = 0; i < iNumSpecies; i++ )
      {
        sprintf( cName, "%s%s", "pr_neighPredPn", p_oPop->TranslateSpeciesCodeToName( i ));
        FillSpeciesSpecificValue( p_oElement, cName, "pr_nppnVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true );
      ...
      for ( i = 0; i < iNumSpecies; i++ )
      {
        sprintf( cName, "%s%s", "pr_neighPredMastingPn", p_oPop->TranslateSpeciesCodeToName( i ));
        FillSpeciesSpecificValue( p_oElement, cName, "pr_npmpnVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true );
        for ( j = 0; j < m_iNumBehaviorSpecies; j++ )
        {
      ...
      for ( i = 0; i < iNumSpecies; i++ )
      {
        sprintf( cName, "%s%s", "pr_neighPredNonMastingPn", p_oPop->TranslateSpeciesCodeToName( i ));
        FillSpeciesSpecificValue( p_oElement, cName, "pr_npnmpnVal", p_fTemp, m_iNumBehaviorSpecies, p_oPop, true );
        for ( j = 0; j < m_iNumBehaviorSpecies; j++ )
        {
          mp_fNonMastingPn[j][i] = p_fTemp[j].val;
        }
      ...
      if (m_bUseThresholdToDecideMast) {
        ...
        {
          modelErr stcErr;
          strcpy( stcErr.cFunction, "clNeighborhoodSeedPredation::ReadParameterFileData" );
          strcpy( stcErr.cMoreInfo, "Masting seed density threshold cannot be negative." );
          stcErr.iErrorCode = BAD_DATA;
          throw( stcErr );
        }
    ...
    for ( i = 0; i < iNumSpecies; i++ ) {
      if ( mp_fMinNeighDBH[i] < 0 )
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clNeighborhoodSeedPredation::ReadParameterFileData" );
        strcpy( stcErr.cMoreInfo, "Minimum neighbor DBH cannot be negative." );
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      }
    }
    ...
    if ( m_fRadius < 0 )
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clNeighborhoodSeedPredation::ReadParameterFileData" );
      strcpy( stcErr.cMoreInfo, "Neighborhood radius cannot be negative." );
      stcErr.iErrorCode = BAD_DATA;
      throw( stcErr );
    }
