  ...
  clGrowthBase( p_oSimManager )
{
  ...
  {
    //Set namestring
    strcpy( m_cNameString, "WeibullClimateQuadratgrowthshell" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clWeibullClimateQuadratGrowth::clWeibullClimateQuadratGrowth" );
    throw( stcErr );
  }
}
...
void clWeibullClimateQuadratGrowth::ValidateData(clTreePopulation *p_oPop)
{
  try
  {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "Temperature effect A values cannot be 0." );
        throw( stcErr );
      }
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "Precipitation effect A values cannot be 0." );
        throw( stcErr );
      }
    }
  ...
  {
    strcpy( err.cFunction, "clWeibullClimateQuadratGrowth::ValidateData" );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clWeibullClimateQuadratGrowth::ValidateData" );
    throw( stcErr );
  }
}
...
void clWeibullClimateQuadratGrowth::SetNameData( char * cNameString )
{
  try
  {
    ...
    if ( strcmp( "WeibullClimateQuadratGrowth", cNameString ) == 0 )
    {
      m_iGrowthMethod = diameter_auto;
    }
    else if ( strcmp( "WeibullClimateQuadratGrowth diam only", cNameString ) == 0 )
    {
    ...
    else
    {
      ...
      strcpy( stcErr.cFunction, "clWeibullClimateQuadratGrowth::SetNameData" );
      throw( stcErr );
    }
  }
  catch ( modelErr & err )
...
float clWeibullClimateQuadratGrowth::CalcDiameterGrowthValue( clTree * p_oTree, clTreePopulation * p_oPop, float fHeightGrowth )
{
  ...
  if (fGrowth < -1 ) {
    modelErr stcErr;
    strcpy( stcErr.cMoreInfo, "Panic: expected growth not calculated." );
    strcpy( stcErr.cFunction, "clWeibullClimateQuadratGrowth::CalcDiameterGrowthValue" );
    stcErr.iErrorCode = BAD_DATA;
    throw( stcErr );
  }
...
void clWeibullClimateQuadratGrowth::PreGrowthCalcs( clTreePopulation * p_oPop )
{
  ...
  catch ( ... )
  {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clWeibullClimateQuadratGrowth::PreCalcGrowth" );
    throw( stcErr );
  }
}
...
void clWeibullClimateQuadratGrowth::SetupGrid(clTreePopulation *p_oPop)
{
  ...
  if (NULL == mp_oGrid) {
    ...
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "growth_", i);
      mp_iGridGrowthCodes[i] = mp_oGrid->RegisterFloat( cLabel );
    }
    ...
    for (i = 0; i < iNumTotalSpecies; i++) {
      sprintf(cLabel, "%s%d", "growth_", i);
      mp_iGridGrowthCodes[i] = mp_oGrid->GetFloatDataCode( cLabel );
      if (-1 == mp_iGridGrowthCodes[i]) {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clWeibullClimateQuadratGrowth::SetupGrid" );
      ...
      if (-1 == m_iNumNeighCode) {
        ...
        strcpy( stcErr.cMoreInfo,
          "Couldn't find the \"num_neigh\" member of the \"Weibull Climate Quadrat Growth\" grid.");
        stcErr.iErrorCode = BAD_DATA;
        throw( stcErr );
      }
    }
