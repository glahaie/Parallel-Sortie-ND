...
clCompetitionHarvest::clCompetitionHarvest( clSimManager * p_oSimManager ) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  try
  {
    strcpy( m_cNameString, "CompetitionHarvest" );
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clCompetitionHarvest::clCompetitionHarvest" );
    throw( stcErr );
  }
}
...
void clCompetitionHarvest::RegisterTreeDataMembers()
{
  ...
  for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ )
  {
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clCompetitionHarvest::RegisterTreeDataMembers" );
                                                  ...
                                                  clTreePopulation *p_oPop )
{
  ...
  try
  {
    ...
    for (i = 0; i < m_iNumBehaviorSpecies; i++) {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cFunction, "clCompetitionHarvest::ReadParameterFileData" );
        stcErr.iErrorCode = BAD_DATA;
    ...
    if (m_bIsSpeciesSpecific) {
      ...
      if (fabs(fTemp - 1.0) > 0.0001) {
        ...
        strcpy( stcErr.cFunction, "clCompetitionHarvest::ReadParameterFileData" );
        stcErr.iErrorCode = BAD_DATA;
        strcpy( stcErr.cMoreInfo, "Proportion to cut for different species must add up to 1." );
        throw( stcErr );
      }
    }
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ )
    {
      sprintf( cName, "%s%s%s", "di_compHarv",
                  p_oPop->TranslateSpeciesCodeToName( mp_iWhatSpecies[i] ), "NeighborLambda" );
      FillSpeciesSpecificValue( p_oElement, cName, "di_nlVal",
    ...
    if (interval == iTemp) {
      ...
      strcpy( stcErr.cFunction, "clCompetitionHarvest::ReadParameterFileData" );
      stcErr.iErrorCode = BAD_DATA;
      sprintf( stcErr.cMoreInfo, "%s%d%s", "Unknown harvest type \"", iTemp, "\"." );
      throw( stcErr );
    }
    ...
    if (ba_prop == harvest && (m_fAmtToCut < 0 || m_fAmtToCut > 1)) {
      ...
      strcpy( stcErr.cFunction, "clCompetitionHarvest::ReadParameterFileData" );
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cMoreInfo, "The amount to cut must be a proportion between 0 and 1." );
      throw( stcErr );
    } else if (m_fAmtToCut < 0) {
      modelErr stcErr;
      strcpy( stcErr.cFunction, "clCompetitionHarvest::ReadParameterFileData" );
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cMoreInfo, "The amount to cut must be greater than 0." );
      throw( stcErr );
    }
    ...
    if (m_iInterval < 1) {
      ...
      strcpy( stcErr.cFunction, "clCompetitionHarvest::ReadParameterFileData" );
      stcErr.iErrorCode = BAD_DATA;
      strcpy( stcErr.cMoreInfo, "The harvest interval must be greater than 0." );
      throw( stcErr );
    }
    ...
    cName[0] = '\0';
    FillSingleValue( p_oElement, "di_compHarvHarvestedListFile", cName,
                     MAX_FILENAME_SIZE, false );
    if ('\0' != cName[0]) {
      m_sHarvestListFilename = cName;
    }
...
void clCompetitionHarvest::GetData( xercesc::DOMDocument * p_oDoc )
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clCompetitionHarvest::GetData" );
    throw( stcErr );
  }
}
...
void clCompetitionHarvest::SetupCOEGrids(clTreePopulation * p_oPop)
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clCompetitionHarvest::SetupCOEGrids" );
    throw( stcErr );
  }
}
...
void clCompetitionHarvest::SetupGrids()
{
  try
  {
    ...
    if (NULL == mp_oResultsGrid) {
      ...
      for ( i = 0; i < m_iNumSpecies; i++ )
      {
        sprintf( cLabel, "%s%d", "Cut Density_", i );
        mp_iDenCutCodes[i] = mp_oResultsGrid->RegisterInt( cLabel );
        sprintf( cLabel, "%s%d", "Cut Basal Area_", i );
        mp_iBaCutCodes[i] = mp_oResultsGrid->RegisterFloat( cLabel );
      }
    } else {
      ...
      for (i = 0; i < m_iNumSpecies; i++) {
        sprintf( cLabel, "%s%d", "Cut Density_", i );
        mp_iDenCutCodes[i] = mp_oResultsGrid->GetFloatDataCode( cLabel );
        if (-1 == mp_iDenCutCodes[i]) {
          modelErr stcErr;
          strcpy( stcErr.cFunction, "clCompetitionHarvest::SetupGrids" );
          sprintf( stcErr.cMoreInfo, "%s%s%s", "Couldn't find the \"", cLabel,
        ...
        sprintf( cLabel, "%s%d", "Cut Basal Area_", i );
        mp_iBaCutCodes[i] = mp_oResultsGrid->GetFloatDataCode( cLabel );
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clCompetitionHarvest::SetupHarvestGrid" );
    throw( stcErr );
  }
}
...
void clCompetitionHarvest::Action()
{
  ...
  catch ( ... )
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy( stcErr.cFunction, "clCompetitionHarvest::Action" );
    throw( stcErr );
  }
}
...
float clCompetitionHarvest::CalculateOneCOE(clPlot *p_oPlot, clTreePopulation *p_oPop, clTree *p_oTree)
{
  ...
  sprintf( cQuery, "%s%f%s%f%s%f%s%f", "distance=",
            mp_fMaxCrowdingRadius[iTargetSp], "FROM x=", fTargetX,
