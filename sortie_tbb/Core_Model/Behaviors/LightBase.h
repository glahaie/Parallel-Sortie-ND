...
//note: need the virtual keyword to avoid base class ambiguity.
  ...
  */
  ...
  /*
   * For parallel implementation: creates a new mp_fPhoto matrix
   */
  private:

  // creates a local matrix and initializes all values to 1.0
  float** createMPPhotoMatrix (int m_iNumAltAng, int m_iNumAziAng) {

      float** mp_ffPhoto = new float*[m_iNumAltAng];
      for(int i = 0; i < m_iNumAltAng; ++i) {
          mp_ffPhoto[i] = new float[m_iNumAziAng];
          for(int j = 0; j < m_iNumAziAng; ++j) {
              mp_ffPhoto[i][j] = 1.0;
          }
      }
      return mp_ffPhoto;
  ...
  void deleteMPPhotoMatrix(float **mp_ffPhoto, int m_iNumAltAng) {
      for(int i = 0; i < m_iNumAltAng;++i) {
          delete[] mp_ffPhoto[i];
      }
      delete[] mp_ffPhoto;
  ...
  friend class clGLiLight;
  friend class clGLIMap;
  friend class clQuadractGLILight;
  friend class clSailLight;
  friend class clLightDepSeedSurvival;
