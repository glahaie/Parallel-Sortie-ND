...
clNCIJuvenileGrowth::clNCIJuvenileGrowth(clSimManager * p_oSimManager) :
  ...
  catch ( ... )
  {
    ...
    strcpy( stcErr.cFunction, "clNCIJuvenileGrowth::clNCIJuvenileGrowth" );
    throw( stcErr );
  }
}
...
void clNCIJuvenileGrowth::ReadParameterFile(xercesc::DOMDocument * p_oDoc) {
  try
  {
        ...
        && clTreePopulation::seedling != mp_whatSpeciesTypeCombos[i].iType )
    {
      ...
      strcpy( stcErr.cMoreInfo, "This behavior can only be applied to seedlings and saplings." );
      throw( stcErr );
    }
    ...
    for ( i = 0; i < iNumTotalSpecies; i++ )
    {
      ...
      FillSpeciesSpecificValue( p_oElement, cName, "gr_jnlVal", p_fTempValues,
...
void clNCIJuvenileGrowth::ValidateData() {
  try
  {
    ...
    for ( i = 0; i < iNumTotalSpecies; i++ )
    {
      ...
      {
        modelErr stcErr;
        strcpy( stcErr.cMoreInfo, "Minimum neighbor Diam10 for NCI cannot be less than 0." );
        throw( stcErr );
      }
    }
    ...
    {
      modelErr stcErr;
      strcpy( stcErr.cMoreInfo, "The NCI Diam10 divisor must be greater than 0." );
      throw( stcErr );
    }
  }
  catch ( modelErr & err )
...
void clNCIJuvenileGrowth::SetNameData(char * cNameString) {
  ...
  if ( strcmp( "NCIJuvenileGrowth", cNameString ) == 0 )
  {
    m_iGrowthMethod = diameter_auto;
  }
  else if ( strcmp( "NCIJuvenileGrowth diam only", cNameString ) == 0 )
  {
