...
clHeightGLIWeibullMortality::clHeightGLIWeibullMortality(clSimManager *p_oSimManager) :
 ...
 try {
   ...
   strcpy(m_cNameString, "height gli weibull mortshell");
   ...
   strcpy(stcErr.cFunction, "clHeightGLIWeibullMortality::clHeightGLIWeibullMortality");
   throw(stcErr);
 }
}
...
void clHeightGLIWeibullMortality::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
  try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clHeightGLIWeibullMortality::DoShellSetup");
   throw(stcErr);
 }
}
  ...
  try {
    ...
    for ( i = 0; i < m_iNumBehaviorSpecies; i++ ) {
      if (p_fTempValues[i].val < 0 || p_fTempValues[i].val > 1) {
        ...
        strcpy(stcErr.cFunction, "clHeightGLIWeibullMortality::ReadParameterFileData");
        strcpy(stcErr.cMoreInfo, "All values in max mortality must be between 0 and 1.");
        throw(stcErr);
      }
  ...
  try
  {
    ...
    for ( i = 0; i < m_iNumSpeciesTypeCombos; i++ ) {
      ...
      if (-1 == mp_iLightCodes[mp_iIndexes[iSp]][iTp]) {
        modelErr stcErr;
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clHeightGLIWeibullMortality::GetTreeDataMemberCodes");
        strcpy(stcErr.cMoreInfo, "All trees that use Height GLI Mortality must also use a light behavior.");
        throw(stcErr);
      }
