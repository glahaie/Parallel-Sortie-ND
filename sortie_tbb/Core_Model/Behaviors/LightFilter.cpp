...
clLightFilter::clLightFilter(clSimManager * p_oSimManager) : clWorkerBase( p_oSimManager ), clBehaviorBase( p_oSimManager )
{
  ...
  {
    //Set the namestring
    strcpy(m_cNameString, "LightFilter");
  ...
  catch (...)
  {
    ...
    strcpy(stcErr.cFunction, "clLightFilter::clLightFilter");
    throw(stcErr);
  }
}
...
void clLightFilter::GetData(xercesc::DOMDocument * p_oDoc)
{
  try
  {
    ...
    for (i = 0; i < m_iNumSpeciesTypeCombos; i++)
    {
      ...
      {
        modelErr stcErr;
        strcpy(stcErr.cFunction, "clLightFilter::GetData");
        sprintf(stcErr.cMoreInfo, "%s%d%s%d%s", "Type/species combo species=", mp_whatSpeciesTypeCombos[i].iSpecies,
             " type=", mp_whatSpeciesTypeCombos[i].iType, " does not have a light behavior compatible with light filtering.");
...
void clLightFilter::Action()
{
  ...
  catch (...)
  {
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clLightFilter::Action");
    throw(stcErr);
  }
}
