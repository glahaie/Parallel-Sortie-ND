...
clBCMort::clBCMort(clSimManager *p_oSimManager) :
 ...
 try {
   ...
   strcpy(m_cNameString, "bcmortshell");
   ...
   strcpy(stcErr.cFunction, "clBCMort::clBCMort");
   throw(stcErr);
 }
}
...
void clBCMort::DoShellSetup(xercesc::DOMDocument *p_oDoc) {
 try {
   ...
   modelErr stcErr;
   stcErr.iErrorCode = UNKNOWN;
   strcpy(stcErr.cFunction, "clBCMort::DoShellSetup");
   throw(stcErr);
 }
}
     ...
     for (j = 0; j < iTotalTypes; j++)
       if (mp_bUsesThisMortality[i][j]) {
         ...
         if (-1 == mp_iGrowthCodes[i][j]) {
           modelErr stcErr;
           strcpy(stcErr.cFunction, "clBCMort::GetGrowthVariableCodes");
