...
class clSimManager {
  ...
  char m_cAppPath[MAX_FILENAME_SIZE]; /**<Path to this application, terminated by
                                      path separator.  Not a const
                                      because we have to set this at runtime*/


  long m_iActualSeed; /**<The actual random seed that was used.  This is
             only needed in case random seed in the parameter file is zero -
             this allows capture for re-creation if needed.*/


  float m_fNumYearsPerTimestep; /**<Number of years per timestep - could be
  fractional or less than 1*/

  //Versions
  int m_iMajorVersion; /**<Model's major version number.  For version control.*/
  int m_iMinorVersion; /**<Model's minor version number.  For version contro.*/
  int m_iRandomSeed;  /**<Seed for the random number generator.
               This is read from the parameter file.  It must be negative; if
               entered as a positive, it's negativized.  If it's 0, a negative
               integer is chosen.*/

  int m_iNumTimesteps; /**<Number of timesteps for the run*/
  int m_iCurrentTimestep; /**<Timestep that the model is currently on.
               If the model is paused this is the timestep just finished.*/
  int m_iTargetTimestep; /**<If the model was paused this holds
               the original target timestep so it can resume.*/
  int m_iBatchGroup; /**<If this is a batch run, this is the ith parameter
               file group of the current batch file.  If not in batch mode,
               this is 0.*/
  int m_iBatchNumber; /**<If this is a batch run, this is the ith run of
               the current parameter file.  If not in batch mode, this is 0.*/
  ...
  //XML tools
  xercesc::DOMBuilder *mp_oXMLParser; /**<Xerces file parser.*/
  clXercesErrorHandler *mp_oXMLErrorHandler; /**<Custom error handler for Xerces
          file parser.*/
  ...
  * @param cAppPath Path of the application, sized a max of MAX_FILENAME_SIZE
  * and ending with the path separator.
  */
  clSimManager(int iMajorVersion, int iMinorVersion, const char *cAppPath);
  ...
  * @return Path to the application, sized MAX_FILENAME_SIZE and ending with the
  * path separator.
  */
  char* GetAppPath() {return m_cAppPath;};
  ...
  */
  char* GetParFilename() {return m_cParFilename;};
  ...
  *
  * @param cFileName Path and name of file to read.
  */
  void ReadFile(const char *cFileName);
  ...
  */
  fileType GetFileType(const char *cFileName);
  ...
  *
  * @param cPopName Population's namestring.
  ...
  */
  clPopulationBase* GetPopulationObject(const char *cPopName);
  ...
  * Returns a behavior object.
  *
  * @param cBehaviorName Behavior's namestring.
  * @return The behavior object, or NULL if there is no such index.  For most
  * purposes, it will be necessary to cast the pointer to the particular
  * behavior class desired.
  */
  clBehaviorBase* GetBehaviorObject(const char *cBehaviorName);
  ...
  *
  * @param cBatchFile File path and name to the batch file to execute.
  */
  void RunBatch(const char *cBatchFile);
  ...
  * creating a new one.
  *
  * @param cGridName The new grid's namestring.
  ...
  */
clGrid* CreateGrid(const char* cGridName, short int iNumIntVals,
