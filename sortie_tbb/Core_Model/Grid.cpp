...
// Constructor
//////////////////////////////////////////////////////////////////////////////
clGrid::clGrid(clSimManager *p_oSimManager, const char *cGridName,
    short int iNumIntVals, short int iNumFloatVals, short int iNumCharVals,
  ...
  try {
    ...
    if (strlen(cGridName) == 0) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::clGrid");
      strcpy(stcErr.cMoreInfo, "Grid name must be supplied.");
      throw(stcErr);
    }
    if (fXCellLength < 0 || fYCellLength < 0) {
      ...
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::clGrid");
      sprintf(stcErr.cMoreInfo, "%s%g%s%g",
          "Either X or Y grid cell length is less than 0.  X = ", fXCellLength,
          " Y = ",  fYCellLength);
      throw(stcErr);
    }
    if (iNumIntVals < 0 || iNumFloatVals < 0 || iNumCharVals < 0 ||
        iNumBoolVals < 0) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::clGrid");
      strcpy(stcErr.cMoreInfo, "Data member numbers cannot be negative.");
      throw(stcErr);
    }

    //Take the data we were passed and assign it
    p_oPlot = mp_oSimManager->GetPlotObject();
    strncpy(m_cNameString, cGridName, MAX_NAMESTRING_SIZE - 1);
    //This doesn't always happen with strncpy - make it explicit
    m_cNameString[MAX_NAMESTRING_SIZE - 1] = '\0';
    if (0 == fXCellLength)
    ...
    for (i = 0; i < m_iNumXCells; i++) {
      ...
      for (j = 0; j < m_iNumYCells; j++) {
        ...
        //Char data members
        if (m_iNumCharVals == 0) mp_gridVals[i][j].p_cCharVals = NULL;
        else {
          mp_gridVals[i][j].p_cCharVals = new char*[m_iNumCharVals];
          for (k = 0; k < m_iNumCharVals; k++) {
            mp_gridVals[i][j].p_cCharVals[k] =
                new char[MAX_GRID_CHAR_VALUE_SIZE];
            strcpy(mp_gridVals[i][j].p_cCharVals[k], "");
          }
        }
        //Bool data members
    ...
    //Label arrays
    if (0 == m_iNumIntVals) mp_cIntLabels = NULL;
    else {
      mp_cIntLabels = new char*[m_iNumIntVals];
      for (i = 0; i < m_iNumIntVals; i++) {
        mp_cIntLabels[i] = new char[MAX_GRID_LABEL_SIZE];
        strcpy(mp_cIntLabels[i], "");
      }
    }
    if (0 == m_iNumFloatVals) mp_cFloatLabels = NULL;
    else {
      mp_cFloatLabels = new char*[m_iNumFloatVals];
      for (i = 0; i < m_iNumFloatVals; i++) {
        mp_cFloatLabels[i] = new char[MAX_GRID_LABEL_SIZE];
        strcpy(mp_cFloatLabels[i], "");
      }
    }
    if (0 == m_iNumCharVals) mp_cCharLabels = NULL;
    else {
      mp_cCharLabels = new char*[m_iNumCharVals];
      for (i = 0; i < m_iNumCharVals; i++) {
        mp_cCharLabels[i] = new char[MAX_GRID_LABEL_SIZE];
        strcpy(mp_cCharLabels[i], "");
      }
    }
    if (0 == m_iNumBoolVals) mp_cBoolLabels = NULL;
    else {
      mp_cBoolLabels = new char*[m_iNumBoolVals];
      for (i = 0; i < m_iNumBoolVals; i++) {
        mp_cBoolLabels[i] = new char[MAX_GRID_LABEL_SIZE];
        strcpy(mp_cBoolLabels[i], "");
      }
    }
    ...
    m_iNumPackageCharVals = m_iNumCharVals;
    m_iNumPackageBoolVals = m_iNumBoolVals;
    if (0 == m_iNumIntVals) mp_cPackageIntLabels = NULL;
    else {
      mp_cPackageIntLabels = new char*[m_iNumIntVals];
      for (i = 0; i < m_iNumIntVals; i++) {
        mp_cPackageIntLabels[i] = new char[MAX_GRID_LABEL_SIZE];
        strcpy(mp_cPackageIntLabels[i], "");
      }
    }
    if (0 == m_iNumFloatVals) mp_cPackageFloatLabels = NULL;
    else {
      mp_cPackageFloatLabels = new char*[m_iNumFloatVals];
      for (i = 0; i < m_iNumFloatVals; i++) {
        mp_cPackageFloatLabels[i]= new char[MAX_GRID_LABEL_SIZE];
        strcpy(mp_cPackageFloatLabels[i], "");
      }
    }
    if (0 == m_iNumCharVals) mp_cPackageCharLabels = NULL;
    else {
      mp_cPackageCharLabels = new char*[m_iNumCharVals];
      for (i = 0; i < m_iNumCharVals; i++) {
        mp_cPackageCharLabels[i] = new char[MAX_GRID_LABEL_SIZE];
        strcpy(mp_cPackageCharLabels[i], "");
      }
    }
    if (0 == m_iNumBoolVals) mp_cPackageBoolLabels = NULL;
    else {
      mp_cPackageBoolLabels = new char*[m_iNumBoolVals];
      for (i = 0; i < m_iNumBoolVals; i++) {
        mp_cPackageBoolLabels[i] = new char[MAX_GRID_LABEL_SIZE];
        strcpy(mp_cPackageBoolLabels[i], "");
      }
    }
...
void clGrid::ChangePackageDataStructure(short int iNumIntVals,
  ...
  try {
    ...
    if (!m_bPackageChangeAllowed) {
      ...
      strcpy(stcErr.cFunction, "clGrid::ChangePackageDataStructure");
      sprintf(stcErr.cMoreInfo, "%s%s", "Package data structure may not be ",
          "changed after packages have been created.");
      throw(stcErr);
    }

    //Delete all the old label arrays
    if (mp_cPackageIntLabels) {
      for (i = 0; i < m_iNumIntVals; i++)
        delete[] mp_cPackageIntLabels[i];
      delete[] mp_cPackageIntLabels;
    }
    mp_cPackageIntLabels = NULL;

    if (mp_cPackageFloatLabels) {
      for (i = 0; i < m_iNumFloatVals; i++)
        delete[] mp_cPackageFloatLabels[i];
      delete[] mp_cPackageFloatLabels;
    }
    mp_cPackageFloatLabels = NULL;

    if (mp_cPackageCharLabels) {
      for (i = 0; i < m_iNumCharVals; i++)
        delete[] mp_cPackageCharLabels[i];
      delete[] mp_cPackageCharLabels;
    }
    mp_cPackageCharLabels = NULL;

    if (mp_cPackageBoolLabels) {
      for (i = 0; i < m_iNumBoolVals; i++)
        delete[] mp_cPackageBoolLabels[i];
      delete[] mp_cPackageBoolLabels;
    }
    mp_cPackageBoolLabels = NULL;

    //Assign the new variables
    m_iNumPackageIntVals = iNumIntVals;
    m_iNumPackageFloatVals = iNumFloatVals;
    m_iNumPackageCharVals = iNumCharVals;
    m_iNumPackageBoolVals = iNumBoolVals;

    //Declare the arrays
    if (0 == m_iNumPackageIntVals) mp_cPackageIntLabels = NULL;
    else {
      mp_cPackageIntLabels = new char*[iNumIntVals];
      for (i = 0; i < iNumIntVals; i++) {
        mp_cPackageIntLabels[i] = new char[MAX_GRID_LABEL_SIZE];
        strcpy(mp_cPackageIntLabels[i], "");
      }
    }
    if (0 == m_iNumPackageFloatVals) mp_cPackageFloatLabels = NULL;
    else {
      mp_cPackageFloatLabels = new char*[iNumFloatVals];
      for (i = 0; i < iNumFloatVals; i++) {
        mp_cPackageFloatLabels[i]= new char[MAX_GRID_LABEL_SIZE];
        strcpy(mp_cPackageFloatLabels[i], "");
      }
    }
    if (0 == m_iNumPackageCharVals) mp_cPackageCharLabels = NULL;
    else {
      mp_cPackageCharLabels = new char*[iNumCharVals];
      for (i = 0; i < iNumCharVals; i++) {
        mp_cPackageCharLabels[i] = new char[MAX_GRID_LABEL_SIZE];
        strcpy(mp_cPackageCharLabels[i], "");
      }
    }
    if (0 == m_iNumPackageBoolVals) mp_cPackageBoolLabels = NULL;
    else {
      mp_cPackageBoolLabels = new char*[iNumBoolVals];
...
clPackage* clGrid::CreatePackage(
  ...
  if (NULL == p_oPreviousPackage) {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::CreatePackage(clPackage)");
    strcpy(stcErr.cMoreInfo, "Pointer passed cannot be NULL.");
    throw(stcErr);
  }
...
clPackage* clGrid::CreatePackageOfCell(int iX, int iY) {
  ...
  if (iX < 0 || iX >= m_iNumXCells || iY < 0 || iY >= m_iNumYCells) {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::CreatePackageOfGrid");
    sprintf(stcErr.cMoreInfo, "%s%d%s%d", "Invalid grid numbers.  X = ", iX,
        ", Y = ", iY);
    throw(stcErr);
  }
...
clPackage* clGrid::CreatePackageAtPoint(float fX, float fY) {
  ...
  if (fX >= m_fXPlotLength || fX < 0
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::CreatePackageAtPoint");
    sprintf(stcErr.cMoreInfo, "%s%g%s%g",
        "At least one grid coordinate is outside of grid - X = ", fX, " & Y = ",
        fY);
    throw(stcErr);
  }
...
clPackage* clGrid::GetFirstPackageOfCell(int iX, int iY) {
  ...
  if (iX < 0 || iX >= m_iNumXCells || iY < 0 || iY >= m_iNumYCells) {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::GetFirstPackageOfCell");
    sprintf(stcErr.cMoreInfo, "%s%s%s%d%s%d","Invalid grid numbers for grid ",
        m_cNameString, ".  X = ", iX, ", Y = ", iY);
    throw(stcErr);
  }
...
clPackage* clGrid::GetFirstPackageAtPoint(float fX, float fY) {
  ...
  if (fX >= m_fXPlotLength || fX < 0
    ...
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::GetFirstPackageAtPoint");
    sprintf(stcErr.cMoreInfo, "%s%s%s%g%s%g",
        "At least one grid coordinate is outside of grid ", m_cNameString,
        " - X = ", fX, " & Y = ", fY);
    throw(stcErr);
  }
...
void clGrid::GetPointOfCell(short int iCellX, short int iCellY,
  ...
  try {
    ...
    if (iCellX < 0 || iCellX >= m_iNumXCells ||
      ...
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::GetPointOfCell");
      sprintf(stcErr.cMoreInfo, "%s%s%s%d%s%d","Invalid grid numbers for grid ",
          m_cNameString, ".  X = ", iCellX, ", Y = ", iCellY);
      throw(stcErr);
    }
...
void clGrid::GetCellOfPoint(float fX, float fY, short int *iCellX,
  ...
  try {
    ...
    *iCellY = (int)floor(fY / m_fYCellSize);
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clGrid::GetCellOfPoint");
    throw(stcErr);
  }
}
...
void clGrid::DeletePackage(clPackage *p_oOldPackage){
  try {
    ...
    modelErr stcErr;
    stcErr.iErrorCode = UNKNOWN;
    strcpy(stcErr.cFunction, "clGrid::DeletePackage");
    throw(stcErr);
  }
}
...
clGrid::~clGrid() {
  ...
  for (i = 0; i < m_iNumXCells; i++) {
    for (j = 0; j < m_iNumYCells; j++) {
      ...
      for (k = 0; k < m_iNumCharVals; k++)
        delete[] mp_gridVals[i][j].p_cCharVals[k];
      delete[] mp_gridVals[i][j].p_cCharVals;
  ...
  delete[] mp_cIntLabels; mp_cIntLabels = NULL;
  delete[] mp_cFloatLabels; mp_cFloatLabels = NULL;
  delete[] mp_cCharLabels; mp_cCharLabels = NULL;
  delete[] mp_cBoolLabels; mp_cBoolLabels = NULL;
  ...
  for (i = 0; i < m_iNumPackageFloatVals; i++)
    delete[] mp_cPackageFloatLabels[i];
  for (i = 0; i < m_iNumPackageCharVals; i++)
    delete[] mp_cPackageCharLabels[i];
  for (i = 0; i < m_iNumPackageBoolVals; i++)
    delete[] mp_cPackageBoolLabels[i];
  delete[] mp_cPackageIntLabels; mp_cPackageIntLabels = NULL;
  delete[] mp_cPackageFloatLabels; mp_cPackageFloatLabels = NULL;
  delete[] mp_cPackageCharLabels; mp_cPackageCharLabels = NULL;
  delete[] mp_cPackageBoolLabels; mp_cPackageBoolLabels = NULL;
...
void clGrid::GetData(DOMDocument *p_oDoc) {
  try {
    if (NULL == p_oDoc) { //throw error
      ...
      strcpy(stcErr.cFunction, "clGrid::GetData");
      strcpy(stcErr.cMoreInfo, "DOM document pointer passed was NULL.");
      throw(stcErr);
    }
...
void clGrid::GetValueAtPoint(float fX, float fY, short int iCode,
  ...
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumIntVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::GetValueAtPoint (int)");
...
void clGrid::GetValueAtPoint(float fX, float fY,  short int iCode,
  ...
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumFloatVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::GetValueAtPoint (float)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    }
...
void clGrid::GetValueAtPoint(float fX, float fY, short int iCode,
  ...
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumIntVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::GetValueAtPoint (char)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    }
    ...
    //Return the value pointed to by the indexes
    strcpy(p_cValHolder, mp_gridVals[iXGrid][iYGrid].p_cCharVals[iCode]);
  }
  catch (modelErr&err) {throw(err);}
  catch (modelMsg &msg) {throw(msg);} //non-fatal error
  catch (...) {
...
void clGrid::GetValueAtPoint(float fX, float fY, short int iCode,
  ...
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumBoolVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::GetValueAtPoint (bool)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    }
...
void clGrid::GetValueOfCell(int iX, int iY, short int iCode,
  ...
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumIntVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::GetValueOfCell (int)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
      ...
      sprintf(stcErr.cMoreInfo, "%s%d%s%d",
          "At least one grid coordinate is outside of grid - X = ", iX, " & Y = ",
          iY);
      throw(stcErr);
    }
...
void clGrid::GetValueOfCell(int iX, int iY, short int iCode,
  ...
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumFloatVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::GetValueOfCell (float)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    } else if (iX >= m_iNumXCells || iX < 0 || iY >= m_iNumYCells || iY < 0) {
      modelErr stcErr;
...
void clGrid::GetValueOfCell(int iX, int iY, short int iCode,
  ...
  try {
    //Make sure that the code is valid and that the grid coordinates are OK -
    //if not throw error
    if (iCode < 0 || iCode >= m_iNumCharVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::GetValueOfCell (char)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    } else if (iX >= m_iNumXCells || iX < 0 || iY >= m_iNumYCells || iY < 0) {
      modelErr stcErr;
    ...
    strcpy(p_cValHolder, mp_gridVals[iX][iY].p_cCharVals[iCode]);
...
void clGrid::GetValueOfCell(int iX, int iY, short int iCode,
  ...
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumBoolVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::GetValueOfCell (bool)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    } else if (iX >= m_iNumXCells || iX < 0 || iY >= m_iNumYCells || iY < 0) {
      modelErr stcErr;
...
float clGrid::GetAverageValue(float fX, float fY, short int iCode,
  ...
  if (0 > iCode || (bFloat && iCode >= m_iNumFloatVals)
    ...
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clGrid::GetAverageValue");
    stcErr.iErrorCode = BAD_DATA;
    sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
    throw(stcErr);
    ...
    strcpy(stcErr.cFunction, "clGrid::GetAverageValue");
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cMoreInfo, "Radius cannot be less than 0.");
    throw(stcErr);
  }
...
float clGrid::GetAverageValue(float fFromX, float fFromY, float fToX, float fToY,
  ...
  if (0 > iCode || (bFloat && iCode >= m_iNumFloatVals)
    ...
    strcpy(stcErr.cFunction, "clGrid::GetAverageValue");
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cMoreInfo, "At least one grid coordinate is outside of grid.");
    throw(stcErr);
  } else if (fFromX > fToX || fFromY > fToY) {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clGrid::GetAverageValue");
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cMoreInfo, "The rectangle has negative area.");
    throw(stcErr);
  }
...
float clGrid::GetOriginXOfCell(int iX) {
  if (iX < 0 || iX >= m_iNumXCells) {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clGrid::GetOriginXOfCell");
    strcpy(stcErr.cMoreInfo, "X grid number is not inside the plot.");
...
float clGrid::GetOriginYOfCell(int iY) {
  if (iY < 0 || iY >= m_iNumYCells) {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clGrid::GetOriginYOfCell");
    strcpy(stcErr.cMoreInfo, "Y grid number is not inside the plot.");
    stcErr.iErrorCode = BAD_ARGUMENT;
    throw(stcErr);
  }
...
float clGrid::GetEndXOfCell(int iX) {
  if (iX < 0 || iX >= m_iNumXCells) {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clGrid::GetEndXOfCell");
    strcpy(stcErr.cMoreInfo, "X grid number is not inside the plot.");
    stcErr.iErrorCode = BAD_ARGUMENT;
    throw(stcErr);
  }
...
float clGrid::GetEndYOfCell(int iY) {
  if (iY < 0 || iY >= m_iNumYCells) {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clGrid::GetEndYOfCell");
    strcpy(stcErr.cMoreInfo, "Y grid number is not inside the plot.");
    stcErr.iErrorCode = BAD_ARGUMENT;
    throw(stcErr);
  }
...
void clGrid::SetValueAtPoint(float fX, float fY, short int iCode,
  ...
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumFloatVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::SetValueAtPoint (float)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    } else if (fX >= m_fXPlotLength || fX < 0
        || fY >= m_fYPlotLength || fY < 0) {
      ...
      strcpy(stcErr.cFunction, "clGrid::SetValueAtPoint (float)");
      sprintf(stcErr.cMoreInfo, "%s%g%s%g",
          "At least one grid coordinate is outside of grid - X = ", fX,
          " & Y = ", fY);
      throw(stcErr);
    }
...
void clGrid::SetValueAtPoint(float fX, float fY, short int iCode,
  ...
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumIntVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::SetValueAtPoint (int)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    } else if (fX >= m_fXPlotLength || fX < 0
        || fY >= m_fYPlotLength || fY < 0) {
...
void clGrid::SetValueAtPoint(float fX, float fY, short int iCode,
  ...
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumCharVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::SetValueAtPoint (char)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    } else if (fX >= m_fXPlotLength || fX < 0
        || fY >= m_fYPlotLength || fY < 0) {
    ...
    strncpy(mp_gridVals[iXGrid][iYGrid].p_cCharVals[iCode], p_cValue,
        MAX_GRID_CHAR_VALUE_SIZE - 1);
    //This doesn't always happen with strncpy - make it explicit
    mp_gridVals[iXGrid][iYGrid].p_cCharVals[iCode][MAX_GRID_CHAR_VALUE_SIZE - 1]
                                                   = '\0';
  }
  catch (modelErr&err) {throw(err);}
  catch (modelMsg &msg) {throw(msg);} //non-fatal error
...
void clGrid::SetValueAtPoint(float fX, float fY, short int iCode,
  ...
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumBoolVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::SetValueAtPoint (bool)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    } else if (fX >= m_fXPlotLength || fX < 0
        || fY >= m_fYPlotLength || fY < 0) {
...
void clGrid::SetValueOfCell(int iX, int iY, short int iCode, float fValue) {
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumFloatVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::SetValueOfCell (float)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    } else if (iX >= m_iNumXCells || iX < 0 || iY >= m_iNumYCells || iY < 0) {
      modelErr stcErr;
      ...
      strcpy(stcErr.cFunction, "clGrid::SetValueOfCell (float)");
      sprintf(stcErr.cMoreInfo, "%s%d%s%d",
          "At least one grid coordinate is outside of grid - X = ", iX,
          " & Y = ", iY);
      throw(stcErr);
    }
...
void clGrid::SetValueOfCell(int iX, int iY, short int iCode, int iValue) {
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumIntVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::SetValueOfCell (int)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    } else if (iX >= m_iNumXCells || iX < 0 || iY >= m_iNumYCells || iY < 0) {
      modelErr stcErr;
...
// SetValueOfCell()
/////////////////////////////////////////////////////////////////////////////
void clGrid::SetValueOfCell(int iX, int iY, short int iCode,char *p_cValue){
  try {
    //Make sure the data member code is valid and coordinates are within the
    //grid.  If not, throw error
    if (iCode < 0 || iCode >= m_iNumCharVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::SetValueOfCell (char)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    } else if (iX >= m_iNumXCells || iX < 0 || iY >= m_iNumYCells || iY < 0) {
      modelErr stcErr;
    ...
    strncpy(mp_gridVals[iX][iY].p_cCharVals[iCode], p_cValue,
        MAX_GRID_CHAR_VALUE_SIZE - 1);
    //This doesn't always happen with strncpy - make it explicit
    mp_gridVals[iX][iY].p_cCharVals[iCode][MAX_GRID_CHAR_VALUE_SIZE - 1]
                                           = '\0';
  } //end of try block
  catch (modelErr&err) {throw(err);}
  catch (modelMsg &msg) {throw(msg);} //non-fatal error
...
void clGrid::SetValueOfCell(int iX, int iY, short int iCode, bool bValue) {
  try {
    ...
    if (iCode < 0 || iCode >= m_iNumBoolVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::SetValueOfCell (bool)");
      sprintf(stcErr.cMoreInfo, "%s%d", "Invalid data code - ", iCode);
      throw(stcErr);
    } else if (iX >= m_iNumXCells || iX < 0 || iY >= m_iNumYCells || iY < 0) {
      modelErr stcErr;
...
void clGrid::ReadMapFile(DOMDocument *p_oDoc) {
  try {
    ...
    *p_cohChars = NULL, *p_cohBools = NULL;
    ...
    cTemp[] = "";         //For initializing grid values
    ...
    iNumCohInts, iNumCohFloats, iNumCohChars, iNumCohBools,
    ...
    if (0 == iNumElements) //nothing there - exit
      ...
      for (i = 0; i < iNumElements; i++) {
        ...
        XMLString::release(&sVal);
        if (strcmp(cData, m_cNameString) == 0) {
          p_oGrid = (DOMElement *) p_oNode;
          bFound = true;
    ...
    if (fMapXLen != p_oPlot->GetXPlotLength()
      ...
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
      sprintf(stcErr.cMoreInfo, "%s%g%s%g",
    ...
    if (fCellXLen != m_fXCellSize || fCellYLen != m_fYCellSize) {
      ...
      strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
      sprintf(stcErr.cMoreInfo, "%s%g%s%g",
          "One of the grid cell sizes for this map doesn't match.  X = ",
      ...
      if (iNumInts > 0) {
        ...
        for (i = 0; i < iNumInts; i++) {
          ...
          if (0 == p_ints[i].iCodeInMap && strcmp(cData, "0") != 0) {
            ...
            stcErr.iErrorCode = BAD_DATA;
            strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
            sprintf(stcErr.cMoreInfo, "%s%s", "Can't convert code to number: ",
                cData);
            delete[] cData; cData = NULL;
            throw(stcErr);
          }
          ...
          if (-1 == p_ints[i].iCodeForGrid) { //unrecognized label - throw error
            ...
            stcErr.iErrorCode = BAD_DATA;
            strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
            sprintf(stcErr.cMoreInfo, "%s%s", "Unrecognized data member label: "
                , cData);
            delete[] cData; cData = NULL;
            throw(stcErr);
    ...
    if (0 != p_oNodeList->getLength()) {
      ...
      if (iNumChars > 0) {
        //Declare the translation array for the int codes
        p_chars = new stcCodeTrans[iNumChars];
        for (i = 0; i < iNumChars; i++) {
          ...
          cData = XMLString::transcode(p_oElement->getAttributeNode(
              sVal)->getNodeValue());
          XMLString::release(&sVal);
          p_chars[i].iCodeForGrid = GetCharDataCode(cData);
    ...
    if (0 != p_oNodeList->getLength()) {
      ...
      p_oChildren = p_oElement->getElementsByTagName(sVal);
      XMLString::release(&sVal);
      iNumCohChars = p_oChildren->getLength();
      if (iNumCohChars > 0) {
        //Declare the translation array for the int codes
        p_cohChars = new stcCodeTrans[iNumCohChars];
        for (i = 0; i < iNumCohChars; i++) {
          //Get the code for this data member according to the map
          ...
          cData = XMLString::transcode(p_oElement->getAttributeNode(
              sVal)->getNodeValue());
          XMLString::release(&sVal);
          p_cohChars[i].iCodeForGrid = GetPackageCharDataCode(cData);
    ...
    for (i = 0; i < m_iNumXCells; i++)
      for (j = 0; j < m_iNumYCells; j++) {
        ...
        for (k = 0; k < m_iNumCharVals; k++) SetValueOfCell(i, j, k, cTemp);
    ...
    for (i = 0; i < iNumGridVals; i++) {
      ...
      if (NULL == p_oCell) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
        sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Can't find cell # ", i,
            " for grid ", m_cNameString);
        throw(stcErr);
      }
      ...
      if (NULL == p_oAttr) {
        ...
        stcErr.iErrorCode = BAD_DATA;
        strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
        sprintf(stcErr.cMoreInfo, "%s%s", "Can't find X attribute for a cell for grid ",
            m_cNameString);
        throw(stcErr);
      }
      ...
      for (j = 0; j < iNumCellVals; j++) {
        ...
        if (-1 == iGridCode) { //unrecognized code - throw error
          ...
          strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
          stcErr.iErrorCode = BAD_DATA;
          sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Unrecognized int code ",
              iMapCode, " in map ", m_cNameString);
          throw(stcErr);
        }
        cData =
            XMLString::transcode(p_oElement->getFirstChild()->getNodeValue());
      ...
      for (j = 0; j < iNumCellVals; j++) {
        ...
        if (-1 == iGridCode) { //unrecognized code - throw error
          ...
          strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
          stcErr.iErrorCode = BAD_DATA;
          sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Unrecognized float code ",
              iMapCode, " in map ", m_cNameString);
          throw(stcErr);
        }
        cData =
      ...
      for (j = 0; j < iNumCellVals; j++) {
        ...
        if (-1 == iGridCode) { //unrecognized code - throw error
          ...
          strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
          stcErr.iErrorCode = BAD_DATA;
          sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Unrecognized char code ",
              iMapCode, " in map ", m_cNameString);
          throw(stcErr);
        }
        cData =
            XMLString::transcode(p_oElement->getFirstChild()->getNodeValue());
        delete[] cData; cData = NULL;
        SetValueOfCell(iXGridVal, iYGridVal, iGridCode, cData);
      ...
      for (j = 0; j < iNumCellVals; j++) {
        ...
        if (-1 == iGridCode) { //unrecognized code - throw error
          ...
          strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
          stcErr.iErrorCode = BAD_DATA;
          sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Unrecognized bool code ",
              iMapCode, " in map ", m_cNameString);
          throw(stcErr);
        }
        cData =
      ...
      for (m = 0; m < iNumPackages; m++) {
        ...
        for (j = 0; j < iNumCellVals; j++) {
          ...
          if (-1 == iGridCode) { //unrecognized code - throw error
            ...
            strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
            stcErr.iErrorCode = BAD_DATA;
            sprintf(stcErr.cMoreInfo, "%s%s%d%s%s", "Unrecognized package ",
                "int code ", iMapCode, " in map ", m_cNameString);
            throw(stcErr);
          }
          cData =
              XMLString::transcode(p_oElement->getFirstChild()->getNodeValue());
          ...
          if (0 == iTemp && strcmp(cData, "0") != 0) {
            ...
            strcpy(stcErr.cFunction, "clGrid::ReadMapFile");
            stcErr.iErrorCode = BAD_DATA;
            sprintf(stcErr.cMoreInfo, "%s%s",
                "Invalid int value in map: ", cData);
            delete[] cData; cData = NULL;
            throw(stcErr);
          }
          delete[] cData; cData = NULL;
        ...
        for (j = 0; j < iNumCellVals; j++) {
          ...
          if (-1 == iGridCode) { //unrecognized code - throw error
            ...
            stcErr.iErrorCode = BAD_DATA;
            sprintf(stcErr.cMoreInfo, "%s%s%d%s%s", "Unrecognized package ",
                "float code ", iMapCode, " in map ", m_cNameString);
            throw(stcErr);
          }
          cData =
              XMLString::transcode(p_oElement->getFirstChild()->getNodeValue());
          ...
          if (0 == fTemp && cData[0] != '0') {
            ...
            stcErr.iErrorCode = BAD_DATA;
            sprintf(stcErr.cMoreInfo, "%s%s",
                "Invalid float value in map: ",cData);
            delete[] cData; cData = NULL;
            throw(stcErr);
          }
          delete[] cData; cData = NULL;
        ...
        for (j = 0; j < iNumCellVals; j++) {
          ...
          if (-1 == iGridCode) { //unrecognized code - throw error
            ...
            stcErr.iErrorCode = BAD_DATA;
            sprintf(stcErr.cMoreInfo, "%s%s%d%s%s", "Unrecognized package ",
                "char code ", iMapCode, " in map ", m_cNameString);
            throw(stcErr);
          }
          cData =
              XMLString::transcode(p_oElement->getFirstChild()->getNodeValue());
        ...
        for (j = 0; j < iNumCellVals; j++) {
          ...
          if (-1 == iGridCode) { //unrecognized code - throw error
            ...
            stcErr.iErrorCode = BAD_DATA;
            sprintf(stcErr.cMoreInfo, "%s%s%d%s%s", "Unrecognized package ",
                "bool code ", iMapCode, " in map ", m_cNameString);
            throw(stcErr);
          }
          cData =
              XMLString::transcode(p_oElement->getFirstChild()->getNodeValue());
          if (strcmp(cData, "true") == 0) bTemp = true;
            ...
            stcErr.iErrorCode = BAD_DATA;
            sprintf(stcErr.cMoreInfo,"%s%s",
                "Invalid bool value in map: ", cData);
            delete[] cData; cData = NULL;
            throw(stcErr);
          }
          delete[] cData; cData = NULL;
...
// RegisterDataMember()
//////////////////////////////////////////////////////////////////////////////
short int clGrid::RegisterDataMember(const char*cLabel, short int iNumVals,
    char **p_cLabels) {
  ...
  if (0 == strlen(cLabel)) {
    modelErr stcErr;
    strcpy(stcErr.cFunction, "clGrid::RegisterDataMember");
    stcErr.iErrorCode = BAD_DATA;
  ...
  if (-1 == iReturnCode) {
    ...
    strcpy(stcErr.cFunction, "clGrid::RegisterDataMember");
    stcErr.iErrorCode = ILLEGAL_OP;
    strcpy(stcErr.cMoreInfo, "Too many data member registrations.");
    throw(stcErr);
  }

  //Truncate the label if it's too long
  strncpy(p_cLabels[iReturnCode], cLabel, MAX_GRID_LABEL_SIZE - 1);
  //This doesn't always happen with strncpy - make it explicit
  p_cLabels[iReturnCode][MAX_GRID_LABEL_SIZE - 1] = '\0';
  return iReturnCode;
...
// RegisterInt()
//////////////////////////////////////////////////////////////////////////////
short int clGrid::RegisterInt(const char*cLabel) {
  short int iReturnCode;
  iReturnCode = RegisterDataMember(cLabel, m_iNumIntVals, mp_cIntLabels);
  //We may need to set up a package data member too
  if (!m_bPackageDataChanged)
    RegisterDataMember(cLabel, m_iNumPackageIntVals,
        mp_cPackageIntLabels);
  return iReturnCode;
}
...
// RegisterFloat()
//////////////////////////////////////////////////////////////////////////////
short int clGrid::RegisterFloat(const char*cLabel) {
  short int iReturnCode;
  iReturnCode = RegisterDataMember(cLabel, m_iNumFloatVals, mp_cFloatLabels);
  //We may need to set up a package data member too
  if (!m_bPackageDataChanged)
    RegisterDataMember(cLabel, m_iNumPackageFloatVals,
        mp_cPackageFloatLabels);
  return iReturnCode;
}
...
// RegisterChar()
//////////////////////////////////////////////////////////////////////////////
short int clGrid::RegisterChar(const char*cLabel) {
  short int iReturnCode;
  iReturnCode = RegisterDataMember(cLabel, m_iNumCharVals, mp_cCharLabels);
  //We may need to set up a package data member too
  if (!m_bPackageDataChanged)
    RegisterDataMember(cLabel, m_iNumPackageCharVals,
        mp_cPackageCharLabels);
  return iReturnCode;
}
...
// RegisterBool()
//////////////////////////////////////////////////////////////////////////////
short int clGrid::RegisterBool(const char*cLabel) {
  short int iReturnCode;
  iReturnCode = RegisterDataMember(cLabel, m_iNumBoolVals, mp_cBoolLabels);
  //We may need to set up a package data member too
  if (!m_bPackageDataChanged)
    RegisterDataMember(cLabel, m_iNumPackageBoolVals,
        mp_cPackageBoolLabels);
  return iReturnCode;
}
...
// RegisterPackageInt()
//////////////////////////////////////////////////////////////////////////////
short int clGrid::RegisterPackageInt(const char*cLabel) {
  return RegisterDataMember(cLabel, m_iNumPackageIntVals, mp_cPackageIntLabels);
}

//////////////////////////////////////////////////////////////////////////////
// RegisterPackageFloat()
//////////////////////////////////////////////////////////////////////////////
short int clGrid::RegisterPackageFloat(const char*cLabel) {
  return RegisterDataMember(cLabel,m_iNumPackageFloatVals,mp_cPackageFloatLabels);
}

//////////////////////////////////////////////////////////////////////////////
// RegisterPackageChar()
//////////////////////////////////////////////////////////////////////////////
short int clGrid::RegisterPackageChar(const char*cLabel) {
  return RegisterDataMember(cLabel, m_iNumPackageCharVals, mp_cPackageCharLabels);
}

//////////////////////////////////////////////////////////////////////////////
// RegisterPackageBool()
//////////////////////////////////////////////////////////////////////////////
short int clGrid::RegisterPackageBool(const char* cLabel) {
  return RegisterDataMember(cLabel, m_iNumPackageBoolVals, mp_cPackageBoolLabels);
}

//////////////////////////////////////////////////////////////////////////////
// GetDataCode()
//////////////////////////////////////////////////////////////////////////////
short int clGrid::GetIntDataCode(const char* cLabel) {
  char cSearcher[MAX_GRID_LABEL_SIZE];
  ...
  if (0 == strlen(cLabel)) {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::GetIntDataCode");
    strcpy(stcErr.cMoreInfo, "Data label is required.");
    throw stcErr;
  }
  ...
  //would have been
  strncpy(cSearcher, cLabel, MAX_GRID_LABEL_SIZE - 1);
  //This doesn't always happen with strncpy - make it explicit
  cSearcher[MAX_GRID_LABEL_SIZE - 1] = '\0';

  //Search for the label to return
  for (i = 0; i < m_iNumIntVals; i++)
    if (strcmp(mp_cIntLabels[i], cSearcher) == 0) {
      iReturnCode = i;
      break;
    }
  return iReturnCode;
...
//----------------------------------------------------------------------------
short int clGrid::GetFloatDataCode(const char *cLabel) {
  char cSearcher[MAX_GRID_LABEL_SIZE];
  short int iReturnCode = -1, i;
  ...
  if (0 == strlen(cLabel)) {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::GetFloatDataCode");
    strcpy(stcErr.cMoreInfo, "Data label is required.");
    throw stcErr;
  }
  ...
  //Search for the label to return
  for (i = 0; i < m_iNumFloatVals; i++)
    if (strcmp(mp_cFloatLabels[i], cSearcher) == 0) {
      iReturnCode = i;
      break;
    }
  return iReturnCode;
...
//----------------------------------------------------------------------------
short int clGrid::GetCharDataCode(const char *cLabel) {
  char cSearcher[MAX_GRID_LABEL_SIZE];
  short int iReturnCode = -1, i;
  ...
  if (0 == strlen(cLabel)) {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::GetCharDataCode");
    strcpy(stcErr.cMoreInfo, "Data label is required.");
    throw stcErr;
  }
  ...
  //Search for the label to return
  for (i = 0; i < m_iNumCharVals; i++)
    if (strcmp(mp_cCharLabels[i], cSearcher) == 0) {
      iReturnCode = i;
      break;
    }
  return iReturnCode;
...
//----------------------------------------------------------------------------
short int clGrid::GetBoolDataCode(const char *cLabel) {
  char cSearcher[MAX_GRID_LABEL_SIZE];
  short int iReturnCode = -1, i;
  ...
  if (0 == strlen(cLabel)) {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::GetBoolDataCode");
    strcpy(stcErr.cMoreInfo, "Data label is required.");
    throw stcErr;
  }
  ...
  //Search for the label to return
  for (i = 0; i < m_iNumBoolVals; i++)
    if (strcmp(mp_cBoolLabels[i], cSearcher) == 0) {
      iReturnCode = i;
      break;
    }
  return iReturnCode;
...
// GetPackageDataCode()
//////////////////////////////////////////////////////////////////////////////
short int clGrid::GetPackageIntDataCode(const char *cLabel) {
  short int iReturnCode = -1, i;
  ...
  if (0 == strlen(cLabel)) {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::GetPackageIntDataCode");
    strcpy(stcErr.cMoreInfo, "Data label is required.");
    throw stcErr;
  }
  ...
  //Search for the label to return
  for (i = 0; i < m_iNumPackageIntVals; i++)
    if (strcmp(mp_cPackageIntLabels[i], cLabel) == 0) {
      iReturnCode = i;
      break;
    }
  return iReturnCode;
...
//----------------------------------------------------------------------------
short int clGrid::GetPackageFloatDataCode(const char *cLabel) {
  short int iReturnCode = -1, i;
  ...
  if (0 == strlen(cLabel)) {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::GetPackageFloatDataCode");
    strcpy(stcErr.cMoreInfo, "Data label is required.");
    throw stcErr;
  }
  ...
  //Search for the label to return
  for (i = 0; i < m_iNumPackageFloatVals; i++)
    if (strcmp(mp_cPackageFloatLabels[i], cLabel) == 0) {
      iReturnCode = i;
      break;
    }
  return iReturnCode;
...
//----------------------------------------------------------------------------
short int clGrid::GetPackageCharDataCode(const char *cLabel) {
  short int iReturnCode = -1, i;
  ...
  if (0 == strlen(cLabel)) {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::GetPackageCharDataCode");
    strcpy(stcErr.cMoreInfo, "Data label is required.");
    throw stcErr;
  }
  ...
  //Search for the label to return
  for (i = 0; i < m_iNumPackageCharVals; i++)
    if (strcmp(mp_cPackageCharLabels[i], cLabel) == 0) {
      iReturnCode = i;
      break;
    }
  return iReturnCode;
...
//----------------------------------------------------------------------------
short int clGrid::GetPackageBoolDataCode(const char *cLabel) {
  short int iReturnCode = -1, i;
  ...
  if (0 == strlen(cLabel)) {
    modelErr stcErr;
    stcErr.iErrorCode = BAD_DATA;
    strcpy(stcErr.cFunction, "clGrid::GetPackageBoolDataCode");
    strcpy(stcErr.cMoreInfo, "Data label is required.");
    throw stcErr;
  }
  ...
  //Search for the label to return
  for (i = 0; i < m_iNumPackageBoolVals; i++)
    if (strcmp(mp_cPackageBoolLabels[i], cLabel) == 0) {
      iReturnCode = i;
      break;
    }
  return iReturnCode;
...
// GetDataLabel()
//////////////////////////////////////////////////////////////////////////////
const char* clGrid::GetIntDataLabel(short int iCode) {
  //If the code's invalid, return an empty string
  if (iCode < 0 || iCode >= m_iNumIntVals) return "";
  else return mp_cIntLabels[iCode];
}
//----------------------------------------------------------------------------
const char* clGrid::GetFloatDataLabel(short int iCode) {
  //If the code's invalid, return an empty string
  if (iCode < 0 || iCode >= m_iNumFloatVals) return "";
  else return mp_cFloatLabels[iCode];
}
//----------------------------------------------------------------------------
const char* clGrid::GetCharDataLabel(short int iCode) {
  //If the code's invalid, return an empty string
  if (iCode < 0 || iCode >= m_iNumCharVals) return "";
  else return mp_cCharLabels[iCode];
}
//----------------------------------------------------------------------------
const char* clGrid::GetBoolDataLabel(short int iCode) {
  //If the code's invalid, return an empty string
  if (iCode < 0 || iCode >= m_iNumBoolVals) return "";
  else return mp_cBoolLabels[iCode];
}
//----------------------------------------------------------------------------
const char* clGrid::GetPackageIntDataLabel(short int iCode) {
  //If the code's invalid, return an empty string
  if (iCode < 0 || iCode >= m_iNumPackageIntVals) return "";
  else return mp_cPackageIntLabels[iCode];
}
//----------------------------------------------------------------------------
const char* clGrid::GetPackageFloatDataLabel(short int iCode) {
  //If the code's invalid, return an empty string
  if (iCode < 0 || iCode >= m_iNumPackageFloatVals) return "";
  else return mp_cPackageFloatLabels[iCode];
}
//----------------------------------------------------------------------------
const char* clGrid::GetPackageCharDataLabel(short int iCode) {
  //If the code's invalid, return an empty string
  if (iCode < 0 || iCode >= m_iNumPackageCharVals) return "";
  else return mp_cPackageCharLabels[iCode];
}
//----------------------------------------------------------------------------
const char* clGrid::GetPackageBoolDataLabel(short int iCode) {
  //If the code's invalid, return an empty string
  if (iCode < 0 || iCode >= m_iNumPackageBoolVals) return "";
  else return mp_cPackageBoolLabels[iCode];
}
//----------------------------------------------------------------------------
...
clPackage::clPackage(clGrid *p_oParentGrid,
  ...
  try {
    ...
    if (NULL == p_oParentGrid || NULL == p_oParentCell) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPackage::clPackage");
      strcpy(stcErr.cMoreInfo, "Parent pointers cannot be NULL.");
      throw(stcErr);
    }
    ...
    //Char data members
    if (mp_oParentGrid->m_iNumPackageCharVals == 0) mp_cCharVals = NULL;
    else {
      mp_cCharVals = new char*[mp_oParentGrid->m_iNumPackageCharVals];
      for (k = 0; k < mp_oParentGrid->m_iNumPackageCharVals; k++) {
        mp_cCharVals[k] = new char[MAX_GRID_CHAR_VALUE_SIZE];
        strcpy(mp_cCharVals[k], "");
      }
    }
    //Bool data members
...
clPackage::~clPackage() {
  ...
  for (int i = 0; i < mp_oParentGrid->m_iNumPackageCharVals; i++)
    delete[] mp_cCharVals[i];
  delete[] mp_cCharVals;
  delete[] mp_bBoolVals;
}
...
void clPackage::SetValue(short int iCode, int iValue) {
  try {
    ...
    if (iCode < 0 || iCode >= mp_oParentGrid->m_iNumPackageIntVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPackage::SetValue(int)");
      sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Invalid data code:  ", iCode,
          " for grid ", mp_oParentGrid->m_cNameString);
      throw(stcErr);
    }
...
void clPackage::SetValue(short int iCode, float fValue) {
  try {
    ...
    if (iCode < 0 || iCode >= mp_oParentGrid->m_iNumPackageFloatVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPackage::SetValue(float)");
      sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Invalid data code:  ", iCode,
          " for grid ", mp_oParentGrid->m_cNameString);
      throw(stcErr);
    }
...
//---------------------------------------------------------------------------
void clPackage::SetValue(short int iCode, char *p_cValue) {
  try {
    //Verify the code
    if (iCode < 0 || iCode >= mp_oParentGrid->m_iNumPackageCharVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPackage::SetValue(char)");
      sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Invalid data code:  ", iCode,
          " for grid ", mp_oParentGrid->m_cNameString);
      throw(stcErr);
    }
    strncpy(mp_cCharVals[iCode], p_cValue, MAX_GRID_CHAR_VALUE_SIZE - 1);
    //This doesn't always happen with strncpy - make it explicit
    mp_cCharVals[iCode][MAX_GRID_CHAR_VALUE_SIZE - 1] = '\0';
  }
  catch (modelErr&err) {throw(err);}
  catch (modelMsg &msg) {throw(msg);} //non-fatal error
  catch (...) {
...
void clPackage::SetValue(short int iCode, bool bValue) {
  try {
    ...
    if (iCode < 0 || iCode >= mp_oParentGrid->m_iNumPackageBoolVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPackage::SetValue(bool)");
      sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Invalid data code:  ", iCode,
          " for grid ", mp_oParentGrid->m_cNameString);
      throw(stcErr);
    }
...
void clPackage::GetValue(short int iCode, int *p_iValHolder) {
  try {
    ...
    if (iCode < 0 || iCode >= mp_oParentGrid->m_iNumPackageIntVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPackage::GetValue(int)");
      sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Invalid data code:  ", iCode,
          " for grid ", mp_oParentGrid->m_cNameString);
      throw(stcErr);
    }
...
void clPackage::GetValue(short int iCode, float *p_fValHolder) {
  try {
    ...
    if (iCode < 0 || iCode >= mp_oParentGrid->m_iNumPackageFloatVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPackage::GetValue(float)");
      sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Invalid data code:  ", iCode,
          " for grid ", mp_oParentGrid->m_cNameString);
      throw(stcErr);
    }
...
//---------------------------------------------------------------------------
void clPackage::GetValue(short int iCode, char *p_cValHolder) {
  try {
    //Verify the code
    if (iCode < 0 || iCode >= mp_oParentGrid->m_iNumPackageCharVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPackage::GetValue(char)");
      sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Invalid data code:  ", iCode,
          " for grid ", mp_oParentGrid->m_cNameString);
      throw(stcErr);
    }
    strcpy(p_cValHolder, mp_cCharVals[iCode]);
  }
  catch (modelErr&err) {throw(err);}
  catch (modelMsg &msg) {throw(msg);} //non-fatal error
  catch (...) {
...
void clPackage::GetValue(short int iCode, bool *p_bValHolder) {
  try {
    ...
    if (iCode < 0 || iCode >= mp_oParentGrid->m_iNumPackageBoolVals) {
      modelErr stcErr;
      stcErr.iErrorCode = BAD_DATA;
      strcpy(stcErr.cFunction, "clPackage::GetValue(bool)");
      sprintf(stcErr.cMoreInfo, "%s%d%s%s", "Invalid data code:  ", iCode,
          " for grid ", mp_oParentGrid->m_cNameString);
      throw(stcErr);
    }
