...
// CreateGrid
/////////////////////////////////////////////////////////////////////////////
clGrid* clGridManager::CreateGrid(const char *cGridName,
  ...
  //Create our grid
  p_oReturnGrid =
      new clGrid(mp_oSimManager, cGridName, iNumIntVals, iNumFloatVals,
          iNumCharVals, iNumBoolVals, fXCellLength, fYCellLength);
  ...
  for (i = 0; i < m_iNumObjects; i++)
    if (strcmp(cGridName, mp_oObjectArray[i]->GetName()) == 0) {
...
void clGridManager::CreateObjects(DOMDocument *p_oDoc) {
  try {
    ...
    char *cData,                  //for capturing text data
         *cGridName;              //name of the grid from the map file
    ...
    for (i = 0; i < iNumGrids; i++) {
      ...
      cGridName = XMLString::transcode(p_oGridEl->getAttributeNode(sVal)->getNodeValue());
      XMLString::release(&sVal);

      //Do we already have this grid object?
      p_oTemp = PassObjectPointer(cGridName);
      ...
      p_oGrid = CreateGrid(cGridName, iNumInts, iNumFloats, iNumChars,
                           iNumBools, fCellXLen, fCellYLen);
      delete[] cGridName; cGridName = NULL;
        ...
        if (0 != p_oNodeList->getLength()) {
          ...
          for (j = 0; j < iNumPackageInts; j++) {
            ...
            cData = XMLString::transcode(p_oElement->getAttributeNode(
                     sVal)->getNodeValue());
            XMLString::release(&sVal);
            p_oGrid->RegisterPackageInt(cData);
            delete[] cData; cData = NULL;
        ...
        if (0 != p_oNodeList->getLength()) {
          ...
          for (j = 0; j < iNumPackageFloats; j++) {
            ...
            cData = XMLString::transcode(p_oElement->getAttributeNode(
                     sVal)->getNodeValue());
            XMLString::release(&sVal);
            p_oGrid->RegisterPackageFloat(cData);
            delete[] cData; cData = NULL;
        ...
        if (0 != p_oNodeList->getLength()) {
          ...
          for (j = 0; j < iNumPackageChars; j++) {
            ...
            cData = XMLString::transcode(p_oElement->getAttributeNode(
                     sVal)->getNodeValue());
            XMLString::release(&sVal);
            p_oGrid->RegisterPackageChar(cData);
            delete[] cData; cData = NULL;
        ...
        if (0 != p_oNodeList->getLength()) {
          ...
          for (j = 0; j < iNumPackageBools; j++) {
            ...
            cData = XMLString::transcode(p_oElement->getAttributeNode(
                     sVal)->getNodeValue());
            XMLString::release(&sVal);
            p_oGrid->RegisterPackageBool(cData);
            delete[] cData; cData = NULL;
    ...
    strcpy(stcErr.cFunction, "clGridManager::CreateObjects");
    throw(stcErr);
  }
}
//----------------------------------------------------------------------------
